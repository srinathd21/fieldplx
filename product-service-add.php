<?php
/**
 * FieldPlx - Add Product / Service
 *
 * Upload as:
 * /public_html/product-service-add.php
 *
 * PHP 7.2+ / MySQLi
 */
...
if (
    ...
    header(
        ...
        rawurlencode('product-service-add.php')
    );
    exit;
}
...
if (function_exists('requirePermission')) {
    requirePermission(
        ...
        'You do not have permission to add products or services.'
    );
}
...
$pageTitle = 'Add Product / Service - FieldPlx';
$activePage = 'product-service-add';
...
if (!function_exists('psAddFetchAll')) {
    function psAddFetchAll(mysqli_stmt $stmt)
    {
        $rows = array();
...
if (!function_exists('psAddOld')) {
    function psAddOld($key, $default = '')
    {
        if (!isset($_POST[$key])) {
            return $default;
        }
...
if (!function_exists('psAddNullable')) {
    function psAddNullable($value)
    {
...
if (!function_exists('psAddCsrfToken')) {
    function psAddCsrfToken()
    {
...
if (!function_exists('psAddVerifyCsrf')) {
    function psAddVerifyCsrf($token)
    {
...
if (!function_exists('psAddLogActivity')) {
    function psAddLogActivity(
        mysqli $conn,
        $tenantId,
        $userId,
        $itemId,
...
if (!function_exists('psAddDefaultUnitForType')) {
    function psAddDefaultUnitForType(
        $itemType,
...
$initialItemType = psAddOld(
    'item_type',
    'service'
);

$defaultUnitName =
    psAddDefaultUnitForType(
        $initialItemType,
        $defaultUnitsByItemType,
        $unitNamesByLowercase,
        $unitMeasurements
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $name = psAddOld('name');
    $sku = psAddOld('sku');
    $description = psAddOld('description');
    $unitName = psAddOld(
        'unit_name',
        $defaultUnitName
    );

    $unitCostRaw = psAddOld(
        'unit_cost',
        '0'
    );

    $unitPriceRaw = psAddOld(
        'unit_price',
        '0'
    );
    ...
    /*
     * This value must finally contain a valid tax_rates.id
     * because product_services.tax_rate_id has a foreign key
     * to the tenant tax_rates table.
     */
    $taxRateId = null;
    ...
    /*
     * Validate the selected platform tax and resolve it to a
     * tenant tax_rates.id required by the foreign key.
     */
    if (
        empty($errors) &&
        ...
        if (empty($errors)) {
            /*
             * First reuse an existing active tenant tax rate
             * having the same name and percentage.
             */
            $stmt = $conn->prepare("
                SELECT id
                FROM tax_rates
        ...
        if (
            ...
            /*
             * The platform tax does not yet exist for this tenant,
             * so create a tenant copy and use its generated ID.
             */
            $taxType = 'percentage';
            $isDefaultTax = 0;
            $isActiveTax = 1;
    ...
    if (empty($errors)) {
        ...
        if (!$stmt) {
            ...
            if ($stmt->num_rows > 0) {
                $errors[] =
                    'An item with the same name or SKU already exists.';
            }
    ...
    if (empty($errors)) {
        $skuValue = psAddNullable($sku);
        $descriptionValue =
            psAddNullable($description);
        $unitNameValue =
            psAddNullable($unitName);
        ...
        if (!$stmt) {
            $errors[] =
                'Unable to prepare the save operation.';
        } else {
            /*
             * 12 variables / 12 type characters:
             * i s s s s s d d i i i s
             */
            $stmt->bind_param(
                'isssssddiiis',
                $tenantId,
            ...
            $errors[] =
                'Product or service could not be saved: ' .
                $stmt->error;
    ...
    <div class="pa-header">
        <div class="pa-header-main">
            ...
            <div>
                ...
                <p>
                    Create a reusable item for quotations, jobs, invoices, and bookings.
                </p>
            </div>
        </div>
            ...
            <main>
                ...
                <section class="pa-card">
                    <div class="pa-card-head">
                        ...
                        <p>
                            Enter the item name, code, description, and unit.
                        </p>
                    </div>
                        ...
                        <div class="pa-grid">
                            <div class="pa-field full">
                                ...
                                <input
                                    ...
                                    class="pa-input"
                                    maxlength="190"
                                    value="<?= e(psAddOld('name')); ?>"
                                    placeholder="Example: AC Service, Paint Material, Site Visit Fee"
                                    required
                                >
                            </div>
                            ...
                            <div class="pa-field full">
                                ...
                                ><?= e(psAddOld('description')); ?></textarea>
                            </div>
                        </div>
                    </div>
                </section>
                ...
                <section class="pa-card">
                    <div class="pa-card-head">
                        <h2>Pricing & Tax</h2>
                        <p>
                            Set cost, selling price, and applicable tax.
                        </p>
                    </div>
                                ...
                                >
                                    ...
                                    <?php foreach ($taxRates as $taxRate): ?>
                                        <option
                                            ...
                                            <?= (int) psAddOld('system_tax_rate_id') ===
                                                (int) $taxRate['id']
                                                    ? 'selected'
                                                    : ''; ?>
                                        >
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var unitDefaultsByType = <?= json_encode(
        array(
            'service' => psAddDefaultUnitForType(
                'service',
                $defaultUnitsByItemType,
                $unitNamesByLowercase,
                $unitMeasurements
            ),
            'product' => psAddDefaultUnitForType(
                'product',
                $defaultUnitsByItemType,
                $unitNamesByLowercase,
                $unitMeasurements
            ),
            'material' => psAddDefaultUnitForType(
                'material',
                $defaultUnitsByItemType,
                $unitNamesByLowercase,
                $unitMeasurements
            ),
            'fee' => psAddDefaultUnitForType(
                'fee',
                $defaultUnitsByItemType,
                $unitNamesByLowercase,
                $unitMeasurements
            ),
            'discount' => psAddDefaultUnitForType(
                'discount',
                $defaultUnitsByItemType,
                $unitNamesByLowercase,
                $unitMeasurements
