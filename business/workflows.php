...
<head>
    ...
    <title>Workflows - FieldPlx</title>
...
<section class="wf-head"><div><h1>Workflows</h1><p>Service-specific prebuilt technician work processes. Each workflow defines exactly what must be completed for a selected service.</p></div><div class="wf-actions"><button class="wf-btn" id="refreshBtn"><i class="bi bi-arrow-clockwise"></i> Refresh</button><a class="wf-btn primary" href="add-workflow.php"><i class="bi bi-plus-lg"></i> Add Workflow</a></div></section>
<section class="row g-3 mb-3"><div class="col-xl-3 col-md-6"><div class="wf-stat"><div class="wf-stat-row"><span class="wf-stat-icon"><i class="bi bi-diagram-3"></i></span><div><span class="wf-stat-label">Total Workflows</span><strong class="wf-stat-value" id="statTotal">0</strong></div></div></div></div><div class="col-xl-3 col-md-6"><div class="wf-stat"><div class="wf-stat-row"><span class="wf-stat-icon"><i class="bi bi-tools"></i></span><div><span class="wf-stat-label">Services Configured</span><strong class="wf-stat-value" id="statServices">0</strong></div></div></div></div><div class="col-xl-3 col-md-6"><div class="wf-stat"><div class="wf-stat-row"><span class="wf-stat-icon"><i class="bi bi-list-check"></i></span><div><span class="wf-stat-label">Process Steps</span><strong class="wf-stat-value" id="statSteps">0</strong></div></div></div></div><div class="col-xl-3 col-md-6"><div class="wf-stat"><div class="wf-stat-row"><span class="wf-stat-icon"><i class="bi bi-ui-checks-grid"></i></span><div><span class="wf-stat-label">Builder Fields</span><strong class="wf-stat-value" id="statFields">0</strong></div></div></div></div></section>
<section class="wf-card"><div class="wf-toolbar"><div class="wf-search"><i class="bi bi-search"></i><input id="searchInput" type="search" placeholder="Search service, workflow or code"></div><select class="wf-filter" id="statusFilter"><option value="">All Status</option><option value="draft">Draft</option><option value="active">Active</option><option value="inactive">Inactive</option><option value="archived">Archived</option></select><select class="wf-filter" id="serviceFilter"><option value="">All Services</option></select><div class="wf-spacer"></div><button class="wf-btn" id="clearBtn"><i class="bi bi-x-circle"></i> Clear</button></div><div class="wf-table-wrap"><table class="wf-table"><thead><tr><th>S/No</th><th>Service</th><th>Workflow</th><th>Steps</th><th>Fields</th><th>Version</th><th>Status</th><th>Updated</th><th>Action</th></tr></thead><tbody id="workflowBody"><tr><td colspan="9" class="wf-empty">Loading workflows...</td></tr></tbody></table></div><div class="wf-pagination"><span id="countText">Showing 0 workflows</span><div><button class="wf-btn" id="prevBtn"><i class="bi bi-chevron-left"></i></button> <button class="wf-btn" id="nextBtn"><i class="bi bi-chevron-right"></i></button></div></div></section>
...
<script>(function(){'use strict';var csrf=<?= json_encode($workflowsCsrfToken) ?>;var state={page:1,search:'',status:'',service:''},timer=null,searchTimer=null;var body=document.getElementById('workflowBody'),toast=document.getElementById('toast'),msg=document.getElementById('toastMessage');function esc(v){return String(v==null?'':v).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#039;')}function note(t,m){if(timer)clearTimeout(timer);toast.className='wf-toast '+t+' show';msg.textContent=m;timer=setTimeout(function(){toast.classList.remove('show')},3000)}function req(fd){fd.append('csrf_token',csrf);return fetch('api/workflows.php',{method:'POST',body:fd,credentials:'same-origin',headers:{'X-Requested-With':'XMLHttpRequest','Accept':'application/json'}}).then(function(r){return r.text().then(function(x){var d;try{d=x.trim()?JSON.parse(x):{}}catch(e){throw new Error(x.replace(/<[^>]*>/g,' ').replace(/\s+/g,' ').trim()||'Invalid server response.')}if(!r.ok||!d.success)throw new Error(d.message||'Request failed.');return d})})}function load(){var fd=new FormData();fd.append('action','list');fd.append('page',state.page);fd.append('search',state.search);fd.append('status',state.status);fd.append('service_id',state.service);req(fd).then(function(d){var rows=d.workflows||[],html='';if(!rows.length)html='<tr><td colspan="9" class="wf-empty">No workflows found.</td></tr>';rows.forEach(function(r,i){html+='<tr><td>'+(((state.page-1)*10)+i+1)+'</td><td><strong>'+esc(r.service_name||'-')+'</strong><br><small>'+esc(r.service_sku||'')+'</small></td><td><strong>'+esc(r.name)+'</strong><br><small>'+esc(r.code||'-')+'</small></td><td>'+Number(r.step_count||0)+'</td><td>'+Number(r.field_count||0)+'</td><td>v'+Number(r.version_no||1)+'</td><td><span class="wf-badge '+esc(r.status)+'">'+esc(r.status)+'</span></td><td>'+esc(r.updated_at||r.created_at||'-')+'</td><td><a class="wf-icon-btn" href="view-workflow.php?id='+Number(r.id)+'" title="View"><i class="bi bi-eye"></i></a><a class="wf-icon-btn" href="edit-workflow.php?id='+Number(r.id)+'" title="Edit"><i class="bi bi-pencil"></i></a><button class="wf-icon-btn" data-action="duplicate" data-id="'+Number(r.id)+'" title="Duplicate"><i class="bi bi-copy"></i></button><button class="wf-icon-btn danger" data-action="archive" data-id="'+Number(r.id)+'" title="Archive"><i class="bi bi-archive"></i></button></td></tr>'});body.innerHTML=html;var s=d.summary||{};document.getElementById('statTotal').textContent=Number(s.total||0);document.getElementById('statServices').textContent=Number(s.services||0);document.getElementById('statSteps').textContent=Number(s.steps||0);document.getElementById('statFields').textContent=Number(s.fields||0);var sel=document.getElementById('serviceFilter'),cur=state.service,oh='<option value="">All Services</option>';(d.services||[]).forEach(function(x){oh+='<option value="'+Number(x.id)+'">'+esc(x.name)+'</option>'});sel.innerHTML=oh;sel.value=cur;var p=d.pagination||{};document.getElementById('countText').textContent='Showing '+Number(p.from||0)+'-'+Number(p.to||0)+' of '+Number(p.total||0)+' workflows';document.getElementById('prevBtn').disabled=state.page<=1;document.getElementById('nextBtn').disabled=state.page>=Number(p.pages||1);if(d.builder_schema_ready===false){note('warning',d.builder_schema_message||'Workflow Builder database update is missing. Run migration_workflow_builder.sql.')}}).catch(function(e){note('error',e.message)})}body.onclick=function(e){var b=e.target.closest('[data-action]');if(!b)return;var fd=new FormData();fd.append('workflow_id',b.dataset.id);fd.append('action',b.dataset.action==='duplicate'?'duplicate':'archive');req(fd).then(function(d){note('success',d.message);load()}).catch(function(er){note('error',er.message)})};document.getElementById('refreshBtn').onclick=load;document.getElementById('clearBtn').onclick=function(){state={page:1,search:'',status:'',service:''};document.getElementById('searchInput').value='';document.getElementById('statusFilter').value='';document.getElementById('serviceFilter').value='';load()};document.getElementById('searchInput').oninput=function(e){if(searchTimer)clearTimeout(searchTimer);searchTimer=setTimeout(function(){state.search=e.target.value.trim();state.page=1;load()},250)};document.getElementById('statusFilter').onchange=function(e){state.status=e.target.value;state.page=1;load()};document.getElementById('serviceFilter').onchange=function(e){state.service=e.target.value;state.page=1;load()};document.getElementById('prevBtn').onclick=function(){if(state.page>1){state.page--;load()}};document.getElementById('nextBtn').onclick=function(){state.page++;load()};document.getElementById('toastClose').onclick=function(){toast.classList.remove('show')};load()})();</script></div></main></div>
