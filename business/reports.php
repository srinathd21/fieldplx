<?php
/* FieldPlx Business Reports - Version 1.0.0 - 2026-08-28 */
...
function frTableExists(PDO $pdo, $table)
{
    static $cache = array();
    if (isset($cache[$table])) return $cache[$table];

    $q = $pdo->prepare("SELECT COUNT(*) FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = :name");
    $q->execute(array(':name' => $table));
    $cache[$table] = ((int)$q->fetchColumn() > 0);
    return $cache[$table];
}

function frColumnExists(PDO $pdo, $table, $column)
{
    static $cache = array();
    $key = $table . '.' . $column;
    if (isset($cache[$key])) return $cache[$key];

    $q = $pdo->prepare("SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = :table_name AND COLUMN_NAME = :column_name");
    $q->execute(array(':table_name' => $table, ':column_name' => $column));
    $cache[$key] = ((int)$q->fetchColumn() > 0);
    return $cache[$key];
}

function frScalar(PDO $pdo, $sql, array $params, $default = 0)
{
    try {
        $q = $pdo->prepare($sql);
        $q->execute($params);
        ...
        error_log('FieldPlx reports scalar failed: ' . $e->getMessage());
        return $default;
    }
}

function frRows(PDO $pdo, $sql, array $params)
{
    try {
        $q = $pdo->prepare($sql);
        $q->execute($params);
        ...
        error_log('FieldPlx reports rows failed: ' . $e->getMessage());
        return array();
    }
}
...
function frMoney($value, $symbol, $position, $decimals)
{
...
$from = frDate(isset($_GET['from']) ? $_GET['from'] : '') ?: $defaultFrom;
$to = frDate(isset($_GET['to']) ? $_GET['to'] : '') ?: $today;
if ($from > $to) {
    $tmp = $from;
    $from = $to;
    $to = $tmp;
...
$branches = array();
if (frTableExists($pdo, 'branches')) {
    $branches = frRows(
        $pdo,
        "SELECT id, name, branch_code, is_head_office FROM branches WHERE tenant_id = :tenant_id AND status = 'active' ORDER BY is_head_office DESC, name ASC",
...
try {
    if (frTableExists($pdo, 'tenants') && frTableExists($pdo, 'currencies')) {
        $currencySql = "SELECT c.symbol, c.symbol_position, c.decimal_places, c.currency_code
                        FROM tenants t
                        LEFT JOIN currencies c ON c.id = t.currency_id
                        WHERE t.id = :tenant_id
                        LIMIT 1";
        $currencyRow = frRows($pdo, $currencySql, array(':tenant_id' => $tenantId));
        if (!empty($currencyRow[0])) $currency = array_merge($currency, $currencyRow[0]);
    }
} catch (Throwable $e) {
    error_log('FieldPlx reports currency failed: ' . $e->getMessage());
}
...
function frBranchClause($alias, $branchId, &$params)
{
    if ($branchId > 0) {
        $params[':branch_id'] = $branchId;
        return " AND {$alias}.branch_id = :branch_id ";
    }
    return '';
}
...
if (frTableExists($pdo, 'payments')) {
    $p = array(':tenant_id' => $tenantId, ':from_date' => $from . ' 00:00:00', ':to_date' => $to . ' 23:59:59');
    $branchSql = frBranchClause('p', $branchId, $p);
    $summary['revenue'] = (float)frScalar(
        $pdo,
        "SELECT COALESCE(SUM(p.amount),0) FROM payments p WHERE p.tenant_id=:tenant_id AND p.status='succeeded' AND COALESCE(p.received_at,p.created_at) BETWEEN :from_date AND :to_date {$branchSql}",
        $p,
        0
    );
}

if (frTableExists($pdo, 'invoices')) {
    $p = array(':tenant_id' => $tenantId, ':from_date' => $from, ':to_date' => $to);
    $branchSql = frBranchClause('i', $branchId, $p);
    $summary['outstanding'] = (float)frScalar(
        $pdo,
        "SELECT COALESCE(SUM(i.balance_due),0) FROM invoices i WHERE i.tenant_id=:tenant_id AND i.status NOT IN ('paid','cancelled','archived','written_off') AND COALESCE(i.issue_date,DATE(i.created_at)) BETWEEN :from_date AND :to_date {$branchSql}",
        $p,
        0
    );
}

if (frTableExists($pdo, 'jobs')) {
    $p = array(':tenant_id' => $tenantId, ':from_date' => $from . ' 00:00:00', ':to_date' => $to . ' 23:59:59');
    $branchSql = frBranchClause('j', $branchId, $p);
    $summary['jobs'] = (int)frScalar($pdo, "SELECT COUNT(*) FROM jobs j WHERE j.tenant_id=:tenant_id AND j.deleted_at IS NULL AND j.created_at BETWEEN :from_date AND :to_date {$branchSql}", $p, 0);
    $summary['completed_jobs'] = (int)frScalar($pdo, "SELECT COUNT(*) FROM jobs j WHERE j.tenant_id=:tenant_id AND j.deleted_at IS NULL AND j.status='completed' AND j.created_at BETWEEN :from_date AND :to_date {$branchSql}", $p, 0);
}

if (frTableExists($pdo, 'quotes')) {
    $p = array(':tenant_id' => $tenantId, ':from_date' => $from . ' 00:00:00', ':to_date' => $to . ' 23:59:59');
    $branchSql = frBranchClause('q', $branchId, $p);
    $summary['quotes'] = (int)frScalar($pdo, "SELECT COUNT(*) FROM quotes q WHERE q.tenant_id=:tenant_id AND q.created_at BETWEEN :from_date AND :to_date {$branchSql}", $p, 0);
    $summary['approved_quotes'] = (int)frScalar($pdo, "SELECT COUNT(*) FROM quotes q WHERE q.tenant_id=:tenant_id AND q.status IN ('approved','converted') AND q.created_at BETWEEN :from_date AND :to_date {$branchSql}", $p, 0);
}

if (frTableExists($pdo, 'clients')) {
    $p = array(':tenant_id' => $tenantId, ':from_date' => $from . ' 00:00:00', ':to_date' => $to . ' 23:59:59');
    $branchSql = frBranchClause('c', $branchId, $p);
    $summary['clients'] = (int)frScalar($pdo, "SELECT COUNT(*) FROM clients c WHERE c.tenant_id=:tenant_id AND c.deleted_at IS NULL AND c.created_at BETWEEN :from_date AND :to_date {$branchSql}", $p, 0);
}

if (frTableExists($pdo, 'service_requests')) {
    $p = array(':tenant_id' => $tenantId, ':from_date' => $from . ' 00:00:00', ':to_date' => $to . ' 23:59:59');
    $branchSql = frBranchClause('sr', $branchId, $p);
    $summary['requests'] = (int)frScalar($pdo, "SELECT COUNT(*) FROM service_requests sr WHERE sr.tenant_id=:tenant_id AND sr.created_at BETWEEN :from_date AND :to_date {$branchSql}", $p, 0);
}

$quoteConversion = $summary['quotes'] > 0 ? round(($summary['approved_quotes'] / $summary['quotes']) * 100, 1) : 0;
$jobCompletion = $summary['jobs'] > 0 ? round(($summary['completed_jobs'] / $summary['jobs']) * 100, 1) : 0;

$jobStatus = array();
if (frTableExists($pdo, 'jobs')) {
    $p = array(':tenant_id' => $tenantId, ':from_date' => $from . ' 00:00:00', ':to_date' => $to . ' 23:59:59');
    $branchSql = frBranchClause('j', $branchId, $p);
    $jobStatus = frRows($pdo, "SELECT j.status, COUNT(*) AS total FROM jobs j WHERE j.tenant_id=:tenant_id AND j.deleted_at IS NULL AND j.created_at BETWEEN :from_date AND :to_date {$branchSql} GROUP BY j.status ORDER BY total DESC", $p);
}
...
if (frTableExists($pdo, 'quotes')) {
    $p = array(':tenant_id' => $tenantId, ':from_date' => $from . ' 00:00:00', ':to_date' => $to . ' 23:59:59');
    $branchSql = frBranchClause('q', $branchId, $p);
    $quoteStatus = frRows($pdo, "SELECT q.status, COUNT(*) AS total, COALESCE(SUM(q.total),0) AS amount FROM quotes q WHERE q.tenant_id=:tenant_id AND q.created_at BETWEEN :from_date AND :to_date {$branchSql} GROUP BY q.status ORDER BY total DESC", $p);
}
...
if (frTableExists($pdo, 'invoices')) {
    $p = array(':tenant_id' => $tenantId, ':from_date' => $from, ':to_date' => $to);
    $branchSql = frBranchClause('i', $branchId, $p);
    $invoiceStatus = frRows($pdo, "SELECT i.status, COUNT(*) AS total, COALESCE(SUM(i.total),0) AS amount, COALESCE(SUM(i.balance_due),0) AS balance FROM invoices i WHERE i.tenant_id=:tenant_id AND COALESCE(i.issue_date,DATE(i.created_at)) BETWEEN :from_date AND :to_date {$branchSql} GROUP BY i.status ORDER BY total DESC", $p);
}
...
if (frTableExists($pdo, 'payments')) {
    $p = array(':tenant_id' => $tenantId, ':from_date' => $from . ' 00:00:00', ':to_date' => $to . ' 23:59:59');
    $branchSql = frBranchClause('p', $branchId, $p);
    $recentPayments = frRows(
        $pdo,
        "SELECT p.payment_no,p.amount,p.payment_method,p.payment_channel,p.status,p.received_at,p.created_at,c.display_name AS client_name,i.invoice_no
         FROM payments p
         ...
         WHERE p.tenant_id=:tenant_id AND COALESCE(p.received_at,p.created_at) BETWEEN :from_date AND :to_date {$branchSql}
         ORDER BY COALESCE(p.received_at,p.created_at) DESC, p.id DESC LIMIT 8",
        $p
    );
}

$topClients = array();
if (frTableExists($pdo, 'invoices') && frTableExists($pdo, 'clients')) {
    $p = array(':tenant_id' => $tenantId, ':from_date' => $from, ':to_date' => $to);
    $branchSql = frBranchClause('i', $branchId, $p);
    $topClients = frRows(
        $pdo,
        "SELECT c.id,c.display_name,c.phone,COUNT(i.id) AS invoice_count,COALESCE(SUM(i.total),0) AS invoiced,COALESCE(SUM(i.amount_paid),0) AS collected,COALESCE(SUM(i.balance_due),0) AS outstanding
         FROM invoices i
         INNER JOIN clients c ON c.id=i.client_id AND c.tenant_id=i.tenant_id
         WHERE i.tenant_id=:tenant_id AND COALESCE(i.issue_date,DATE(i.created_at)) BETWEEN :from_date AND :to_date {$branchSql}
         GROUP BY c.id,c.display_name,c.phone
         ORDER BY invoiced DESC,c.display_name ASC LIMIT 8",
        $p
    );
}

$branchPerformance = array();
if ($branchId <= 0 && frTableExists($pdo, 'branches')) {
    $branchPerformance = frRows(
        $pdo,
        "SELECT b.id,b.name,b.branch_code,
            (SELECT COUNT(*) FROM jobs j WHERE j.tenant_id=b.tenant_id AND j.branch_id=b.id AND j.deleted_at IS NULL AND j.created_at BETWEEN :jf AND :jt) AS jobs,
            (SELECT COUNT(*) FROM quotes q WHERE q.tenant_id=b.tenant_id AND q.branch_id=b.id AND q.created_at BETWEEN :qf AND :qt) AS quotes,
            (SELECT COALESCE(SUM(p.amount),0) FROM payments p WHERE p.tenant_id=b.tenant_id AND p.branch_id=b.id AND p.status='succeeded' AND COALESCE(p.received_at,p.created_at) BETWEEN :pf AND :pt) AS collected
         FROM branches b
         WHERE b.tenant_id=:tenant_id AND b.status='active'
         ORDER BY collected DESC,b.name ASC",
        array(
            ':jf' => $from . ' 00:00:00', ':jt' => $to . ' 23:59:59',
            ':qf' => $from . ' 00:00:00', ':qt' => $to . ' 23:59:59',
            ':pf' => $from . ' 00:00:00', ':pt' => $to . ' 23:59:59',
            ':tenant_id' => $tenantId
        )
    );
}
...
if (isset($_GET['export']) && $_GET['export'] === 'summary') {
    ...
    header('Content-Disposition: attachment; filename="fieldplx-report-summary-' . $from . '-to-' . $to . '.csv"');
    $out = fopen('php://output', 'w');
    fputcsv($out, array('FieldPlx Report Summary'));
    fputcsv($out, array('From', $from));
    fputcsv($out, array('To', $to));
    fputcsv($out, array('Branch', $branchId > 0 ? $branchId : 'All'));
    fputcsv($out, array());
    fputcsv($out, array('Metric', 'Value'));
    fputcsv($out, array('Collected Revenue', $summary['revenue']));
    fputcsv($out, array('Outstanding', $summary['outstanding']));
    fputcsv($out, array('Jobs', $summary['jobs']));
    fputcsv($out, array('Completed Jobs', $summary['completed_jobs']));
    fputcsv($out, array('Quotes', $summary['quotes']));
    fputcsv($out, array('Approved / Converted Quotes', $summary['approved_quotes']));
    fputcsv($out, array('New Customers', $summary['clients']));
    fputcsv($out, array('Service Requests', $summary['requests']));
    fclose($out);
    exit;
}
?>
        ...
        <div class="fieldplx-content-wrapper">
            <div class="fd-dashboard">
                <section class="fr-head">
                    <div>
                        <h1 class="fr-title">Reports</h1>
                        <p class="fr-sub">Business performance overview for customers, enquiries, quotations, jobs, invoices and collections. All values are tenant-scoped and follow the selected report period.</p>
                    </div>
                    <div class="fr-actions">
                        <a class="fr-btn" href="reports.php"><i class="bi bi-arrow-clockwise"></i> Reset</a>
                        <a class="fr-btn primary" href="?<?= frH(http_build_query(array('from'=>$from,'to'=>$to,'branch_id'=>$branchId,'export'=>'summary'))) ?>"><i class="bi bi-download"></i> Export Summary</a>
                    </div>
                </section>

                <section class="fr-filter-card">
                    <form class="fr-filter" method="get" action="reports.php">
                        <div class="fr-field"><label>From Date</label><input class="fr-input" type="date" name="from" value="<?= frH($from) ?>"></div>
                        <div class="fr-field"><label>To Date</label><input class="fr-input" type="date" name="to" value="<?= frH($to) ?>"></div>
                        <div class="fr-field">
                            <label>Branch</label>
                            <select class="fr-input" name="branch_id" <?= $sessionBranchId > 0 ? 'disabled' : '' ?>>
                                <option value="0">All Branches</option>
                                <?php foreach ($branches as $branch): ?>
                                    <option value="<?= (int)$branch['id'] ?>" <?= $branchId === (int)$branch['id'] ? 'selected' : '' ?>><?= frH($branch['name']) ?><?= !empty($branch['branch_code']) ? ' (' . frH($branch['branch_code']) . ')' : '' ?></option>
                                <?php endforeach; ?>
                            </select>
                            <?php if ($sessionBranchId > 0): ?><input type="hidden" name="branch_id" value="<?= (int)$sessionBranchId ?>"><?php endif; ?>
                        </div>
                        <span class="fr-filter-spacer"></span>
                        <button class="fr-btn primary" type="submit"><i class="bi bi-funnel"></i> Apply Filter</button>
                    </form>
                </section>

                <section class="row g-3 fr-summary">
                    <div class="col-xl-3 col-md-6"><article class="fr-stat"><div class="fr-stat-row"><span class="fr-stat-icon"><i class="bi bi-cash-stack"></i></span><div><span class="fr-stat-label">Collected Revenue</span><strong class="fr-stat-value"><?= frH(frMoney($summary['revenue'],$symbol,$symbolPosition,$decimalPlaces)) ?></strong><span class="fr-stat-note">Successful payments in selected period</span></div></div></article></div>
                    <div class="col-xl-3 col-md-6"><article class="fr-stat"><div class="fr-stat-row"><span class="fr-stat-icon"><i class="bi bi-receipt"></i></span><div><span class="fr-stat-label">Outstanding</span><strong class="fr-stat-value"><?= frH(frMoney($summary['outstanding'],$symbol,$symbolPosition,$decimalPlaces)) ?></strong><span class="fr-stat-note">Open invoice balance</span></div></div></article></div>
                    <div class="col-xl-3 col-md-6"><article class="fr-stat"><div class="fr-stat-row"><span class="fr-stat-icon"><i class="bi bi-briefcase"></i></span><div><span class="fr-stat-label">Jobs</span><strong class="fr-stat-value"><?= (int)$summary['jobs'] ?></strong><span class="fr-stat-note"><?= (int)$summary['completed_jobs'] ?> completed · <?= frH($jobCompletion) ?>%</span></div></div></article></div>
                    <div class="col-xl-3 col-md-6"><article class="fr-stat"><div class="fr-stat-row"><span class="fr-stat-icon"><i class="bi bi-file-earmark-text"></i></span><div><span class="fr-stat-label">Quotations</span><strong class="fr-stat-value"><?= (int)$summary['quotes'] ?></strong><span class="fr-stat-note"><?= (int)$summary['approved_quotes'] ?> approved / converted · <?= frH($quoteConversion) ?>%</span></div></div></article></div>
                    <div class="col-xl-3 col-md-6"><article class="fr-stat"><div class="fr-stat-row"><span class="fr-stat-icon"><i class="bi bi-people"></i></span><div><span class="fr-stat-label">New Customers</span><strong class="fr-stat-value"><?= (int)$summary['clients'] ?></strong><span class="fr-stat-note">Created in selected period</span></div></div></article></div>
                    <div class="col-xl-3 col-md-6"><article class="fr-stat"><div class="fr-stat-row"><span class="fr-stat-icon"><i class="bi bi-inbox"></i></span><div><span class="fr-stat-label">Service Requests</span><strong class="fr-stat-value"><?= (int)$summary['requests'] ?></strong><span class="fr-stat-note">Enquiries created in selected period</span></div></div></article></div>
                    <div class="col-xl-3 col-md-6"><article class="fr-stat"><div class="fr-stat-row"><span class="fr-stat-icon"><i class="bi bi-check2-circle"></i></span><div><span class="fr-stat-label">Job Completion</span><strong class="fr-stat-value"><?= frH($jobCompletion) ?>%</strong><span class="fr-stat-note">Completed jobs / total jobs</span></div></div></article></div>
                    <div class="col-xl-3 col-md-6"><article class="fr-stat"><div class="fr-stat-row"><span class="fr-stat-icon"><i class="bi bi-graph-up-arrow"></i></span><div><span class="fr-stat-label">Quote Conversion</span><strong class="fr-stat-value"><?= frH($quoteConversion) ?>%</strong><span class="fr-stat-note">Approved or converted quotations</span></div></div></article></div>
                </section>

                <section class="fr-grid">
                    <article class="fr-card"><div class="fr-card-head"><div><h2>Job Status</h2><p>Operational job distribution</p></div><i class="bi bi-briefcase" style="color:var(--fd-green-dark)"></i></div><div class="fr-card-body"><div class="fr-status-list">
                        <?php if (!$jobStatus): ?><div class="fr-empty">No job data for this period.</div><?php else: foreach ($jobStatus as $row): ?><div class="fr-status-row"><span class="fr-status-dot"></span><span class="fr-status-name"><?= frH(str_replace('_',' ',$row['status'])) ?></span><span class="fr-status-count"><?= (int)$row['total'] ?></span></div><?php endforeach; endif; ?>
                    </div></div></article>
                    <article class="fr-card"><div class="fr-card-head"><div><h2>Quotation Status</h2><p>Quotation count and value</p></div><i class="bi bi-file-earmark-text" style="color:var(--fd-green-dark)"></i></div><div class="fr-card-body"><div class="fr-status-list">
                        <?php if (!$quoteStatus): ?><div class="fr-empty">No quotation data for this period.</div><?php else: foreach ($quoteStatus as $row): ?><div class="fr-status-row"><span class="fr-status-dot"></span><span class="fr-status-name"><?= frH(str_replace('_',' ',$row['status'])) ?></span><span class="fr-status-count"><?= (int)$row['total'] ?></span><span class="fr-status-amount"><?= frH(frMoney($row['amount'],$symbol,$symbolPosition,$decimalPlaces)) ?></span></div><?php endforeach; endif; ?>
                    </div></div></article>
                </section>

                <section class="fr-card fr-section-gap">
                    <div class="fr-card-head"><div><h2>Invoice Summary</h2><p>Invoice value and pending balance by status</p></div><i class="bi bi-receipt-cutoff" style="color:var(--fd-green-dark)"></i></div>
                    <div class="fr-table-wrap"><table class="fr-table"><thead><tr><th>Status</th><th>Invoices</th><th>Invoice Value</th><th>Outstanding</th></tr></thead><tbody>
                    <?php if (!$invoiceStatus): ?><tr><td class="fr-empty" colspan="4">No invoice data for this period.</td></tr><?php else: foreach ($invoiceStatus as $row): ?><tr><td><span class="fr-badge <?= $row['status']==='paid'?'':'blue' ?>"><?= frH(str_replace('_',' ',$row['status'])) ?></span></td><td><?= (int)$row['total'] ?></td><td><?= frH(frMoney($row['amount'],$symbol,$symbolPosition,$decimalPlaces)) ?></td><td><?= frH(frMoney($row['balance'],$symbol,$symbolPosition,$decimalPlaces)) ?></td></tr><?php endforeach; endif; ?>
                    </tbody></table></div>
                </section>

                <section class="fr-grid fr-section-gap">
                    <article class="fr-card"><div class="fr-card-head"><div><h2>Recent Collections</h2><p>Latest payment transactions</p></div><a class="fr-btn" href="payments.php">View Payments</a></div><div class="fr-table-wrap"><table class="fr-table"><thead><tr><th>Payment</th><th>Customer</th><th>Invoice</th><th>Mode</th><th>Amount</th><th>Status</th></tr></thead><tbody>
                        <?php if (!$recentPayments): ?><tr><td class="fr-empty" colspan="6">No payment data for this period.</td></tr><?php else: foreach ($recentPayments as $row): ?><tr><td><span class="fr-name"><?= frH($row['payment_no']) ?></span><span class="fr-muted"><?= frH($row['received_at'] ?: $row['created_at']) ?></span></td><td><?= frH($row['client_name'] ?: '-') ?></td><td><?= frH($row['invoice_no'] ?: '-') ?></td><td><?= frH(ucwords(str_replace('_',' ',$row['payment_method']))) ?></td><td><?= frH(frMoney($row['amount'],$symbol,$symbolPosition,$decimalPlaces)) ?></td><td><span class="fr-badge <?= $row['status']==='succeeded'?'':'gray' ?>"><?= frH(str_replace('_',' ',$row['status'])) ?></span></td></tr><?php endforeach; endif; ?>
                    </tbody></table></div></article>
                    <article class="fr-card"><div class="fr-card-head"><div><h2>Top Customers</h2><p>Ranked by invoice value</p></div><a class="fr-btn" href="clients.php">View Customers</a></div><div class="fr-table-wrap"><table class="fr-table"><thead><tr><th>Customer</th><th>Invoices</th><th>Invoiced</th><th>Collected</th><th>Outstanding</th></tr></thead><tbody>
                        <?php if (!$topClients): ?><tr><td class="fr-empty" colspan="5">No customer invoice data for this period.</td></tr><?php else: foreach ($topClients as $row): ?><tr><td><span class="fr-name"><?= frH($row['display_name']) ?></span><span class="fr-muted"><?= frH($row['phone'] ?: '-') ?></span></td><td><?= (int)$row['invoice_count'] ?></td><td><?= frH(frMoney($row['invoiced'],$symbol,$symbolPosition,$decimalPlaces)) ?></td><td><?= frH(frMoney($row['collected'],$symbol,$symbolPosition,$decimalPlaces)) ?></td><td><?= frH(frMoney($row['outstanding'],$symbol,$symbolPosition,$decimalPlaces)) ?></td></tr><?php endforeach; endif; ?>
                    </tbody></table></div></article>
                </section>

                <?php if ($branchId <= 0 && !empty($branches)): ?>
                <section class="fr-card fr-section-gap">
                    <div class="fr-card-head"><div><h2>Branch Performance</h2><p>Jobs, quotations and successful collections by branch</p></div><i class="bi bi-building" style="color:var(--fd-green-dark)"></i></div>
                    <div class="fr-table-wrap"><table class="fr-table"><thead><tr><th>Branch</th><th>Code</th><th>Jobs</th><th>Quotations</th><th>Collected Revenue</th></tr></thead><tbody>
                        <?php if (!$branchPerformance): ?><tr><td class="fr-empty" colspan="5">No branch performance data for this period.</td></tr><?php else: foreach ($branchPerformance as $row): ?><tr><td><span class="fr-name"><?= frH($row['name']) ?></span></td><td><?= frH($row['branch_code'] ?: '-') ?></td><td><?= (int)$row['jobs'] ?></td><td><?= (int)$row['quotes'] ?></td><td><?= frH(frMoney($row['collected'],$symbol,$symbolPosition,$decimalPlaces)) ?></td></tr><?php endforeach; endif; ?>
                    </tbody></table></div>
                </section>
                <?php endif; ?>
            </div>
