<?php
/* FieldPlx Inventory API - Version 1.1.0 - 2026-08-29 */
...
try {
    ...
    if ($action==='adjust_meta') {
        ...
        $productsStmt=$pdo->prepare("SELECT id,name,sku,unit_name FROM products WHERE tenant_id=:t AND deleted_at IS NULL AND status='active' AND track_inventory=1 ORDER BY name");
    ...
    if ($action==='adjust_stock') {
        ...
        if(!in_array($adjustmentType,array('add','remove','set'),true)) invOut(422,false,'Select a valid adjustment type.');
        if($quantity<=0) invOut(422,false,'Enter a quantity greater than zero.');

        $productCheck=$pdo->prepare("SELECT id,name FROM products WHERE id=:p AND tenant_id=:t AND deleted_at IS NULL AND status='active' AND track_inventory=1 LIMIT 1");
        $productCheck->execute(array(':p'=>$productId,':t'=>$tenantId));
        $product=$productCheck->fetch(PDO::FETCH_ASSOC);
        if(!$product) invOut(404,false,'Tracked product not found.');
        ...
        try {
            ...
            elseif($adjustmentType==='remove') $newQty=$current-$quantity;
