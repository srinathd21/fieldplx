...
<body>
    ...
    <main class="fieldplx-main-content">
      <div class="fieldplx-content-wrapper">
        <div class="fd-dashboard">
          <section class="qa-head">
            <div>
              <h1 class="qa-title"><?= $viewOnly ? 'View Quotation' : ($quoteId > 0 ? 'Edit Quotation' : 'Add Quotation') ?></h1>
              <p class="qa-sub">Create a quotation directly for a customer or optionally link it to an enquiry /
                revisit.</p>
            </div>
            <div class="qa-actions"><a class="qa-btn qa-nav-link" href="quotes.php"><i class="bi bi-arrow-left"></i>
                Back to Quotations</a></div>
          </section>
          <form id="quoteForm" autocomplete="off">
            <input type="hidden" name="quote_id" value="<?= (int) $quoteId ?>"><input type="hidden" name="request_id"
              id="realRequestId" value=""><input type="hidden" name="assessment_reschedule_id"
              id="assessmentRescheduleId" value=""><input type="hidden" name="items_json" id="itemsJson" value="[]">
            <section class="fd-card qa-card">
              <div class="qa-section">
                <div class="qa-section-title">
                  <div><strong>1. Select Enquiry / Revisit</strong><small>Optional. Leave this blank to create a direct
                      quotation.</small></div><span class="qa-currency" id="currencyBadge">Currency</span>
                </div>
                <div class="qa-field"><label>Service Enquiry / Revisit</label><select name="request_source"
                    id="requestId" <?= $viewOnly ? 'disabled' : '' ?>>
                    <option value="">Direct Quotation - No Enquiry</option>
                  </select></div>
                <div class="qa-enquiry-info" id="enquiryInfo">
                  <div class="qa-info"><small>Customer</small><strong id="infoCustomer">-</strong></div>
                  <div class="qa-info"><small>Location</small><strong id="infoLocation">-</strong></div>
                  <div class="qa-info"><small>Requested Service</small><strong id="infoService">-</strong></div>
                  <div class="qa-info"><small>Enquiry Status</small><strong id="infoStatus">-</strong></div>
                  <div class="qa-info"><small>Quotation Source</small><strong id="infoSource">-</strong></div>
                  <div class="qa-info"><small>Visit Schedule</small><strong id="infoVisit">-</strong></div>
                  <div class="qa-info"><small>Revisit Remarks</small><strong id="infoRemarks">-</strong></div>
                </div>
                <div class="qa-direct-box" id="directBox">
                  <div class="qa-grid">
                    <div class="qa-field"><label>Customer *</label><select name="client_id" id="clientId"
                        <?= $viewOnly ? 'disabled' : '' ?>>
                        <option value="">Select Customer</option>
                      </select></div>
                    <div class="qa-field"><label>Customer Location</label><select name="location_id" id="locationId"
                        <?= $viewOnly ? 'disabled' : '' ?>>
              ...
              <div class="qa-builder show" id="builder">
                <div class="qa-section">
                  <div class="qa-section-title">
                    <div><strong>2. Quotation Details</strong><small>Add title, status, validity and customer-facing
                        introduction.</small></div>
                  </div>
                  <div class="qa-grid">
                    <div class="qa-field"><label>Quotation Title *</label><input type="text" name="title"
                        id="quoteTitle" maxlength="190" required <?= $viewOnly ? 'disabled' : '' ?>></div>
                    <div class="qa-field"><label>Status *</label><select name="status" id="quoteStatus" required
                        <?= $viewOnly ? 'disabled' : '' ?>>
                        <option value="sent" selected>Send for Approval</option>
                    ...
                    <div class="qa-field"><label>Valid Until</label><input type="date" name="valid_until"
                        id="validUntil" <?= $viewOnly ? 'disabled' : '' ?>></div>
                    <div class="qa-field full"><label>Introduction / Notes</label><textarea name="introduction"
                        id="introduction" <?= $viewOnly ? 'disabled' : '' ?>></textarea></div>
                  </div>
                </div>
                <div class="qa-section">
                  <div class="qa-section-title">
                    <div><strong>3. Services, Products & Charges</strong><small>Add service/catalog charges, existing
                        products, or create a new product without refreshing.</small></div>
                  </div><?php if (!$viewOnly): ?>
                    <div class="qa-tools"><select id="catalogItem">
                        <option value="">Search service / material / fee / discount</option>
                      </select><button type="button" class="qa-btn" id="addCatalog"><i class="bi bi-cart-plus"></i> Add
                        Selected</button><button type="button" class="qa-btn" id="addManual"><i class="bi bi-plus-lg"></i>
                        Manual / Other Charge</button></div>
                    <div class="qa-product-tools">
                      <div class="qa-product-field"><label>Product</label><input type="text" id="productSearch"
                          list="productOptions" placeholder="Search existing product or type new product name"><datalist
                          id="productOptions"></datalist></div>
                      <div class="qa-product-field"><label>Qty</label><input type="number" id="productQty" min="0.001"
                          step="0.001" value="1"></div><button type="button" class="qa-btn primary" id="addProduct"><i
                          class="bi bi-cart-plus"></i> Add to Cart</button><button type="button"
                        class="qa-btn qa-product-create" id="openProductModal"><i class="bi bi-plus-circle"></i> Create
                        New Product</button>
                    </div><?php endif; ?>
                </div>
                <div class="qa-table-wrap">
                  <table class="qa-table">
                    ...
                    <tbody id="itemRows">
                      <tr>
                        <td colspan="12" class="qa-empty">Add a service, product or charge.</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
                <div class="qa-total-wrap">
                  <div class="qa-total-box">
                    <div class="qa-total-row"><span>Subtotal</span><strong id="subTotal">0.00</strong></div>
                    <div class="qa-total-row"><span>Discount</span><strong id="discountTotal">0.00</strong></div>
                    <div class="qa-total-row"><span>Tax</span><strong id="taxTotal">0.00</strong></div>
                    <div class="qa-total-row grand"><span>Quotation Total</span><strong id="grandTotal">0.00</strong>
                    </div>
                  </div>
                </div>
                <div class="qa-footer"><a class="qa-btn qa-nav-link"
                    href="quotes.php">Cancel</a><?php if (!$viewOnly): ?><button class="qa-btn primary" type="submit"
                      id="saveButton"><span class="qa-loader"></span><i class="bi bi-check2-circle"></i>
                      <span><?= $quoteId > 0 ? 'Update Quotation' : 'Create Quotation' ?></span></button><?php endif; ?></div>
              </div>
            </section>
          </form>
        </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">Create New Product</h5><button type="button" class="btn-close"
            data-bs-dismiss="modal"></button>
        </div>
        <div class="modal-body qa-product-modal">
          <div class="qa-product-grid">
            <div><label>Product Name *</label><input type="text" id="newProductName" maxlength="190"></div>
            <div><label>Unit / Base Price *</label><input type="number" id="newProductBasePrice" min="0" step="0.01"
                value="0.00"></div>
            <div><label>Markup Type *</label><select id="newProductMarkupType">
                <option value="percentage">Percentage %</option>
                <option value="fixed">Fixed Amount</option>
              </select></div>
            <div><label>Markup Value</label><input type="number" id="newProductMarkupValue" min="0" step="0.01"
                value="0.00"></div>
        ...
        <div class="modal-footer"><button type="button" class="qa-btn" data-bs-dismiss="modal">Cancel</button><button
            type="button" class="qa-btn primary" id="createProductButton"><span class="qa-loader"></span><i
              class="bi bi-check-lg"></i> Create Product</button></div>
      </div>
    </div>
  </div>
  <div class="modal fade qa-modal" id="unsavedModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">Unsaved Quotation Changes</h5><button type="button" class="btn-close"
            data-bs-dismiss="modal"></button>
        </div>
        <div class="modal-body qa-unsaved-copy">You have unsaved quotation changes. Do you want to leave this page and
          discard them?</div>
        <div class="modal-footer"><button type="button" class="qa-btn" data-bs-dismiss="modal">Stay Here</button><button
            type="button" class="qa-btn primary" id="leavePageButton">Leave Page</button></div>
      </div>
    </div>
  </div>
