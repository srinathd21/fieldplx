...
<head>
    ...
    <title>My Job Details - FieldPlx</title>
...
'use strict';
var jobId=<?= (int)$jobId ?>,
    csrfToken=<?= json_encode($myJobsCsrfToken) ?>,
...
function startTravelling(){if(!navigator.geolocation){notify('Location permission is required to start travelling.',false);return}var btn=document.getElementById('startTravellingButton');if(btn)btn.disabled=true;navigator.geolocation.getCurrentPosition(function(pos){postTravel('start',geoPayload(pos)).then(function(d){setTravelUi('on_the_way');startWatcher();notify('Marked On the Way. Customer live tracking is active.',true)}).catch(function(e){notify(e.message,false)}).finally(function(){if(btn)btn.disabled=false})},function(err){if(btn)btn.disabled=false;notify(err&&err.message?err.message:'Please allow location access to start travelling.',false)},{enableHighAccuracy:true,maximumAge:0,timeout:20000})}
function markArrived(){var btn=document.getElementById('arrivedButton');if(btn)btn.disabled=true;postTravel('arrived',{}).then(function(){trackingActive=false;stopWatcher();setTravelUi('arrived');notify('Marked Arrived. Customer tracking has stopped.',true)}).catch(function(e){notify(e.message,false)}).finally(function(){if(btn)btn.disabled=false})}
...
fetch('api/my-jobs.php',{method:'POST',body:fd,credentials:'same-origin',headers:{'X-Requested-With':'XMLHttpRequest','Accept':'application/json'}}).then(parse).then(function(d){
    var r=d.job||{},a=d.assignment||{};currentJob=r;
    var workBtn=document.getElementById('updateWorkButton');
    if(workBtn&&Number(r.workflow_id||0)>0&&['cancelled','closed','archived'].indexOf(String(r.status||''))===-1){workBtn.href='update-workflow.php?id='+Number(r.id);workBtn.style.display='inline-flex'}
    var address=[r.address_line1,r.address_line2,r.city,r.state,r.postal_code].filter(Boolean).join(', ');
    var destination='';
    if(r.latitude!==null&&r.latitude!==''&&r.longitude!==null&&r.longitude!==''){destination=String(r.latitude)+','+String(r.longitude)}else if(address){destination=address}
    var mapsBtn=document.getElementById('openMapsButton');if(mapsBtn&&destination)mapsBtn.href='https://www.google.com/maps/dir/?api=1&destination='+encodeURIComponent(destination);
    var terminal=['cancelled','completed','ready_to_invoice','invoiced','closed','archived'].indexOf(String(r.status||''))!==-1;
    content.innerHTML='<section class="fd-card mjv-card"><div class="mjv-card-head"><h3>Job Information</h3></div><div class="mjv-body"><div class="mjv-job-top"><div><span class="mjv-number">'+esc(r.job_no)+'</span><h2 class="mjv-job-title">'+esc(r.title)+'</h2></div><div><span class="mjv-badge">'+esc(title(r.status))+'</span><span class="mjv-travel-state" id="travelStateBadge">Not Started</span></div></div><div class="mjv-desc">'+esc(r.description||'No job description added.')+'</div><div class="mjv-info"><div class="mjv-info-item"><small>Customer</small><strong>'+esc(r.client_name||'-')+'</strong></div><div class="mjv-info-item"><small>Service Location</small><strong>'+esc(r.location_name||'-')+(address?'<br>'+esc(address):'')+'</strong></div><div class="mjv-info-item"><small>Service</small><strong>'+esc(r.service_name||'-')+'</strong></div><div class="mjv-info-item"><small>Workflow</small><strong>'+esc(r.workflow_name||'Not assigned')+'</strong></div><div class="mjv-info-item"><small>Start Date</small><strong>'+fmt(r.start_date)+'</strong></div><div class="mjv-info-item"><small>End Date</small><strong>'+fmt(r.end_date)+'</strong></div><div class="mjv-info-item"><small>Priority</small><strong>'+esc(title(r.priority))+'</strong></div><div class="mjv-info-item"><small>Job Type</small><strong>'+esc(title(r.job_type))+'</strong></div></div><div class="mjv-section"><h4 class="mjv-section-title">Your Assignment</h4><div class="mjv-person"><span class="mjv-person-icon"><i class="bi bi-person-check"></i></span><div><strong>'+esc(a.label||'Assigned')+'</strong><small>'+esc(a.role||'Technician')+' · '+esc(title(a.assignment_status||'assigned'))+'</small></div></div></div></div></section><aside class="fd-card mjv-card"><div class="mjv-card-head"><h3>Linked Details</h3></div><div class="mjv-body mjv-side-list"><div class="mjv-side-row"><small>Request</small><strong>'+esc(r.request_no||'Direct Job')+'</strong></div><div class="mjv-side-row"><small>Quote</small><strong>'+esc(r.quote_no||'No Quote')+'</strong></div><div class="mjv-side-row"><small>Branch</small><strong>'+esc(r.branch_name||'-')+'</strong></div><div class="mjv-side-row"><small>Completion Rule</small><strong>'+esc(title(r.assignment_completion_mode))+'</strong></div><div class="mjv-side-row"><small>Invoicing</small><strong>'+esc(title(r.invoicing_preference))+'</strong></div></div></aside>';
    if(terminal){setTravelUi('stopped');var sb=document.getElementById('startTravellingButton'),mb=document.getElementById('openMapsButton'),ab=document.getElementById('arrivedButton');if(sb)sb.style.display='none';if(mb)mb.style.display='none';if(ab)ab.style.display='none';return}
    if(!destination){var sb2=document.getElementById('startTravellingButton');if(sb2)sb2.style.display='inline-flex'}
    return loadTravel();
