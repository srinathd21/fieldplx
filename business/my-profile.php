<?php
/* FieldPlx My Profile - Version 1.3.0 - 2026-08-28 */
...
if (empty($_SESSION['my_profile_csrf_token'])) {
    $_SESSION['my_profile_csrf_token'] = bin2hex(random_bytes(32));
}
$csrfToken = (string)$_SESSION['my_profile_csrf_token'];
...
function mpTableExists(PDO $pdo, $table)
{
    static $cache = array();
...
function mpColumnExists(PDO $pdo, $table, $column)
{
    static $cache = array();
    $key = $table . '.' . $column;
...
function mpSessionUserId()
{
    $keys = array('tenant_user_id', 'user_id', 'id', 'business_user_id');
...
function mpUser(PDO $pdo, $tenantId, $userId)
{
    $stmt = $pdo->prepare(
        "SELECT
            id,
...
function mpLookupName(PDO $pdo, $table, $tenantId, $id, $fallback)
{
    $id = (int)$id;

    if ($id <= 0 || !mpTableExists($pdo, $table)) {
        return $fallback;
    }

    if (!mpColumnExists($pdo, $table, 'tenant_id') || !mpColumnExists($pdo, $table, 'name')) {
        return $fallback;
    }

    $allowedTables = array('roles', 'departments', 'branches');
    if (!in_array($table, $allowedTables, true)) {
        return $fallback;
    }

    $sql = "SELECT name FROM `" . $table . "` WHERE id = :id AND tenant_id = :tenant_id LIMIT 1";
...
function mpTenantName(PDO $pdo, $tenantId)
{
    if (!mpTableExists($pdo, 'tenants')) {
        return 'FieldPlx';
    }

    $column = null;
    if (mpColumnExists($pdo, 'tenants', 'display_name')) {
        $column = 'display_name';
    } elseif (mpColumnExists($pdo, 'tenants', 'legal_name')) {
        $column = 'legal_name';
    }
    ...
    $stmt = $pdo->prepare("SELECT `" . $column . "` FROM tenants WHERE id = :id LIMIT 1");
...
function mpFlash($type, $message)
{
    $_SESSION['my_profile_flash'] = array(
        'type' => $type,
        'message' => $message
    );
}
...
$tenantId = mpSessionTenantId();
$userId = mpSessionUserId();
...
try {
    ...
    error_log('FieldPlx My Profile DB: ' . $e->getMessage());
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($csrf === '' || !hash_equals($csrfToken, $csrf)) {
        mpFlash('error', 'Your form session expired. Refresh and try again.');
        mpRedirect();
    }
    ...
    try {
        if ($action === 'save_profile') {
            $firstName = trim((string)(isset($_POST['first_name']) ? $_POST['first_name'] : ''));
            $lastName = trim((string)(isset($_POST['last_name']) ? $_POST['last_name'] : ''));
            $email = strtolower(trim((string)(isset($_POST['email']) ? $_POST['email'] : '')));
            $phone = trim((string)(isset($_POST['phone']) ? $_POST['phone'] : ''));
            $alternatePhone = trim((string)(isset($_POST['alternate_phone']) ? $_POST['alternate_phone'] : ''));
            $jobTitle = trim((string)(isset($_POST['job_title']) ? $_POST['job_title'] : ''));
            ...
            if (strlen($firstName) > 120 || strlen($lastName) > 120) {
                throw new RuntimeException('Name is too long.');
            }

            if (strlen($phone) > 50 || strlen($alternatePhone) > 50) {
                throw new RuntimeException('Phone number is too long.');
            }

            if (strlen($jobTitle) > 120) {
                throw new RuntimeException('Job title is too long.');
            }
            ...
            if ($check->fetchColumn()) {
                throw new RuntimeException('This email is already used by another user in this business.');
            }
            ...
            mpFlash('success', 'Profile updated successfully.');
            mpRedirect();
        }

        if ($action === 'change_password') {
            $currentPassword = (string)(isset($_POST['current_password']) ? $_POST['current_password'] : '');
            $newPassword = (string)(isset($_POST['new_password']) ? $_POST['new_password'] : '');
            $confirmPassword = (string)(isset($_POST['confirm_password']) ? $_POST['confirm_password'] : '');
            ...
            mpFlash('success', 'Password changed successfully.');
            mpRedirect();
        }
        ...
        if ($action === 'upload_avatar') {
            ...
            if ((int)$_FILES['avatar']['size'] > (3 * 1024 * 1024)) {
                throw new RuntimeException('Profile image must be 3 MB or smaller.');
            }

            if (!class_exists('finfo')) {
                throw new RuntimeException('Image validation is not available on this server.');
            }
            ...
            $oldProfile = mpUser($pdo, $tenantId, $userId);

            $relativeDirectory = 'uploads/profile/tenant-' . $tenantId;
            $absoluteDirectory = __DIR__ . '/' . $relativeDirectory;

            if (!is_dir($absoluteDirectory)) {
                if (!@mkdir($absoluteDirectory, 0755, true) && !is_dir($absoluteDirectory)) {
                    throw new RuntimeException('Unable to create the profile image folder.');
                }
            }
            ...
            $oldPath = trim((string)$oldProfile['avatar_path']);
            if ($oldPath !== '' && strpos($oldPath, 'uploads/profile/') === 0) {
                $oldAbsolutePath = __DIR__ . '/' . $oldPath;
                if (is_file($oldAbsolutePath) && $oldAbsolutePath !== $absolutePath) {
                    @unlink($oldAbsolutePath);
                }
            }

            mpFlash('success', 'Profile image updated successfully.');
            mpRedirect();
        }

        throw new RuntimeException('Unsupported profile action.');
    } catch (Throwable $e) {
        error_log('FieldPlx My Profile action: ' . $e->getMessage());
        mpFlash('error', $e->getMessage());
        mpRedirect();
    }
}

try {
    $profile = mpUser($pdo, $tenantId, $userId);
    $roleName = mpLookupName($pdo, 'roles', $tenantId, $profile['role_id'], '');
    $departmentName = mpLookupName($pdo, 'departments', $tenantId, $profile['department_id'], 'Not assigned');
    $branchName = mpLookupName($pdo, 'branches', $tenantId, $profile['branch_id'], 'All / Not assigned');
    $tenantName = mpTenantName($pdo, $tenantId);
} catch (Throwable $e) {
    error_log('FieldPlx My Profile load: ' . $e->getMessage());
    http_response_code(500);
    exit('Unable to load profile.');
}
...
$roleLabel = $roleName;
if ($roleLabel === '') {
    if ((int)$profile['is_tenant_admin'] === 1) {
        $roleLabel = 'Administrator';
    } elseif (trim((string)$profile['job_title']) !== '') {
        $roleLabel = trim((string)$profile['job_title']);
    } else {
        $roleLabel = 'User';
    }
}

$flash = isset($_SESSION['my_profile_flash']) ? $_SESSION['my_profile_flash'] : null;
unset($_SESSION['my_profile_flash']);
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>My Profile - FieldPlx</title>
   ...
   ========================================================== */
    ...
    .fq-table-wrap::-webkit-scrollbar-button {
      display: none !important;
      width: 0 !important;
      height: 0 !important
    }
...
/* ==========================================================
   My Profile page - FieldPlx canonical tenant template
   ========================================================== */
.mp-head {
  display: flex;
  align-items: flex-start;
...
.mp-avatar-large {
  ...
  border-radius: 50%;
  outline: 1px solid #dfe6ef;
  color: var(--fd-navy);
  background: linear-gradient(135deg, #fff, #e8f3d9);
  box-shadow: 0 8px 22px rgba(0, 17, 49, .09);
  font-size: 30px;
  font-weight: 700;
}
...
.mp-camera {
  ...
  color: #fff;
  background: linear-gradient(90deg, #7fc92d, #68aa1d);
  box-shadow: 0 5px 13px rgba(0, 17, 49, .16);
  cursor: pointer;
  font-size: 13px;
...
.mp-toast.success { background: #5d971b; }
.mp-toast.error { background: #e45b66; }
.mp-toast.warning { background: #96a52f; }
.mp-toast.info { background: #123d70; }
            ...
            <div class="fd-dashboard">
                <section class="mp-head">
                    <div>
                        ...
                        <p class="mp-subtitle">Manage your personal information, profile image and account password.</p>
                    </div>
                </section>

                <div class="mp-grid">
                    <aside class="mp-card mp-summary">
                        <div class="mp-avatar-wrap">
                            <div class="mp-avatar-large">
                                <?php if (!empty($profile['avatar_path'])): ?>
                                    <img src="<?= mpH($profile['avatar_path']) ?>" alt="<?= mpH($displayName) ?>">
                                <?php else: ?>
                                    <span><?= mpH($initials) ?></span>
                                <?php endif; ?>
                            </div>

                            <label class="mp-camera" for="avatarInput" title="Change profile image">
                                <i class="bi bi-camera"></i>
                            </label>

                            <form id="avatarForm" method="post" enctype="multipart/form-data" style="display:none;">
                                <input type="hidden" name="csrf_token" value="<?= mpH($csrfToken) ?>">
                                <input type="hidden" name="action" value="upload_avatar">
                                <input id="avatarInput" type="file" name="avatar" accept="image/jpeg,image/png,image/webp">
                            </form>
                        </div>

                        <div class="mp-name"><?= mpH($displayName) ?></div>
                        <div class="mp-role"><?= mpH($roleLabel) ?></div>
                        <span class="mp-status"><?= mpH($profile['status']) ?></span>

                        <div class="mp-meta">
                            <div class="mp-meta-row">
                                <span class="mp-meta-icon"><i class="bi bi-building"></i></span>
                                <span>
                                    <span class="mp-meta-label">Business</span>
                                    <span class="mp-meta-value"><?= mpH($tenantName) ?></span>
                                </span>
                            </div>

                            <div class="mp-meta-row">
                                <span class="mp-meta-icon"><i class="bi bi-envelope"></i></span>
                                <span>
                                    <span class="mp-meta-label">Email</span>
                                    <span class="mp-meta-value"><?= mpH($profile['email']) ?></span>
                                </span>
                            </div>

                            <div class="mp-meta-row">
                                <span class="mp-meta-icon"><i class="bi bi-person-badge"></i></span>
                                <span>
                                    <span class="mp-meta-label">Employee Code</span>
                                    <span class="mp-meta-value"><?= mpH($profile['employee_code'] ?: 'Not assigned') ?></span>
                                </span>
                            </div>

                            <div class="mp-meta-row">
                                <span class="mp-meta-icon"><i class="bi bi-geo-alt"></i></span>
                                <span>
                                    <span class="mp-meta-label">Branch</span>
                                    <span class="mp-meta-value"><?= mpH($branchName) ?></span>
                                </span>
                            </div>

                            <div class="mp-meta-row">
                                <span class="mp-meta-icon"><i class="bi bi-diagram-3"></i></span>
                                <span>
                                    <span class="mp-meta-label">Department</span>
                                    <span class="mp-meta-value"><?= mpH($departmentName) ?></span>
                                </span>
                            </div>

                            <div class="mp-meta-row">
                                <span class="mp-meta-icon"><i class="bi bi-clock-history"></i></span>
                                <span>
                                    <span class="mp-meta-label">Last Login</span>
                                    <span class="mp-meta-value"><?= mpH($profile['last_login_at'] ?: 'Not available') ?></span>
                                </span>
                            </div>
                        </div>
                    </aside>
                    ...
                    <section class="mp-card">
                        <form class="mp-section" method="post" autocomplete="off">
                            <input type="hidden" name="csrf_token" value="<?= mpH($csrfToken) ?>">
                            ...
                            <div class="mp-section-head">
                                <div>
                                    ...
                                    <p>Update your personal contact and display information.</p>
                                </div>
                                <span class="mp-section-icon"><i class="bi bi-person-lines-fill"></i></span>
                            </div>

                            <div class="mp-form-grid">
                                <div class="mp-field">
                                    <label>First Name *</label>
                                    <input type="text" name="first_name" maxlength="120" required value="<?= mpH($profile['first_name']) ?>">
                                </div>

                                <div class="mp-field">
                                    <label>Last Name</label>
                                    <input type="text" name="last_name" maxlength="120" value="<?= mpH($profile['last_name']) ?>">
                                </div>

                                <div class="mp-field">
                                    <label>Email *</label>
                                    <input type="email" name="email" maxlength="190" required value="<?= mpH($profile['email']) ?>">
                                </div>

                                <div class="mp-field">
                                    <label>Phone</label>
                                    <input type="text" name="phone" maxlength="50" value="<?= mpH($profile['phone']) ?>">
                                </div>

                                <div class="mp-field">
                                    <label>Alternate Phone</label>
                                    <input type="text" name="alternate_phone" maxlength="50" value="<?= mpH($profile['alternate_phone']) ?>">
                                </div>

                                <div class="mp-field">
                                    <label>Job Title</label>
                                    <input type="text" name="job_title" maxlength="120" value="<?= mpH($profile['job_title']) ?>">
                                </div>

                                <div class="mp-field">
                                    <label>Role</label>
                                    <input type="text" readonly value="<?= mpH($roleLabel) ?>">
                                </div>

                                <div class="mp-field">
                                    <label>Employee Code</label>
                                    <input type="text" readonly value="<?= mpH($profile['employee_code']) ?>">
                                </div>
                            </div>
                        ...
                        <form class="mp-section" method="post" autocomplete="off">
                            ...
                            <div class="mp-section-head">
                                <div>
                                    ...
                                    <p>Verify your current password before setting a new password.</p>
                                </div>
                                <span class="mp-section-icon"><i class="bi bi-shield-lock"></i></span>
                            </div>

                            <div class="mp-note">
                                <i class="bi bi-info-circle"></i>
                                <span>Use at least 8 characters for your new password. The current password is always required.</span>
                            </div>
...
<?php if ($flash): ?>
<div class="mp-toast <?= mpH($flash['type']) ?>" id="profileToast" role="alert">
    <i class="bi <?= $flash['type'] === 'success' ? 'bi-check-circle-fill' : 'bi-exclamation-circle-fill' ?>"></i>
    <span><?= mpH($flash['message']) ?></span>
    <button type="button" aria-label="Close" id="profileToastClose"><i class="bi bi-x-lg"></i></button>
</div>
<?php endif; ?>
...
(function () {
    ...
    var toastClose = document.getElementById('profileToastClose');
    var toastTimer = null;
