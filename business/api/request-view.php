<?php
/*
 * FieldPlx Request View API - Version 2.2.2 - 2026-09-15
 ...
 * - Archive / delete
 * - Existing checklist selector + new checklist builder
 * - Product / Service markup support
 * - Manual assessment booking confirmation email + resend state
 */
ob_start();
...
function rvLoadRequest(PDO $pdo, $tenant, $requestId)
{
    ...
    $clientName = rvColumn($pdo, 'clients', 'display_name') ? 'c.display_name' : (rvColumn($pdo, 'clients', 'company_name') ? 'c.company_name' : "CONCAT_WS(' ',c.first_name,c.last_name)");
    $clientCompany = rvColumn($pdo, 'clients', 'company_name') ? 'c.company_name' : 'NULL';
    $clientEmail = rvColumn($pdo, 'clients', 'email') ? 'c.email' : 'NULL';
    $clientPhone = rvColumn($pdo, 'clients', 'phone') ? 'c.phone' : 'NULL';
    $clientAltPhone = rvColumn($pdo, 'clients', 'alternate_phone') ? 'c.alternate_phone' : 'NULL';
    $clientType = rvColumn($pdo, 'clients', 'client_type') ? 'c.client_type' : 'NULL';

    $locationJoin = '';
    $locationName = $locationType = $locationAddress1 = $locationAddress2 = $locationCity = $locationState = $locationPostal = $locationContactName = $locationContactPhone = 'NULL';
    if (rvTable($pdo, 'client_locations') && rvColumn($pdo, 'service_requests', 'location_id')) {
        $locationJoin = " LEFT JOIN client_locations l ON l.id=r.location_id AND l.tenant_id=r.tenant_id ";
        $locationName = rvColumn($pdo, 'client_locations', 'name') ? 'l.name' : 'NULL';
        $locationType = rvColumn($pdo, 'client_locations', 'location_type') ? 'l.location_type' : 'NULL';
        $locationAddress1 = rvColumn($pdo, 'client_locations', 'address_line1') ? 'l.address_line1' : 'NULL';
        $locationAddress2 = rvColumn($pdo, 'client_locations', 'address_line2') ? 'l.address_line2' : 'NULL';
        $locationCity = rvColumn($pdo, 'client_locations', 'city') ? 'l.city' : 'NULL';
        $locationState = rvColumn($pdo, 'client_locations', 'state') ? 'l.state' : 'NULL';
        $locationPostal = rvColumn($pdo, 'client_locations', 'postal_code') ? 'l.postal_code' : 'NULL';
        $locationContactName = rvColumn($pdo, 'client_locations', 'contact_name') ? 'l.contact_name' : 'NULL';
        $locationContactPhone = rvColumn($pdo, 'client_locations', 'contact_phone') ? 'l.contact_phone' : 'NULL';
    }

    $branchJoin = '';
    $branchName = 'NULL';
    if (rvTable($pdo, 'branches') && rvColumn($pdo, 'service_requests', 'branch_id')) {
        $branchJoin = " LEFT JOIN branches b ON b.id=r.branch_id AND b.tenant_id=r.tenant_id ";
        $branchName = rvColumn($pdo, 'branches', 'name') ? 'b.name' : 'NULL';
    }

    $creatorJoin = '';
    $createdByName = 'NULL';
    $creatorColumn = null;
    if (rvColumn($pdo, 'service_requests', 'created_by_user_id')) $creatorColumn = 'created_by_user_id';
    elseif (rvColumn($pdo, 'service_requests', 'created_by')) $creatorColumn = 'created_by';
    if ($creatorColumn && rvTable($pdo, 'users')) {
        $creatorJoin = " LEFT JOIN users cu ON cu.id=r.`" . $creatorColumn . "` AND cu.tenant_id=r.tenant_id ";
        $createdByName = "CONCAT_WS(' ',cu.first_name,cu.last_name)";
    }
    ...
    $sql = "SELECT r.*,
                   ...
                   $clientAltPhone AS client_alternate_phone,
                   $clientType AS client_type,
                   $locationName AS location_name,
                   $locationType AS location_type,
                   $locationAddress1 AS location_address1,
                   $locationAddress2 AS location_address2,
                   $locationCity AS location_city,
                   $locationState AS location_state,
                   $locationPostal AS location_postal_code,
                   $locationContactName AS location_contact_name,
                   $locationContactPhone AS location_contact_phone,
...
function rvLoadCatalog(PDO $pdo, $tenant)
{
    ...
    if (rvTable($pdo, 'product_services')) {
        ...
        foreach ($s->fetchAll(PDO::FETCH_ASSOC) as $r) {
            $catalog[] = array(
                ...
                'markup_percent' => (isset($r['unit_cost']) && (float)$r['unit_cost'] > 0) ? round((((float)$r['unit_price'] - (float)$r['unit_cost']) / (float)$r['unit_cost']) * 100, 4) : 0,
    ...
    if (rvTable($pdo, 'products')) {
        ...
        $markupTypeExpr = rvColumn($pdo, 'products', 'markup_type') ? 'p.markup_type' : "'percentage'";
        $markupValueExpr = rvColumn($pdo, 'products', 'markup_value') ? 'p.markup_value' : 'NULL';
        $sql = "SELECT p.id,$skuExpr AS sku,p.name,$descExpr AS description,$costExpr AS unit_cost,$priceExpr AS unit_price,$markupTypeExpr AS markup_type,$markupValueExpr AS markup_value,$taxExpr AS tax_percent FROM products p" . $taxJoin . " WHERE p.tenant_id=:t AND p.status='active'";
        ...
        foreach ($s->fetchAll(PDO::FETCH_ASSOC) as $r) {
            $catalog[] = array(
                ...
                'unit_cost' => isset($r['unit_cost']) ? (float)$r['unit_cost'] : 0,
                'unit_price' => isset($r['unit_price']) ? (float)$r['unit_price'] : 0,
                'markup_percent' => (isset($r['markup_type']) && $r['markup_type'] === 'percentage' && isset($r['markup_value']) && $r['markup_value'] !== null) ? (float)$r['markup_value'] : ((isset($r['unit_cost']) && (float)$r['unit_cost'] > 0) ? round((((float)$r['unit_price'] - (float)$r['unit_cost']) / (float)$r['unit_cost']) * 100, 4) : 0),
                'tax_percent' => isset($r['tax_percent']) ? (float)$r['tax_percent'] : 0
            );
        }
    }
...
function rvResolveCatalog(PDO $pdo, $tenant, $key)
{
    ...
    if ($kind === 's') {
        ...
        $r['markup_percent'] = !empty($r['unit_cost']) ? round((((float)$r['unit_price'] - (float)$r['unit_cost']) / (float)$r['unit_cost']) * 100, 4) : 0;
        return $r;
    }
    ...
    if ($kind === 'p' && rvTable($pdo, 'products')) {
        ...
        $markupTypeExpr = rvColumn($pdo, 'products', 'markup_type') ? 'p.markup_type' : "'percentage'";
        $markupValueExpr = rvColumn($pdo, 'products', 'markup_value') ? 'p.markup_value' : 'NULL';
        $sql = "SELECT p.id,p.name,'product' AS item_type,$descExpr AS description,$costExpr AS unit_cost,$priceExpr AS unit_price,$markupTypeExpr AS markup_type,$markupValueExpr AS markup_value,$taxExpr AS tax_percent FROM products p" . $taxJoin . " WHERE p.id=:id AND p.tenant_id=:t AND p.status='active'";
        if (rvColumn($pdo, 'products', 'deleted_at')) $sql .= " AND p.deleted_at IS NULL";
        $sql .= " LIMIT 1";
        $s = $pdo->prepare($sql);
        $s->execute(array(':id' => $id, ':t' => $tenant));
        ...
        $r['markup_percent'] = (isset($r['markup_type']) && $r['markup_type'] === 'percentage' && isset($r['markup_value']) && $r['markup_value'] !== null) ? (float)$r['markup_value'] : (!empty($r['unit_cost']) ? round((((float)$r['unit_price'] - (float)$r['unit_cost']) / (float)$r['unit_cost']) * 100, 4) : 0);
        return $r;
    }
    return null;
}
...
function rvLoadLines(PDO $pdo, $tenant, $requestId)
{
    ...
    $order = rvColumn($pdo, 'service_request_line_items', 'sort_order') ? 'li.sort_order,li.id' : 'li.id';
    $s = $pdo->prepare("SELECT li.*" . $selectProduct . " FROM service_request_line_items li WHERE li.tenant_id=:t AND li.request_id=:r ORDER BY " . $order);
    ...
    foreach ($rows as &$row) {
        ...
        $row['markup_percent'] = !empty($row['unit_cost']) ? round((((float)$row['unit_price'] - (float)$row['unit_cost']) / (float)$row['unit_cost']) * 100, 4) : 0;
    }
    unset($row);
    return $rows;
}
...
{
    if (!rvTable($pdo, 'users')) return array();
    $where = "tenant_id=:t";
    if (rvColumn($pdo, 'users', 'status')) $where .= " AND status='active'";
    if (rvColumn($pdo, 'users', 'deleted_at')) $where .= " AND deleted_at IS NULL";
    $job = rvColumn($pdo, 'users', 'job_title') ? 'job_title' : 'NULL AS job_title';
    $email = rvColumn($pdo, 'users', 'email') ? 'email' : 'NULL AS email';
    $s = $pdo->prepare("SELECT id,CONCAT(first_name,CASE WHEN last_name IS NOT NULL AND last_name<>'' THEN CONCAT(' ',last_name) ELSE '' END) name,$email,$job FROM users WHERE $where ORDER BY first_name,last_name");
    $s->execute(array(':t' => $tenant));
...
function rvLoadChecklistTemplates(PDO $pdo, $tenant)
{
    if (!rvTable($pdo, 'checklist_templates')) return array();
    try {
        $hasStatus = rvColumn($pdo, 'checklist_templates', 'status');
        $countExpr = rvTable($pdo, 'checklist_template_items') && rvColumn($pdo, 'checklist_template_items', 'checklist_template_id')
            ? "(SELECT COUNT(*) FROM checklist_template_items ci WHERE ci.checklist_template_id=ct.id)"
            : "0";
        $sql = "SELECT ct.id,ct.name,ct.description," . $countExpr . " AS item_count FROM checklist_templates ct WHERE ct.tenant_id=:t";
        if ($hasStatus) $sql .= " AND ct.status='active'";
        $sql .= " ORDER BY ct.name,ct.id";
        $q = $pdo->prepare($sql);
        $q->execute(array(':t'=>$tenant));
        return $q->fetchAll(PDO::FETCH_ASSOC);
    } catch (Throwable $e) {
        error_log('request-view checklist templates: ' . $e->getMessage());
        return array();
    }
}

function rvSyncChecklistLinks(PDO $pdo, $tenant, $requestId, $user, $rawIds)
{
    if (!rvTable($pdo,'service_request_checklists') || !rvTable($pdo,'checklist_templates')) return;
    $rawIds = trim((string)$rawIds);
    if ($rawIds === '') return; // backward compatibility: omitted field means leave existing links unchanged.
    $ids = json_decode($rawIds, true);
    if (!is_array($ids)) rvApiResponse(422, false, 'Selected checklist data is invalid.');
    $clean = array();
    foreach ($ids as $id) {
        $id=(int)$id;
        if ($id<=0) continue;
        $q=$pdo->prepare("SELECT id FROM checklist_templates WHERE id=:id AND tenant_id=:t AND status='active' LIMIT 1");
        $q->execute(array(':id'=>$id,':t'=>$tenant));
        if ($q->fetchColumn()) $clean[$id]=$id;
    }
    $clean=array_values($clean);
    if ($clean) {
        $ph=array();$pa=array(':t'=>$tenant,':r'=>$requestId);
        foreach($clean as $i=>$id){$k=':c'.$i;$ph[]=$k;$pa[$k]=$id;}
        $d=$pdo->prepare("DELETE FROM service_request_checklists WHERE tenant_id=:t AND request_id=:r AND checklist_template_id NOT IN(".implode(',',$ph).")");
        $d->execute($pa);
    } else {
        $d=$pdo->prepare("DELETE FROM service_request_checklists WHERE tenant_id=:t AND request_id=:r");
        $d->execute(array(':t'=>$tenant,':r'=>$requestId));
    }
    ...
    $ins=$pdo->prepare("INSERT INTO service_request_checklists(tenant_id,request_id,checklist_template_id,created_by) VALUES(:t,:r,:ct,:u)");
    foreach($clean as $id){
        $exists->execute(array(':t'=>$tenant,':r'=>$requestId,':ct'=>$id));
        if(!$exists->fetchColumn())$ins->execute(array(':t'=>$tenant,':r'=>$requestId,':ct'=>$id,':u'=>$user));
    }
}
...
function rvBookingEmailEventId(PDO $pdo)
{
    if (!rvTable($pdo,'notification_events')) return 0;
    try {
        $sql = "SELECT id FROM notification_events WHERE event_key='assessment.booking_confirmation'";
        if (rvColumn($pdo,'notification_events','is_active')) $sql .= " AND is_active=1";
        $sql .= " ORDER BY id LIMIT 1";
        $q=$pdo->prepare($sql);
        $q->execute();
        return (int)$q->fetchColumn();
    } catch (Throwable $e) {
        error_log('request-view booking event: '.$e->getMessage());
        return 0;
    }
}

function rvBookingEmailFeatureEnabled(PDO $pdo, $tenant)
{
    if (!rvTable($pdo,'email_feature_settings')) return true;
    if (!rvColumn($pdo,'email_feature_settings','setting_key') || !rvColumn($pdo,'email_feature_settings','is_enabled')) return true;
    foreach(array('assessment_booking_confirmation','assessment.booking_confirmation','booking_confirmation') as $key){
        try {
            $q=$pdo->prepare("SELECT is_enabled FROM email_feature_settings WHERE tenant_id=:t AND setting_key=:k LIMIT 1");
            $q->execute(array(':t'=>$tenant,':k'=>$key));
            $v=$q->fetchColumn();
            if($v!==false)return (int)$v===1;
        } catch (Throwable $e) {
            error_log('request-view booking feature setting: '.$e->getMessage());
            return true;
        }
    }
    return true;
...
function rvTenantMailProfile(PDO $pdo, $tenant)
{
    $out=array('name'=>'FieldPlx','email'=>'','phone'=>'');
    try{
        $q=$pdo->prepare("SELECT COALESCE(NULLIF(display_name,''),NULLIF(legal_name,''),'FieldPlx') name,email,phone FROM tenants WHERE id=:t LIMIT 1");
        $q->execute(array(':t'=>$tenant));$r=$q->fetch(PDO::FETCH_ASSOC);
        if($r){$out['name']=(string)$r['name'];$out['email']=(string)$r['email'];$out['phone']=(string)$r['phone'];}
    }catch(Throwable $e){}
    return $out;
}

function rvBookingEmailTemplate(PDO $pdo, $tenant, $branch, $eventId)
{
    if($eventId<=0 || !rvTable($pdo,'notification_templates')) return null;
    try {
        $hasTenant=rvColumn($pdo,'notification_templates','tenant_id');
        $hasBranch=rvColumn($pdo,'notification_templates','branch_id');
        $hasChannel=rvColumn($pdo,'notification_templates','channel');
        $hasActive=rvColumn($pdo,'notification_templates','is_active');
        $hasDefault=rvColumn($pdo,'notification_templates','is_default');
        $sql="SELECT nt.* FROM notification_templates nt WHERE nt.event_id=:e";
        $params=array(':e'=>$eventId);
        if($hasChannel)$sql.=" AND nt.channel='email'";
        if($hasActive)$sql.=" AND nt.is_active=1";
        if($hasTenant){$sql.=" AND (nt.tenant_id=:t OR nt.tenant_id IS NULL)";$params[':t']=$tenant;}
        if($hasBranch && $branch>0){$sql.=" AND (nt.branch_id=:b OR nt.branch_id IS NULL)";$params[':b']=$branch;}
        elseif($hasBranch){$sql.=" AND nt.branch_id IS NULL";}
        $order=array();
        if($hasTenant)$order[]="CASE WHEN nt.tenant_id=".(int)$tenant." THEN 0 ELSE 1 END";
        if($hasBranch && $branch>0)$order[]="CASE WHEN nt.branch_id=".(int)$branch." THEN 0 ELSE 1 END";
        if($hasDefault)$order[]="nt.is_default DESC";
        $order[]="nt.id DESC";
        $sql.=" ORDER BY ".implode(',',$order)." LIMIT 1";
        $q=$pdo->prepare($sql);
        $q->execute($params);
        $r=$q->fetch(PDO::FETCH_ASSOC);
        return $r?:null;
    } catch (Throwable $e) {
        error_log('request-view booking template: '.$e->getMessage());
        return null;
    }
}

function rvBookingEmailTokens($request, $assessment, $company)
{
    $start=!empty($assessment['scheduled_start'])?strtotime($assessment['scheduled_start']):false;
    $end=!empty($assessment['scheduled_end'])?strtotime($assessment['scheduled_end']):false;
    $address=trim(implode(', ',array_filter(array(
        isset($request['location_address1'])?$request['location_address1']:'',
        isset($request['location_address2'])?$request['location_address2']:'',
        isset($request['location_city'])?$request['location_city']:'',
        isset($request['location_state'])?$request['location_state']:'',
        isset($request['location_postal_code'])?$request['location_postal_code']:''
    ),function($v){return trim((string)$v)!=='';})));
    return array(
        '{{CLIENT_NAME}}'=>(string)(isset($request['client_name'])?$request['client_name']:'Customer'),
        '{{CLIENT_COMPANY_NAME}}'=>(string)(isset($request['client_company'])?$request['client_company']:''),
        '{{CLIENT_EMAIL}}'=>(string)(isset($request['client_email'])?$request['client_email']:''),
        '{{COMPANY_NAME}}'=>(string)$company['name'],
        '{{DEFAULT_EMAIL}}'=>(string)$company['email'],
        '{{DEFAULT_PHONE}}'=>(string)$company['phone'],
        '{{REQUEST_NUMBER}}'=>(string)(isset($request['request_no'])?$request['request_no']:''),
        '{{REQUEST_TITLE}}'=>(string)(isset($request['title'])?$request['title']:''),
        '{{REQUEST_DESCRIPTION}}'=>(string)(isset($request['description'])?$request['description']:''),
        '{{ASSESSMENT_NUMBER}}'=>(string)(isset($assessment['assessment_no'])?$assessment['assessment_no']:''),
        '{{ASSESSMENT_DATE}}'=>$start?date('d M Y',$start):'',
        '{{ASSESSMENT_START}}'=>$start?date('d M Y, h:i A',$start):'Schedule later',
        '{{ASSESSMENT_END}}'=>$end?date('d M Y, h:i A',$end):'',
...
function rvBookingEmailSentState(PDO $pdo, $tenant, $requestId, $assessmentId, $eventId)
{
    $out=array('sent'=>0,'send_count'=>0,'sent_at'=>null,'last_recipient'=>null);
    if($eventId>0 && rvTable($pdo,'notification_queue')){
        try{
            $q=$pdo->prepare("SELECT COUNT(*) FROM notification_queue WHERE tenant_id=:t AND event_id=:e AND channel='email' AND related_type='assessment' AND related_id=:a AND status='sent'");
            $q->execute(array(':t'=>$tenant,':e'=>$eventId,':a'=>$assessmentId));$out['send_count']=(int)$q->fetchColumn();
            $q=$pdo->prepare("SELECT sent_at,recipient_address FROM notification_queue WHERE tenant_id=:t AND event_id=:e AND channel='email' AND related_type='assessment' AND related_id=:a AND status='sent' ORDER BY id DESC LIMIT 1");
            $q->execute(array(':t'=>$tenant,':e'=>$eventId,':a'=>$assessmentId));$r=$q->fetch(PDO::FETCH_ASSOC);
            if($r){$out['sent']=1;$out['sent_at']=$r['sent_at'];$out['last_recipient']=$r['recipient_address'];return $out;}
        }catch(Throwable $e){error_log('request-view booking email state: '.$e->getMessage());}
    }
    if(rvTable($pdo,'activity_events')){
        try{
            $q=$pdo->prepare("SELECT created_at,details_json FROM activity_events WHERE tenant_id=:t AND related_type='service_request' AND related_id=:r AND event_type='assessment_booking_confirmation_sent' ORDER BY id DESC");
            $q->execute(array(':t'=>$tenant,':r'=>$requestId));$rows=$q->fetchAll(PDO::FETCH_ASSOC);
            if($rows){$out['sent']=1;$out['send_count']=max($out['send_count'],count($rows));$out['sent_at']=$rows[0]['created_at'];$d=json_decode((string)$rows[0]['details_json'],true);if(is_array($d)&&!empty($d['recipient']))$out['last_recipient']=$d['recipient'];}
        }catch(Throwable $e){error_log('request-view booking email activity state: '.$e->getMessage());}
    }
    return $out;
}

function rvBookingEmailMeta(PDO $pdo, $tenant, $requestId, $request, $assessment)
{
    $fallback=array(
        'available'=>0,
        'enabled'=>1,
        ...
        'sent_at'=>null,
        'last_recipient'=>null,
        'to'=>(string)(isset($request['client_email'])?$request['client_email']:''),
        'subject'=>'',
        'message'=>'',
        'event_id'=>0,
        'template_id'=>null,
        'template_name'=>'Default booking confirmation',
        'reason'=>''
    );
    try {
        $eventId=rvBookingEmailEventId($pdo);
        $enabled=rvBookingEmailFeatureEnabled($pdo,$tenant);
        $fallback['event_id']=$eventId;
        $fallback['enabled']=$enabled?1:0;
        if(!$assessment){$fallback['reason']='An on-site assessment has not been created yet.';return $fallback;}
        $company=rvTenantMailProfile($pdo,$tenant);
        $template=rvBookingEmailTemplate($pdo,$tenant,(int)(isset($request['branch_id'])?$request['branch_id']:0),$eventId);
        $subject=$template&&!empty($template['subject'])?(string)$template['subject']:'Booking confirmation from {{COMPANY_NAME}} - {{ASSESSMENT_DATE}}';
        $message=$template&&!empty($template['body'])?(string)$template['body']:("Hi {{CLIENT_NAME}},

Your on-site assessment has been booked.

Request: {{REQUEST_NUMBER}} - {{REQUEST_TITLE}}
Assessment: {{ASSESSMENT_NUMBER}}
Date & time: {{ASSESSMENT_START}}" . (!empty($assessment['scheduled_end']) ? " - {{ASSESSMENT_END}}" : "") . "
Location: {{PROPERTY_ADDRESS}}

If you need to make changes, please contact us at {{DEFAULT_EMAIL}} {{DEFAULT_PHONE}}.

Thanks,
{{COMPANY_NAME}}");
        $tokens=rvBookingEmailTokens($request,$assessment,$company);
        $subject=strtr($subject,$tokens);$message=strtr($message,$tokens);
        $status=rvBookingEmailSentState($pdo,$tenant,$requestId,(int)$assessment['id'],$eventId);
        return array_merge($fallback,array(
            'available'=>!empty($assessment['scheduled_start'])?1:0,
            'enabled'=>$enabled?1:0,
            'to'=>(string)(isset($request['client_email'])?$request['client_email']:''),
            'subject'=>$subject,
            'message'=>$message,
            'event_id'=>$eventId,
            'template_id'=>$template&&isset($template['id'])?(int)$template['id']:null,
            'template_name'=>$template&&isset($template['template_name'])?(string)$template['template_name']:'Default booking confirmation',
            'reason'=>!empty($assessment['scheduled_start'])?'':'Schedule the assessment before sending the booking confirmation.'
        ),$status);
    } catch (Throwable $e) {
        error_log('request-view booking meta: '.$e->getMessage());
        $fallback['reason']='Booking email settings could not be loaded, but the request page is still available.';
        return $fallback;
    }
}

function rvBookingQueueStart(PDO $pdo,$tenant,$branch,$eventId,$request,$assessment,$to,$subject,$body,$smtpId)
{
    if($eventId<=0 || !rvTable($pdo,'notification_queue'))return 0;
    try{
        $q=$pdo->prepare("INSERT INTO notification_queue(tenant_id,branch_id,event_id,channel,recipient_type,recipient_id,recipient_address,related_type,related_id,subject,body,smtp_config_id,status,attempts,scheduled_at,sent_at,error_message,created_at) VALUES(:t,:b,:e,'email','client',:c,:addr,'assessment',:a,:sub,:body,:smtp,'processing',1,NOW(),NULL,NULL,NOW())");
        $q->execute(array(':t'=>$tenant,':b'=>$branch>0?$branch:null,':e'=>$eventId,':c'=>(int)$request['client_id'],':addr'=>$to,':a'=>(int)$assessment['id'],':sub'=>substr($subject,0,255),':body'=>$body,':smtp'=>$smtpId>0?$smtpId:null));
        return (int)$pdo->lastInsertId();
    }catch(Throwable $e){error_log('request-view booking queue start: '.$e->getMessage());return 0;}
}

function rvBookingQueueFinish(PDO $pdo,$id,$status,$error='')
{
    if($id<=0 || !rvTable($pdo,'notification_queue'))return;
    if(!in_array($status,array('sent','failed','suppressed'),true))$status='failed';
    try{$q=$pdo->prepare("UPDATE notification_queue SET status=:s,sent_at=CASE WHEN :sent='sent' THEN NOW() ELSE NULL END,error_message=:e WHERE id=:id LIMIT 1");$q->execute(array(':s'=>$status,':sent'=>$status,':e'=>$error!==''?$error:null,':id'=>$id));}catch(Throwable $e){error_log('request-view booking queue finish: '.$e->getMessage());}
}

function rvBookingHtml($message,$companyName)
{
    return '<div style="font-family:Arial,Helvetica,sans-serif;max-width:680px;margin:0 auto;color:#183548;line-height:1.6"><div style="padding:22px 24px;background:#0b3142;color:#fff;border-radius:10px 10px 0 0"><div style="font-size:12px;opacity:.82">'.htmlspecialchars($companyName,ENT_QUOTES,'UTF-8').'</div><div style="margin-top:4px;font-size:21px;font-weight:700">Booking confirmation</div></div><div style="padding:24px;border:1px solid #dfe6ea;border-top:0;border-radius:0 0 10px 10px;background:#fff">'.nl2br(htmlspecialchars($message,ENT_QUOTES,'UTF-8')).'</div></div>';
}
...
try {
    if ($action === 'load') {
        ...
        if (!$request) rvApiResponse(404, false, 'Service request not found.');
        ...
        $data = array(
            ...
            'related' => array('quotes'=>array(),'jobs'=>array()),
            'booking_email' => null,
            'supports_product_id' => 0,
            'load_warnings' => array()
        );

        try { $data['assessment'] = rvLoadAssessment($pdo, $tenant, $requestId); }
        catch (Throwable $e) { $data['load_warnings'][] = 'assessment'; error_log('request-view load assessment: '.$e->getMessage()); }

        try { $data['checklists'] = rvLoadChecklists($pdo, $tenant, $requestId); }
        catch (Throwable $e) { $data['load_warnings'][] = 'checklists'; error_log('request-view load checklists: '.$e->getMessage()); }

        try { $data['checklist_templates'] = rvLoadChecklistTemplates($pdo, $tenant); }
        catch (Throwable $e) { $data['load_warnings'][] = 'checklist_templates'; error_log('request-view load checklist templates: '.$e->getMessage()); }

        try { $data['line_items'] = rvLoadLines($pdo, $tenant, $requestId); }
        catch (Throwable $e) { $data['load_warnings'][] = 'line_items'; error_log('request-view load lines: '.$e->getMessage()); }

        try { $data['catalog'] = rvLoadCatalog($pdo, $tenant); }
        catch (Throwable $e) { $data['load_warnings'][] = 'catalog'; error_log('request-view load catalog: '.$e->getMessage()); }

        try { $data['users'] = rvLoadUsers($pdo, $tenant); }
        catch (Throwable $e) { $data['load_warnings'][] = 'users'; error_log('request-view load users: '.$e->getMessage()); }

        try { $data['notes'] = rvLoadNotes($pdo, $tenant, $requestId); }
        catch (Throwable $e) { $data['load_warnings'][] = 'notes'; error_log('request-view load notes: '.$e->getMessage()); }

        try { $data['history'] = rvLoadHistory($pdo, $tenant, $requestId); }
        catch (Throwable $e) { $data['load_warnings'][] = 'history'; error_log('request-view load history: '.$e->getMessage()); }

        try { $data['attachments'] = rvLoadAttachments($pdo, $tenant, $requestId); }
        catch (Throwable $e) { $data['load_warnings'][] = 'attachments'; error_log('request-view load attachments: '.$e->getMessage()); }

        try { $data['related'] = rvLoadRelated($pdo, $tenant, $requestId); }
        catch (Throwable $e) { $data['load_warnings'][] = 'related'; error_log('request-view load related: '.$e->getMessage()); }

        try { $data['booking_email'] = rvBookingEmailMeta($pdo, $tenant, $requestId, $request, $data['assessment']); }
        catch (Throwable $e) { $data['load_warnings'][] = 'booking_email'; error_log('request-view load booking email: '.$e->getMessage()); }

        try { $data['supports_product_id'] = rvTable($pdo, 'service_request_line_items') && rvColumn($pdo, 'service_request_line_items', 'product_id') ? 1 : 0; }
        catch (Throwable $e) { $data['supports_product_id'] = 0; }
    ...
    if ($action === 'send_booking_confirmation') {
        ...
        if(!$assessment)rvApiResponse(422,false,'Create the on-site assessment before sending a booking confirmation.');
        if(empty($assessment['scheduled_start']))rvApiResponse(422,false,'Schedule the on-site assessment before sending a booking confirmation.');
        if(!rvBookingEmailFeatureEnabled($pdo,$tenant))rvApiResponse(422,false,'Assessment booking confirmation email is disabled in Email Settings.');
        $to=trim((string)rvPost('to',''));if($to==='')$to=trim((string)$request['client_email']);
        $subject=trim((string)rvPost('subject',''));
        $message=trim((string)rvPost('message',''));
        if(!filter_var($to,FILTER_VALIDATE_EMAIL))rvApiResponse(422,false,'Enter a valid customer email address.');
        if($subject==='')rvApiResponse(422,false,'Email subject is required.');
        if($message==='')rvApiResponse(422,false,'Email message is required.');
        if(strlen($subject)>255)rvApiResponse(422,false,'Email subject must be 255 characters or less.');
        if(strlen($message)>50000)rvApiResponse(422,false,'Email message is too long.');
        if(!function_exists('fieldplxSendPlatformMail'))rvApiResponse(500,false,'Platform SMTP mail helper is unavailable.');
        $eventId=rvBookingEmailEventId($pdo);
        if($eventId<=0)rvApiResponse(500,false,'Assessment Booking Confirmation email event is not configured.');
        $before=rvBookingEmailSentState($pdo,$tenant,$requestId,(int)$assessment['id'],$eventId);
        $smtpId=0;$smtpName=null;
        if(function_exists('fieldplxPlatformSmtpConfig')){
            try{$cfg=fieldplxPlatformSmtpConfig($pdo);if($cfg){$smtpId=isset($cfg['id'])?(int)$cfg['id']:0;$smtpName=isset($cfg['config_name'])?$cfg['config_name']:null;}}catch(Throwable $e){}
        }
        $queueId=rvBookingQueueStart($pdo,$tenant,(int)$request['branch_id'],$eventId,$request,$assessment,$to,$subject,$message,$smtpId);
        $company=rvTenantMailProfile($pdo,$tenant);
        try{
            $sentMeta=fieldplxSendPlatformMail($pdo,$to,$subject,rvBookingHtml($message,$company['name']));
            if(is_array($sentMeta)&&!empty($sentMeta['smtp_id']))$smtpId=(int)$sentMeta['smtp_id'];
            if(is_array($sentMeta)&&!empty($sentMeta['config_name']))$smtpName=$sentMeta['config_name'];
            rvBookingQueueFinish($pdo,$queueId,'sent','');
        }catch(Throwable $e){
            rvBookingQueueFinish($pdo,$queueId,'failed',$e->getMessage());
            rvActivity($pdo,$tenant,(int)$request['branch_id'],$user,'assessment_booking_confirmation_failed',$requestId,(int)$request['client_id'],'Assessment booking confirmation email failed',array('assessment_id'=>(int)$assessment['id'],'recipient'=>$to,'subject'=>$subject,'error'=>$e->getMessage(),'queue_id'=>$queueId));
            rvApiResponse(500,false,'Booking confirmation email failed: '.$e->getMessage());
        }
        $details=array('assessment_id'=>(int)$assessment['id'],'recipient'=>$to,'subject'=>$subject,'smtp_config_id'=>$smtpId,'smtp_config_name'=>$smtpName,'queue_id'=>$queueId,'resend'=>!empty($before['sent'])?1:0);
        rvActivity($pdo,$tenant,(int)$request['branch_id'],$user,'assessment_booking_confirmation_sent',$requestId,(int)$request['client_id'],!empty($before['sent'])?'Assessment booking confirmation resent':'Assessment booking confirmation sent',$details);
        rvAudit($pdo,!empty($before['sent'])?'ASSESSMENT_BOOKING_CONFIRMATION_RESENT':'ASSESSMENT_BOOKING_CONFIRMATION_SENT',$tenant,(int)$request['branch_id'],$user,$requestId,null,$details);
        rvApiResponse(200,true,!empty($before['sent'])?'Booking confirmation email resent.':'Booking confirmation email sent.',array('booking_email'=>rvBookingEmailMeta($pdo,$tenant,$requestId,$request,$assessment)));
    }
    ...
    if ($action === 'save_assessment') {
        ...
        try {
            ...
            rvSyncChecklistLinks($pdo, $tenant, $requestId, $user, (string)rvPost('checklist_template_ids', ''));
        ...
        rvApiResponse(200, true, $oldAssessment ? 'Assessment updated.' : 'Assessment scheduled.', array('assessment'=>$assessment,'team_email'=>$mail,'checklists'=>rvLoadChecklists($pdo,$tenant,$requestId),'checklist_templates'=>rvLoadChecklistTemplates($pdo,$tenant),'booking_email'=>rvBookingEmailMeta($pdo,$tenant,$requestId,$request,$assessment)));
    }
    ...
    if ($action === 'create_catalog_item') {
        ...
        if ($itemType === 'service') {
            ...
            $item = array('id'=>'s:'.$id,'source_id'=>$id,'item_source'=>'service','item_type'=>'service','name'=>$name,'description'=>$description,'unit_cost'=>$unitCost,'unit_price'=>$unitPrice,'markup_percent'=>$unitCost>0?round((($unitPrice-$unitCost)/$unitCost)*100,4):0,'tax_percent'=>$taxPercent);
        ...
        if ($existing) {
            $id = (int)$existing['id'];
            $item = array('id'=>'p:'.$id,'source_id'=>$id,'item_source'=>'product','item_type'=>'product','name'=>$existing['name'],'description'=>(string)$existing['description'],'unit_cost'=>(float)$existing['base_unit_price'],'unit_price'=>(float)$existing['selling_price'],'markup_percent'=>(float)$existing['base_unit_price']>0?round((((float)$existing['selling_price']-(float)$existing['base_unit_price'])/(float)$existing['base_unit_price'])*100,4):0,'tax_percent'=>(float)$existing['tax_percent']);
        ...
        $s->execute($params);
        $id = (int)$pdo->lastInsertId();
        $item = array('id'=>'p:'.$id,'source_id'=>$id,'item_source'=>'product','item_type'=>'product','name'=>$name,'description'=>$description,'unit_cost'=>$unitCost,'unit_price'=>$unitPrice,'markup_percent'=>$markup,'tax_percent'=>$taxPercent);
