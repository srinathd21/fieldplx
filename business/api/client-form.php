...
/* Keep AJAX failures readable even when PHP stops before the normal handler. */
register_shutdown_function(function () {
    ...
    echo json_encode(array(
        'success' => false,
        'message' => 'Client form API could not complete the request. Check the PHP error log for the server-side detail.',
        'error_code' => 'CLIENT_FORM_FATAL'
...
function cfCountries(PDO $pdo)
{
    ...
    $isoSelect = cfColumn($pdo, 'countries', 'iso2') ? 'iso2' : "'' AS iso2";
    $where = cfColumn($pdo, 'countries', 'is_active') ? ' WHERE is_active=1' : '';
    return $pdo->query("SELECT id,name,$isoSelect FROM countries$where ORDER BY name")->fetchAll(PDO::FETCH_ASSOC);
}
...
function cfTaxRates(PDO $pdo, $tenantId)
{
    ...
    $jurisdiction = cfColumn($pdo, 'product_tax_rates', 'jurisdiction_name') ? 'jurisdiction_name' : 'NULL AS jurisdiction_name';
    $statusSelect = cfColumn($pdo, 'product_tax_rates', 'status') ? 'status' : "'active' AS status";
    $where = ' WHERE tenant_id=:t';
    if (cfColumn($pdo, 'product_tax_rates', 'status')) {
        $where .= " AND status='active'";
    }
    $sql = "SELECT id,tax_name,rate_percent,$jurisdiction,$statusSelect," . ($hasDefault ? 'is_default' : '0 AS is_default') . "\n"
         . "FROM product_tax_rates" . $where . "\n"
...
function cfLeadSources(PDO $pdo, $tenantId)
{
    ...
    if (cfTable($pdo, 'client_lead_sources')) {
        $sortSelect = cfColumn($pdo, 'client_lead_sources', 'sort_order') ? 'sort_order' : '100 AS sort_order';
        $where = 'tenant_id=:t';
        if (cfColumn($pdo, 'client_lead_sources', 'status')) {
            $where .= " AND status='active'";
        }
        $order = cfColumn($pdo, 'client_lead_sources', 'sort_order') ? 'sort_order,source_name,id' : 'source_name,id';
        $stmt = $pdo->prepare("SELECT id,source_name,$sortSelect FROM client_lead_sources WHERE $where ORDER BY $order");
        $stmt->execute(array(':t' => $tenantId));
...
function cfCustomFields(PDO $pdo, $tenantId, $appliesTo)
{
    ...
    /* The form can still open while a migration is pending. Missing optional columns get safe defaults. */
    $select = array(
        'id',
        cfColumn($pdo, 'client_custom_field_definitions', 'applies_to') ? 'applies_to' : "'client' AS applies_to",
        'field_name',
        cfColumn($pdo, 'client_custom_field_definitions', 'field_type') ? 'field_type' : "'text' AS field_type",
        cfColumn($pdo, 'client_custom_field_definitions', 'is_transferable') ? 'is_transferable' : '0 AS is_transferable',
        cfColumn($pdo, 'client_custom_field_definitions', 'default_value') ? 'default_value' : 'NULL AS default_value',
        cfColumn($pdo, 'client_custom_field_definitions', 'options_json') ? 'options_json' : 'NULL AS options_json',
        cfColumn($pdo, 'client_custom_field_definitions', 'sort_order') ? 'sort_order' : '100 AS sort_order'
    );

    $where = array('tenant_id=:t');
    $params = array(':t' => $tenantId);
    if (cfColumn($pdo, 'client_custom_field_definitions', 'applies_to')) {
        $where[] = 'applies_to=:a';
        $params[':a'] = $appliesTo;
    }
    if (cfColumn($pdo, 'client_custom_field_definitions', 'status')) {
        $where[] = "status='active'";
    }

    $order = cfColumn($pdo, 'client_custom_field_definitions', 'sort_order') ? 'sort_order,field_name,id' : 'field_name,id';
    $stmt = $pdo->prepare('SELECT ' . implode(',', $select) . ' FROM client_custom_field_definitions WHERE ' . implode(' AND ', $where) . ' ORDER BY ' . $order);
    $stmt->execute($params);
...
function cfMeta(PDO $pdo, $tenantId)
{
    ...
    if (cfTable($pdo, 'branches')) {
        $branchCode = cfColumn($pdo, 'branches', 'branch_code') ? 'branch_code' : "'' AS branch_code";
        $where = 'tenant_id=:t';
        if (cfColumn($pdo, 'branches', 'status')) {
            $where .= " AND status='active'";
        }
        $order = cfColumn($pdo, 'branches', 'is_head_office') ? 'is_head_office DESC,name' : 'name';
        $stmt = $pdo->prepare("SELECT id,name,$branchCode FROM branches WHERE $where ORDER BY $order");
        $stmt->execute(array(':t' => $tenantId));
    ...
    if (cfTable($pdo, 'users')) {
        $lastNameExpr = cfColumn($pdo, 'users', 'last_name')
            ? "CASE WHEN last_name IS NOT NULL AND last_name<>'' THEN CONCAT(' ',last_name) ELSE '' END"
            : "''";
        $employeeCode = cfColumn($pdo, 'users', 'employee_code') ? 'employee_code' : "'' AS employee_code";
        $jobTitle = cfColumn($pdo, 'users', 'job_title') ? 'job_title' : "'' AS job_title";
        $where = 'tenant_id=:t';
        if (cfColumn($pdo, 'users', 'status')) {
            $where .= " AND status='active'";
        }
        if (cfColumn($pdo, 'users', 'deleted_at')) {
            $where .= ' AND deleted_at IS NULL';
        }
        $order = cfColumn($pdo, 'users', 'last_name') ? 'first_name,last_name' : 'first_name';
        $stmt = $pdo->prepare("SELECT id,CONCAT(first_name,$lastNameExpr) name,$employeeCode,$jobTitle FROM users WHERE $where ORDER BY $order");
        $stmt->execute(array(':t' => $tenantId));
...
function cfSendClientMail(PDO $pdo, $tenantId, $branchId, $client, $plainPassword, $isEdit, $portalEnabled, $locations)
{
    ...
    if (!function_exists('fieldplxPlatformSmtpConfig') || !function_exists('fieldplxDecryptSmtpPassword') || !function_exists('fieldplxSmtpSendWithConfig')) {
        $result['notice'] = 'Client saved. Welcome email was skipped because the Platform SMTP helper is not installed in this environment.';
        return $result;
    }
...
try {
    ...
    cfRes(500, false, 'Unable to process the client request. Check the PHP error log for the database detail.', array('error_code' => 'CLIENT_FORM_DB_ERROR'));
} catch (Throwable $e) {
