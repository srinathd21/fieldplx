      ...
      <div class="fieldplx-content-wrapper">
        <div class="fd-dashboard">
          <section class="fd-teams-header">
            <div>
              <h1 class="fd-teams-title">Clients</h1>
              <p class="fd-teams-subtitle">Manage CRM leads and clients, contact preferences, branch ownership, tax
          ...
          <section class="fd-card fd-teams-card">
            <div class="fd-teams-toolbar">
              <div class="fd-team-search"><i class="bi bi-search"></i><input type="search" id="clientsSearch"
                  placeholder="Search name, company, email or phone" autocomplete="off"></div><select
            ...
            <form id="clientForm">
              <div class="fd-team-modal-body"><input type="hidden" id="clientId" name="client_id" value="0">
                <div class="fd-team-form-grid">
                  ...
                  <div class="fd-team-field"><label>Display Name</label><input type="text" id="displayName"
                      name="display_name" maxlength="190" required></div>
                  <div class="fd-team-field"><label>Company Name</label><input type="text" id="companyName"
                      name="company_name" maxlength="190"></div>
                  <div class="fd-team-field"><label>First Name</label><input type="text" id="firstName"
                      name="first_name" maxlength="120"></div>
                  <div class="fd-team-field"><label>Last Name</label><input type="text" id="lastName" name="last_name"
                      maxlength="120"></div>
                  <div class="fd-team-section-title">Contact & Ownership</div>
                  <div class="fd-team-field"><label>Email</label><input type="email" id="email" name="email"
                      maxlength="190"></div>
                  <div class="fd-team-field"><label>Phone</label><input type="text" id="phone" name="phone"
                      maxlength="50"></div>
                  <div class="fd-team-field"><label>Alternate Phone</label><input type="text" id="alternatePhone"
                      name="alternate_phone" maxlength="50"></div>
                  <div class="fd-team-field"><label>Source</label><input type="text" id="source" name="source"
                      maxlength="120" placeholder="Website / Referral / Walk-in"></div>
                  <div class="fd-team-field"><label>Branch</label><select id="branchId" name="branch_id">
                      <option value="">No Branch</option>
                    </select></div>
                  <div class="fd-team-field"><label>Account Manager</label><select id="accountManagerId"
                      name="account_manager_id">
                      <option value="">No Account Manager</option>
                    </select></div>
                  <div class="fd-team-field"><label>Preferred Contact</label><select id="preferredContactMethod"
                  ...
                  <div class="fd-team-field"><label>Tax Number</label><input type="text" id="taxNumber"
                      name="tax_number" maxlength="100"></div>
                  <div class="fd-client-checks"><label class="fd-client-check"><input type="checkbox" id="allowEmail"
                        name="allow_email" value="1" checked> Allow Email</label><label class="fd-client-check"><input
                        type="checkbox" id="allowSms" name="allow_sms" value="1" checked> Allow SMS</label></div>
                  <div class="fd-team-section-title">Internal Notes</div>
                  <div class="fd-team-field full"><label>Notes</label><textarea id="notes" name="notes"
        ...
        <script>
          (function () {
            'use strict';
            var csrfToken = <?= json_encode($clientsCsrfToken) ?>; var state = { page: 1, perPage: 10, search: '', type: '', status: '', branchId: '', archiveId: 0, meta: { branches: [], users: [] } }; var tableBody = document.getElementById('clientsTableBody'), clientModal = document.getElementById('clientModalBackdrop'), archiveModal = document.getElementById('archiveModalBackdrop'), clientForm = document.getElementById('clientForm'), saveButton = document.getElementById('saveClientButton'), toast = document.getElementById('clientsToast'), toastMessage = document.getElementById('clientsToastMessage'), toastTimer = null, searchTimer = null;
            function esc(v) { return String(v == null ? '' : v).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&#039;') }
            function initials(v) { var p = String(v || '').trim().split(/\s+/), x = ''; if (p.length) x += p[0].charAt(0); if (p.length > 1) x += p[p.length - 1].charAt(0); return x.toUpperCase() || 'C' }
            function toastShow(t, m) { if (toastTimer) clearTimeout(toastTimer); toast.className = 'fd-team-toast ' + (t || 'info') + ' show'; toastMessage.textContent = m || 'Notification'; toastTimer = setTimeout(function () { toast.classList.remove('show') }, 3000) }
            function loading(b, on) { if (!b) return; b.disabled = !!on; b.classList.toggle('loading', !!on) }
            function parseResponse(r) { return r.text().then(function (raw) { var t = (raw || '').trim(), d; try { d = t ? JSON.parse(t) : {} } catch (e) { var c = t.replace(/<br\s*\/?>/gi, ' ').replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim(); throw new Error(c ? 'Server error: ' + c : 'Server returned an invalid response.') } if (!r.ok || !d.success) throw new Error(d.message || 'Request failed.'); return d }) }
