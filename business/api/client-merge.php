...
/* Always return JSON, even when PHP hits a fatal error during the merge. */
$GLOBALS['cmg_response_sent'] = false;
register_shutdown_function(function () {
    if (!empty($GLOBALS['cmg_response_sent'])) {
        return;
    }
    $error = error_get_last();
    if (!$error || !in_array((int)$error['type'], array(E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR, E_USER_ERROR), true)) {
        return;
    }
    ...
    error_log('FieldPlx client merge fatal error: ' . $error['message'] . ' in ' . $error['file'] . ':' . $error['line']);
    echo json_encode(array(
        'success' => false,
        'message' => 'Client merge failed because of a server-side PHP error. Check the PHP error log for the exact cause.',
        'error_type' => 'fatal'
...
{
    $GLOBALS['cmg_response_sent'] = true;
    while (ob_get_level() > 0) {
        @ob_end_clean();
    }
...
function cmgLocalDebug()
{
    $host = isset($_SERVER['HTTP_HOST']) ? strtolower((string)$_SERVER['HTTP_HOST']) : '';
    return $host === 'localhost' || strpos($host, 'localhost:') === 0 || $host === '127.0.0.1' || strpos($host, '127.0.0.1:') === 0;
}
...
function cmgUserContext(PDO $pdo, $tenantId, $userId)
{
    ...
    $hasRoles = cmgTableExists($pdo, 'roles');
    $hasUserRole = cmgColumnExists($pdo, 'users', 'role_id');
    $hasTenantAdmin = cmgColumnExists($pdo, 'users', 'is_tenant_admin');
    $hasBranch = cmgColumnExists($pdo, 'users', 'branch_id');
    $hasDeletedAt = cmgColumnExists($pdo, 'users', 'deleted_at');
    $hasRoleAdmin = $hasRoles && cmgColumnExists($pdo, 'roles', 'is_admin');

    $roleJoin = ($hasRoles && $hasUserRole)
        ? "LEFT JOIN roles r ON r.id = u.role_id AND r.tenant_id = u.tenant_id"
        : '';
    $roleAdmin = $hasRoleAdmin ? 'COALESCE(r.is_admin, 0)' : '0';
    $roleSelect = $hasUserRole ? 'u.role_id' : '0 AS role_id';
    $tenantAdminSelect = $hasTenantAdmin ? 'u.is_tenant_admin' : '0 AS is_tenant_admin';
    $branchSelect = $hasBranch ? 'u.branch_id' : '0 AS branch_id';
    $deletedWhere = $hasDeletedAt ? ' AND u.deleted_at IS NULL' : '';

    $q = $pdo->prepare("SELECT $roleSelect, $tenantAdminSelect, $branchSelect, $roleAdmin AS is_role_admin
                        FROM users u
                        $roleJoin
...
try {
    ...
    if ($action === 'merge') {
        ...
        $mergeStep = 'starting the merge transaction';
        $pdo->beginTransaction();
        try {
            $mergeStep = 'locking selected clients';
            ...
            $mergeStep = 'recording merge history';
            cmgRecordMergeHistory($pdo, $tenantId, $primaryId, $sourceRows, $userId, $summary);

            /* Commit the merge first. Audit logging must never interfere with the business transaction. */
            $mergeStep = 'committing the merge';
            $pdo->commit();
        } catch (Throwable $e) {
        ...
        /* Audit after commit so an audit-table issue cannot roll back or break a successful merge. */
    ...
    $message = 'Unable to process the client merge request.';
    if (cmgLocalDebug()) {
        $message .= ' ' . $e->getMessage();
    }
    cmgResponse(500, false, $message);
} catch (Throwable $e) {
    $stepText = isset($mergeStep) && $mergeStep !== '' ? ' Step: ' . $mergeStep . '.' : '';
    error_log('FieldPlx client merge error:' . $stepText . ' ' . $e->getMessage());
    $message = $e->getMessage();
    if ($message === '') {
        $message = 'Unable to process the client merge request.';
    }
    if (cmgLocalDebug() && $stepText !== '') {
        $message = rtrim($message) . $stepText;
    }
    cmgResponse(500, false, $message);
}
