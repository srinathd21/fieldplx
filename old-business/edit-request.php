...
<head>
    ...
    <title>Edit Service Request - FieldPlx</title>
...
/* ==========================================================
   Add Service Request - separate page
   ========================================================== */
.fd-ar-head{
  display:flex;
  align-items:flex-start;
...
.fd-ar-body{padding:16px}
.fd-ar-grid{
  display:grid;
  grid-template-columns:repeat(2,minmax(0,1fr));
  gap:13px;
}
.fd-ar-field.full{grid-column:1/-1}
.fd-ar-section{
  grid-column:1/-1;
  margin-top:4px;
  padding:9px 0 5px;
...
.fd-ar-field input:focus,
.fd-ar-field select:focus,
.fd-ar-field textarea:focus{
  border-color:#a9cf75;
  box-shadow:0 0 0 3px rgba(116,184,36,.11);
}
.fd-ar-assign-mode{
...
.fd-ar-mode label strong{
  display:block;
  color:#253750;
  font-size:9.5px;
}
.fd-ar-mode label small{
  display:block;
  margin-top:2px;
  color:#8a96a7;
  font-size:7.8px;
}
.fd-ar-mode input:checked + label{
  border-color:#9cca67;
  background:#f5faee;
  box-shadow:0 0 0 2px rgba(116,184,36,.08);
}
.fd-ar-mode input:checked + label i{
  color:#fff;
  background:var(--fd-green);
}
.fd-ar-assignment-panel{
  grid-column:1/-1;
  padding:12px;
...
.fd-ar-btn:disabled{opacity:.58;cursor:not-allowed}
.fd-ar-loader{
  width:13px;
  height:13px;
  display:none;
  border:2px dotted currentColor;
  border-radius:50%;
  animation:fdArSpin .75s linear infinite;
}
.fd-ar-btn.loading .fd-ar-loader{display:inline-block}
@keyframes fdArSpin{to{transform:rotate(360deg)}}
.fd-ar-toast{
...
.fd-ar-toast.show{opacity:1;transform:translateY(0)}
.fd-ar-toast.success{background:#5d971b}
.fd-ar-toast.error{background:#e45b66}
.fd-ar-toast.warning{background:#96a52f}
.fd-ar-toast.info{background:#123d70}
.fd-ar-toast-msg{min-width:0;flex:1;font-size:8.5px;font-weight:600}
.fd-ar-toast-close{width:19px;height:19px;padding:0;border:0;color:#fff;background:transparent;cursor:pointer}
...
.select2-dropdown{
  ...
  border-radius:8px!important;
  overflow:hidden!important;
  box-shadow:0 10px 25px rgba(0,17,49,.12)!important;
}
...
.fd-ar-select2-option.selected .fd-ar-select2-check{
  border-color:var(--fd-green);
  color:#fff;
  background:var(--fd-green);
}
@media(max-width:767.98px){
  .fd-ar-head{flex-direction:column}
  .fd-ar-grid{grid-template-columns:1fr}
  .fd-ar-field.full,.fd-ar-section,.fd-ar-assignment-panel,.fd-ar-notify{grid-column:auto}
  .fd-ar-assign-mode{grid-template-columns:1fr}
}
@media(max-width:575.98px){
  .fd-ar-footer{flex-direction:column-reverse}
  .fd-ar-footer .fd-ar-btn{width:100%}
  .fd-ar-toast{top:72px;left:12px;right:12px;width:auto}
}

/* Request stage automation correction */
.fd-rflow-note{grid-column:1/-1;padding:10px 12px;border:1px solid #dfe6ef;border-radius:9px;background:#fbfcfd;color:#64748b;font-size:8.5px;line-height:1.5}
.fd-rflow-note strong{color:#263750;font-size:9px}
.fd-rflow-assignment{grid-column:1/-1;padding:12px;border:1px solid #dfe6ef;border-radius:9px;background:#fbfcfd;display:none}
.fd-rflow-assignment.show{display:block}
.fd-rflow-assignment-modes{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin-bottom:11px}
.fd-rflow-mode{position:relative}.fd-rflow-mode input{position:absolute;opacity:0;pointer-events:none}
.fd-rflow-mode label{min-height:50px;padding:9px 10px;display:flex;align-items:center;gap:8px;border:1px solid #dfe5ec;border-radius:8px;background:#fff;color:#53647b;cursor:pointer}
.fd-rflow-mode label i{width:27px;height:27px;display:grid;place-items:center;border-radius:7px;background:#edf2f7;color:#123d70}
.fd-rflow-mode label strong{display:block;color:#263750;font-size:9px}.fd-rflow-mode label small{display:block;margin-top:2px;color:#8b96a7;font-size:7.5px}
.fd-rflow-mode input:checked+label{border-color:#9cca67;background:#f5faee}.fd-rflow-mode input:checked+label i{color:#fff;background:var(--fd-green)}
.fd-rflow-assign-panel{display:none}.fd-rflow-assign-panel.show{display:block}
.fd-rflow-quote{grid-column:1/-1;display:none;border:1px solid #dde6ef;border-radius:10px;background:#fff;overflow:hidden}
.fd-rflow-quote.show{display:block}.fd-rflow-quote-head{padding:12px 13px;display:flex;align-items:center;justify-content:space-between;gap:10px;border-bottom:1px solid #e8edf2;background:#fbfcfd}
.fd-rflow-quote-head h3{margin:0;color:#263750;font-size:11px}.fd-rflow-quote-head small{display:block;margin-top:3px;color:#8a96a7;font-size:8px}.fd-rflow-currency{padding:5px 8px;border-radius:6px;color:#365069;background:#edf2f7;font-size:8px;font-weight:700}
.fd-rflow-cart-tools{padding:11px 12px;display:grid;grid-template-columns:minmax(0,1fr) auto auto;gap:8px;border-bottom:1px solid #eef2f5}
.fd-rflow-cart-wrap{overflow-x:auto;scrollbar-width:thin;scrollbar-color:#9aa0a6 transparent}.fd-rflow-cart-wrap::-webkit-scrollbar{height:3px}.fd-rflow-cart-wrap::-webkit-scrollbar-thumb{background:#9aa0a6;border-radius:999px}
.fd-rflow-cart{width:100%;min-width:1000px;border-collapse:collapse}.fd-rflow-cart th{padding:9px 8px;border-bottom:1px solid #e8edf2;background:#f8fafc;color:#68768a;font-size:8px;text-align:left;text-transform:uppercase}.fd-rflow-cart td{padding:8px;border-bottom:1px solid #f0f3f6;color:#405168;font-size:8.5px;vertical-align:middle}
.fd-rflow-cart input,.fd-rflow-cart select{width:100%;min-height:31px;padding:5px 6px;border:1px solid #dfe5ec;border-radius:6px;color:#33445f;background:#fff;font-size:8.5px}.fd-rflow-cart .num{width:86px}.fd-rflow-cart .qty{width:68px}.fd-rflow-remove{width:28px;height:28px;border:0;border-radius:6px;color:#b9444d;background:#fff0f1;cursor:pointer}
.fd-rflow-empty{padding:22px!important;text-align:center;color:#97a1af!important}.fd-rflow-totals{padding:11px 13px;display:flex;justify-content:flex-end;background:#fbfcfd}.fd-rflow-total-box{width:min(330px,100%);display:grid;gap:6px}.fd-rflow-total-row{display:flex;justify-content:space-between;gap:15px;color:#66758a;font-size:9px}.fd-rflow-total-row.grand{padding-top:7px;border-top:1px solid #dfe6ef;color:#17233b;font-size:11px;font-weight:700}
.fd-rflow-price-wrap{position:relative}.fd-rflow-price-symbol{position:absolute;left:8px;top:50%;transform:translateY(-50%);color:#8793a5;font-size:8px;pointer-events:none}.fd-rflow-price-wrap input{padding-left:22px!important}
.fd-rflow-stage-help{grid-column:1/-1;padding:9px 11px;border-radius:8px;background:#f4f8ed;color:#536a38;font-size:8.5px;line-height:1.5;display:none}.fd-rflow-stage-help.show{display:block}
@media(max-width:767.98px){.fd-rflow-assignment-modes{grid-template-columns:1fr}.fd-rflow-cart-tools{grid-template-columns:1fr}.fd-rflow-cart-tools .fd-ar-btn{width:100%}}
</style>
</head>
...
<section class="fd-ar-head">
  <div><h1 class="fd-ar-title">Edit Service Request</h1><p class="fd-ar-sub">Update the existing customer request using the same workflow as Add Request. Assessment and Job stages support assignment; Quote Required loads or creates the linked quotation.</p></div>
  <a href="requests.php" class="fd-ar-btn"><i class="bi bi-arrow-left"></i> Back to Requests</a>
</section>

<form id="addRequestForm" class="fd-card fd-ar-card">
<input type="hidden" name="request_id" id="requestId" value="0">
<div class="fd-ar-card-head"><div><h3 id="editRequestHeading">Request Information</h3><p>Existing request number is preserved. A Quote or Job is generated only when the selected stage requires it and no linked document already exists.</p></div></div>
<div class="fd-ar-body"><div class="fd-ar-grid">

<div class="fd-ar-section"><span>Customer & Service</span><small>Client → Location → Service</small></div>
<div class="fd-ar-field"><label>Client *</label><select name="client_id" id="clientId" class="fd-select2" required><option value="">Select Client</option></select></div>
<div class="fd-ar-field"><label>Service Location</label><select name="location_id" id="locationId" class="fd-select2"><option value="">Select Client First</option></select></div>
<div class="fd-ar-field"><label>Service *</label><select name="product_service_id" id="serviceId" class="fd-select2" required><option value="">Select Service</option></select></div>
<div class="fd-ar-field"><label>Branch</label><select name="branch_id" id="branchId" class="fd-select2"><option value="">No Branch</option></select></div>

<div class="fd-ar-section"><span>Requirement</span><small>What does the customer need?</small></div>
<div class="fd-ar-field full"><label>Request Title *</label><input name="title" id="title" maxlength="190" required placeholder="Example: AC not cooling - inspection required"></div>
<div class="fd-ar-field full"><label>Requirement / Initial Notes</label><textarea name="description" id="description" maxlength="5000" placeholder="Describe issue, scope, customer expectation, access notes or other details."></textarea></div>
<div class="fd-ar-field"><label>Request Source</label><select name="source" id="source" class="fd-select2"><option value="office">Office</option><option value="website">Website</option><option value="portal">Portal</option><option value="phone">Phone</option><option value="sms">SMS</option><option value="email">Email</option><option value="ai">AI</option><option value="other">Other</option></select></div>
<div class="fd-ar-field"><label>Priority</label><select name="priority" id="priority" class="fd-select2"><option value="low">Low</option><option value="normal" selected>Normal</option><option value="high">High</option><option value="urgent">Urgent</option></select></div>
<div class="fd-ar-field"><label>Preferred Date</label><input type="date" name="preferred_date" id="preferredDate"></div>
<div class="fd-ar-field"><label>Request Stage *</label><select name="status" id="status" class="fd-select2" required><option value="new">New</option><option value="contacting">Contacting</option><option value="information_required">Information Required</option><option value="assessment_required">Assessment Required</option><option value="quote_required">Quote Required</option><option value="job_required">Job Required</option></select></div>
<div class="fd-ar-field"><label>Preferred Time From</label><input type="time" name="preferred_time_from" id="preferredFrom"></div>
<div class="fd-ar-field"><label>Preferred Time To</label><input type="time" name="preferred_time_to" id="preferredTo"></div>

<div class="fd-rflow-stage-help" id="stageHelp"></div>

<div class="fd-rflow-assignment" id="assignmentSection">
  <div class="fd-ar-section" style="padding-top:0"><span>Employee Assignment</span><small id="assignmentStageText">Required for this stage</small></div>
  <div class="fd-rflow-assignment-modes">
    <div class="fd-rflow-mode"><input type="radio" name="assignment_mode" id="modeIndividual" value="individual" checked><label for="modeIndividual"><i class="bi bi-person"></i><span><strong>Individual</strong><small>Assign one employee</small></span></label></div>
    <div class="fd-rflow-mode"><input type="radio" name="assignment_mode" id="modeTeam" value="team"><label for="modeTeam"><i class="bi bi-people"></i><span><strong>Team</strong><small>Assign active team members</small></span></label></div>
    <div class="fd-rflow-mode"><input type="radio" name="assignment_mode" id="modeMultiple" value="multiple"><label for="modeMultiple"><i class="bi bi-person-check"></i><span><strong>Multiple Employees</strong><small>Select more than one</small></span></label></div>
  </div>
  <div class="fd-rflow-assign-panel show" id="individualWrap"><div class="fd-ar-field"><label>Employee *</label><select name="individual_user_id" id="individualUserId" class="fd-select2"><option value="">Select Employee</option></select></div></div>
  <div class="fd-rflow-assign-panel" id="teamWrap"><div class="fd-ar-field"><label>Team *</label><select name="team_id" id="teamId" class="fd-select2"><option value="">Select Team</option></select><div class="fd-ar-selected" id="teamMembersPreview"></div></div></div>
  <div class="fd-rflow-assign-panel" id="multipleWrap"><div class="fd-ar-field"><label>Employees *</label><select name="employee_ids[]" id="multipleUserIds" class="fd-select2-multiple" multiple></select><div class="fd-ar-hint">Select2 supports search and multiple checkbox-style selections.</div></div></div>
  <div class="fd-ar-notify" style="margin-top:11px"><label class="fd-ar-check"><input type="checkbox" name="notify_in_app" value="1" checked> Send In-App Notification</label><label class="fd-ar-check"><input type="checkbox" name="notify_email" value="1" checked> Send Email if Tenant SMTP is configured</label><span class="fd-ar-hint" id="smtpHint">Checking tenant SMTP…</span></div>
</div>

<div class="fd-rflow-quote" id="quoteSection">
  <div class="fd-rflow-quote-head"><div><h3>Quotation Builder</h3><small>Build the quote now. The quote will be generated automatically with this request.</small></div><span class="fd-rflow-currency" id="currencyBadge">Currency</span></div>
  <div class="fd-rflow-cart-tools"><select id="catalogItem" class="fd-select2"><option value="">Search product / service / material / fee / discount</option></select><button type="button" class="fd-ar-btn" id="addCatalogItem"><i class="bi bi-cart-plus"></i> Add Selected</button><button type="button" class="fd-ar-btn" id="addManualItem"><i class="bi bi-plus-lg"></i> Manual / Other Charge</button></div>
  <div class="fd-rflow-cart-wrap"><table class="fd-rflow-cart"><thead><tr><th>Item / Charge</th><th>Description</th><th>Qty</th><th>Unit Cost</th><th>Unit Price</th><th>Discount</th><th>Tax %</th><th>Tax</th><th>Total</th><th>Optional</th><th></th></tr></thead><tbody id="quoteCart"><tr><td colspan="11" class="fd-rflow-empty">Add product, service, material, fee or manual charge.</td></tr></tbody></table></div>
  <div class="fd-rflow-totals"><div class="fd-rflow-total-box"><div class="fd-rflow-total-row"><span>Subtotal</span><strong id="quoteSubtotal">0.00</strong></div><div class="fd-rflow-total-row"><span>Discount</span><strong id="quoteDiscount">0.00</strong></div><div class="fd-rflow-total-row"><span>Tax</span><strong id="quoteTax">0.00</strong></div><div class="fd-rflow-total-row grand"><span>Quote Total</span><strong id="quoteTotal">0.00</strong></div></div></div>
  <input type="hidden" name="quote_items_json" id="quoteItemsJson" value="[]">
</div>

<div class="fd-rflow-note"><strong>Edit flow:</strong> Existing request data is loaded automatically. Assessment Required → assignment can be updated. Quote Required → existing linked quote is loaded into the cart, or a quote is created if missing. Job Required → existing linked job is updated, or a new job is created if missing. Other stages do not require employee selection.</div>

</div></div>
<div class="fd-ar-footer"><a href="requests.php" class="fd-ar-btn">Cancel</a><button type="submit" class="fd-ar-btn primary" id="saveButton"><span class="fd-ar-loader"></span><i class="bi bi-check-lg"></i><span id="saveText">Update Request</span></button></div>
</form>
</div>
<div class="fd-ar-toast info" id="toast"><span class="fd-ar-toast-msg" id="toastMsg">Notification</span><button type="button" class="fd-ar-toast-close" id="toastClose"><i class="bi bi-x"></i></button></div>
...
var cart=[],dirty=false,submitting=false,initializing=true,requestId=Number(new URLSearchParams(window.location.search).get('request_id')||0);
function esc(v){return String(v==null?'':v).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#039;')}
function notify(t,m){if(toastTimer)clearTimeout(toastTimer);toast.className='fd-ar-toast '+(t||'info')+' show';toastMsg.textContent=m||'Notification';toastTimer=setTimeout(function(){toast.classList.remove('show')},3000)}
function loading(on){saveButton.disabled=!!on;saveButton.classList.toggle('loading',!!on)}
function parse(r){return r.text().then(function(raw){var text=(raw||'').trim(),d;try{d=text?JSON.parse(text):{}}catch(e){throw new Error(text.replace(/<br\s*\/?>/gi,' ').replace(/<[^>]*>/g,' ').replace(/\s+/g,' ').trim()||'Invalid server response.')}if(!r.ok||!d.success)throw new Error(d.message||'Request failed.');return d})}
function req(fd){fd.append('csrf_token',csrfToken);return fetch('api/request-form.php',{method:'POST',body:fd,credentials:'same-origin',headers:{'X-Requested-With':'XMLHttpRequest','Accept':'application/json'}}).then(parse)}
function options(rows,first){var h='<option value="">'+esc(first)+'</option>';(rows||[]).forEach(function(x){h+='<option value="'+Number(x.id)+'">'+esc(x.name)+'</option>'});return h}
...
function loadMeta(){var fd=new FormData();fd.append('action','meta');req(fd).then(function(d){setMeta(d.meta||{});return loadRequest()}).catch(function(e){notify('error',e.message)})}
function loadLocations(id,selected){if(!id){$('#locationId').html('<option value="">Select Client First</option>').trigger('change.select2');return Promise.resolve()}var fd=new FormData();fd.append('action','locations');fd.append('client_id',id);return req(fd).then(function(d){$('#locationId').html(options(d.locations||[],'No Location / Not Confirmed')).val(selected?String(selected):'').trigger('change.select2')}).catch(function(e){notify('error',e.message)})}
...
function updateStage(){var s=document.getElementById('status').value,assign=s==='assessment_required'||s==='job_required',quote=s==='quote_required';document.getElementById('assignmentSection').classList.toggle('show',assign);document.getElementById('quoteSection').classList.toggle('show',quote);var help=document.getElementById('stageHelp');var msg='';if(s==='assessment_required')msg='Assessment Required: choose the employee/team responsible for the assessment. No Job or Quote is created yet.';if(s==='quote_required')msg='Quote Required: employee selection is hidden. Build the quotation below; a draft Quote number is generated automatically.';if(s==='job_required')msg='Job Required: choose the employee/team. Saving creates the Service Request and Job automatically and links both records.';help.textContent=msg;help.classList.toggle('show',msg!=='');document.getElementById('assignmentStageText').textContent=s==='job_required'?'This assignment will be copied to the generated Job':'Responsible for assessment';if(!assign){$('#individualUserId').val('').trigger('change.select2');$('#teamId').val('').trigger('change.select2');$('#multipleUserIds').val(null).trigger('change')}document.getElementById('saveText').textContent=quote?'Update Request + Quote':(s==='job_required'?'Update Request + Job':'Update Request')}
function teamPreview(id){var box=document.getElementById('teamMembersPreview');box.innerHTML='';var t=(meta.teams||[]).find(function(x){return Number(x.id)===Number(id)});if(t&&Array.isArray(t.members))t.members.forEach(function(m){box.innerHTML+='<span class="fd-ar-chip"><i class="bi bi-person-check"></i>'+esc(m.name)+'</span>'})}
function addCatalog(){var id=Number($('#catalogItem').val()||0);if(!id){notify('warning','Select a catalog item first.');return}var x=(meta.catalog||[]).find(function(r){return Number(r.id)===id});if(!x)return;cart.push({product_service_id:id,item_name:x.name,description:x.description||'',quantity:1,unit_cost:Number(x.unit_cost||0),unit_price:Number(x.unit_price||0),discount_amount:0,tax_percent:Number(x.tax_percent||0),is_optional:0,item_type:x.item_type||'service'});dirty=true;renderCart()}
function addManual(){cart.push({product_service_id:null,item_name:'Other Charge',description:'',quantity:1,unit_cost:0,unit_price:0,discount_amount:0,tax_percent:0,is_optional:0,item_type:'other'});dirty=true;renderCart()}
...
function renderCart(){var b=document.getElementById('quoteCart');if(!cart.length){b.innerHTML='<tr><td colspan="11" class="fd-rflow-empty">Add product, service, material, fee or manual charge.</td></tr>'}else{var h='';cart.forEach(function(x,i){var c=calc(x);h+='<tr data-i="'+i+'"><td><input data-f="item_name" value="'+esc(x.item_name)+'"></td><td><input data-f="description" value="'+esc(x.description)+'"></td><td><input class="qty" type="number" min="0.001" step="0.001" data-f="quantity" value="'+Number(x.quantity||1)+'"></td><td><div class="fd-rflow-price-wrap"><span class="fd-rflow-price-symbol">'+esc(meta.currency.symbol||'')+'</span><input class="num" type="number" min="0" step="0.01" data-f="unit_cost" value="'+Number(x.unit_cost||0).toFixed(2)+'"></div></td><td><div class="fd-rflow-price-wrap"><span class="fd-rflow-price-symbol">'+esc(meta.currency.symbol||'')+'</span><input class="num" type="number" min="0" step="0.01" data-f="unit_price" value="'+Number(x.unit_price||0).toFixed(2)+'"></div></td><td><div class="fd-rflow-price-wrap"><span class="fd-rflow-price-symbol">'+esc(meta.currency.symbol||'')+'</span><input class="num" type="number" min="0" step="0.01" data-f="discount_amount" value="'+Number(x.discount_amount||0).toFixed(2)+'"></div></td><td><input class="num" type="number" min="0" step="0.01" data-f="tax_percent" value="'+Number(x.tax_percent||0)+'"></td><td>'+money(c.tax)+'</td><td><strong>'+money(c.total)+'</strong></td><td><input type="checkbox" data-f="is_optional" '+(Number(x.is_optional)===1?'checked':'')+'></td><td><button type="button" class="fd-rflow-remove" data-remove="'+i+'"><i class="bi bi-trash"></i></button></td></tr>'});b.innerHTML=h}var sub=0,disc=0,tax=0,total=0;cart.forEach(function(x){var c=calc(x);sub+=c.base;disc+=c.discount;tax+=c.tax;total+=c.total});document.getElementById('quoteSubtotal').textContent=money(sub);document.getElementById('quoteDiscount').textContent=money(disc);document.getElementById('quoteTax').textContent=money(tax);document.getElementById('quoteTotal').textContent=money(total);document.getElementById('quoteItemsJson').value=JSON.stringify(cart)}

function loadRequest(){
    if(requestId<=0){
        initializing=false;
        notify('error','Invalid service request.');
        saveButton.disabled=true;
        return Promise.reject(new Error('Invalid service request.'));
    }

    var fd=new FormData();
    fd.append('action','get_edit');
    fd.append('request_id',requestId);

    return req(fd).then(function(d){
        var r=d.request||{};
        document.getElementById('requestId').value=r.id||requestId;
        document.getElementById('editRequestHeading').textContent='Edit '+(r.request_no||'Service Request');
        $('#clientId').val(r.client_id?String(r.client_id):'').trigger('change.select2');
        $('#serviceId').val(r.product_service_id?String(r.product_service_id):'').trigger('change.select2');
        $('#branchId').val(r.branch_id?String(r.branch_id):'').trigger('change.select2');
        $('#source').val(r.source||'office').trigger('change.select2');
        $('#priority').val(r.priority||'normal').trigger('change.select2');

        var editStatus=r.edit_status||r.status||'new';
        if($('#status option[value="'+editStatus+'"]').length===0){
            $('#status').append(new Option(readableStatus(editStatus),editStatus,false,false));
        }
        $('#status').val(editStatus).trigger('change.select2');

        document.getElementById('title').value=r.title||'';
        document.getElementById('description').value=r.description||'';
        document.getElementById('preferredDate').value=r.preferred_date||'';
        document.getElementById('preferredFrom').value=r.preferred_time_from||'';
        document.getElementById('preferredTo').value=r.preferred_time_to||'';

        return loadLocations(r.client_id,r.location_id).then(function(){
            var a=d.assignment||{};
            if(a.mode){
                var radio=document.querySelector('input[name="assignment_mode"][value="'+a.mode+'"]');
                if(radio)radio.checked=true;
                updateAssignmentMode();

                if(a.mode==='individual'){
                    $('#individualUserId').val(a.user_ids&&a.user_ids.length?String(a.user_ids[0]):'').trigger('change.select2');
                }else if(a.mode==='team'){
                    $('#teamId').val(a.team_id?String(a.team_id):'').trigger('change.select2');
                    teamPreview(a.team_id);
                }else if(a.mode==='multiple'){
                    $('#multipleUserIds').val((a.user_ids||[]).map(String)).trigger('change');
                }
            }

            cart=(d.quote_items||[]).map(function(x){
                return {
                    product_service_id:x.product_service_id?Number(x.product_service_id):null,
                    ...
                    is_optional:Number(x.is_optional||0),
                    item_type:x.item_type||'manual'
                };
            });
            ...
            if(d.linked_quote_no){
                document.querySelector('#quoteSection .fd-rflow-quote-head small').textContent=
                    'Editing linked quote '+d.linked_quote_no+'. Existing quote number is preserved.';
            }

            if(d.linked_job_no){
                document.getElementById('stageHelp').textContent=
                    'This request already has linked job '+d.linked_job_no+'. Job Required will update the linked job instead of creating a duplicate.';
                document.getElementById('stageHelp').classList.add('show');
            }
...
function readableStatus(v){
    return String(v||'').replace(/_/g,' ').replace(/\b\w/g,function(x){return x.toUpperCase()});
}

function validateAssignment(){var s=document.getElementById('status').value;if(s!=='assessment_required'&&s!=='job_required')return true;var m=mode();if(m==='individual'&&!$('#individualUserId').val()){notify('warning','Select an employee for this stage.');return false}if(m==='team'&&!$('#teamId').val()){notify('warning','Select a team for this stage.');return false}if(m==='multiple'&&!($('#multipleUserIds').val()||[]).length){notify('warning','Select at least one employee.');return false}return true}
form.addEventListener('input',function(){if(!initializing)dirty=true});form.addEventListener('change',function(){if(!initializing)dirty=true});window.addEventListener('beforeunload',function(e){if(dirty&&!submitting){e.preventDefault();e.returnValue=''}});
$('#clientId').on('change',function(){if(!initializing)loadLocations(this.value,'')});$('#teamId').on('change',function(){teamPreview(this.value)});$('#status').on('change',updateStage);document.querySelectorAll('input[name="assignment_mode"]').forEach(function(x){x.addEventListener('change',updateAssignmentMode)});
document.getElementById('addCatalogItem').onclick=addCatalog;document.getElementById('addManualItem').onclick=addManual;document.getElementById('quoteCart').addEventListener('input',function(e){var tr=e.target.closest('tr[data-i]');if(!tr)return;var i=Number(tr.dataset.i),f=e.target.dataset.f;if(!f||!cart[i])return;if(f==='is_optional')cart[i][f]=e.target.checked?1:0;else if(['quantity','unit_cost','unit_price','discount_amount','tax_percent'].indexOf(f)>=0)cart[i][f]=Number(e.target.value||0);else cart[i][f]=e.target.value;dirty=true;renderCart()});document.getElementById('quoteCart').addEventListener('change',function(e){if(e.target.dataset.f==='is_optional'){var tr=e.target.closest('tr[data-i]'),i=Number(tr.dataset.i);cart[i].is_optional=e.target.checked?1:0;dirty=true;renderCart()}});document.getElementById('quoteCart').addEventListener('click',function(e){var b=e.target.closest('[data-remove]');if(!b)return;cart.splice(Number(b.dataset.remove),1);dirty=true;renderCart()});
form.addEventListener('submit',function(e){e.preventDefault();if(!form.reportValidity()){notify('warning','Complete the required request fields.');return}var from=document.getElementById('preferredFrom').value,to=document.getElementById('preferredTo').value;if(from&&to&&from>=to){notify('warning','Preferred end time must be after start time.');return}if(!validateAssignment())return;var s=document.getElementById('status').value;if(s==='quote_required'&&!cart.length){notify('warning','Add at least one quotation item.');return}renderCart();var fd=new FormData(form);fd.append('action','update');fd.set('request_id',requestId);submitting=true;loading(true);req(fd).then(function(d){dirty=false;var msg=d.message||'Service request updated.';if(d.quote_no)msg+=' Quote: '+d.quote_no+'.';if(d.job_no)msg+=' Job: '+d.job_no+'.';notify('success',msg);setTimeout(function(){window.location.href='requests.php'},1100)}).catch(function(er){submitting=false;notify('error',er.message)}).finally(function(){loading(false)})});
document.getElementById('toastClose').onclick=function(){toast.classList.remove('show')};initSelect2();updateAssignmentMode();updateStage();loadMeta();
})();
</script>
            </div>
        </main>
