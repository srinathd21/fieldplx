...
/**
 * FieldPlx Mobile Master Controls - Departments
 *
 * GET    departments.php
 * GET    departments.php?id=1
 * POST   departments.php
 * PUT    departments.php?id=1
 * PATCH  departments.php?id=1
 * DELETE departments.php?id=1
 */
...
try {
    if ($method === 'GET') {
        ...
        if ($id > 0) {
            $stmt = $pdo->prepare($select . " AND d.id = :id LIMIT 1");
            $stmt->execute(array(
                ':tenant_id' => $tenantId,
                ':id' => $id
            ));
        ...
        $stmt = $pdo->prepare($select . " ORDER BY d.name ASC");
    ...
    if (in_array($method, array('POST', 'PUT', 'PATCH'), true)) {
        ...
        $old = $id > 0
            ? mcapi_fetch_row($pdo, 'departments', $tenantId, $id)
            : null;
        ...
        $params = array(
            ...
            ':name' => $name,
            ':code' => mcapi_string('code', '') ?: null,
            ':description' => mcapi_string('description', '') ?: null,
            ':status' => $status
        );
        ...
        $new = mcapi_fetch_row($pdo, 'departments', $tenantId, $id);
    ...
    if ($method === 'DELETE') {
        ...
        $old = mcapi_fetch_row($pdo, 'departments', $tenantId, $id);
        ...
        if ((int)$usersStmt->fetchColumn() > 0) {
            mcapi_response(
                409,
                false,
                'This department is assigned to employees. Reassign them before deleting it.',
                null,
                'department_in_use'
            );
        }
    ...
    mcapi_pdo_error($e, 'departments');
} catch (Throwable $e) {
    error_log('FieldPlx mobile departments API error: ' . $e->getMessage());
    mcapi_response(
        500,
        false,
