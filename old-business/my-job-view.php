...
<div class="mjv-modal-backdrop" id="updateJobModal">
  <section class="mjv-modal" role="dialog" aria-modal="true" aria-labelledby="updateJobTitle">
    <div class="mjv-modal-head"><h3 id="updateJobTitle">Update Job</h3><button type="button" class="mjv-modal-close" id="closeUpdateJobModal"><i class="bi bi-x-lg"></i></button></div>
...
fetch('api/my-jobs.php',{method:'POST',body:fd,credentials:'same-origin',headers:{'X-Requested-With':'XMLHttpRequest','Accept':'application/json'}}).then(parse).then(function(d){
    ...
    if(workBtn&&Number(r.workflow_id||0)>0){workBtn.href='update-workflow.php?id='+Number(r.id)}
...
var updateJobForm=document.getElementById('updateJobForm');if(updateJobForm)updateJobForm.addEventListener('submit',saveJobUpdate);
