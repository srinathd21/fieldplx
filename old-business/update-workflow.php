...
if (empty($_SESSION['my_jobs_csrf_token'])) {
    $_SESSION['my_jobs_csrf_token'] = bin2hex(random_bytes(32));
}

$myJobsCsrfToken = (string)$_SESSION['my_jobs_csrf_token'];
...
<head>
    ...
    <title>Update Work - FieldPlx</title>
...
a,a:link,a:visited,a:hover,a:focus,a:active{text-decoration:none!important}.mjv-head{display:flex;align-items:flex-start;justify-content:space-between;gap:14px;margin-bottom:15px}.mjv-title{margin:0 0 5px;color:#0b1933;font-size:21px;font-weight:700}.mjv-sub{margin:0;color:#6f7b90;font-size:10px}.mjv-back{min-height:37px;padding:0 11px;display:inline-flex;align-items:center;gap:6px;border:1px solid #dfe5ec;border-radius:8px;color:#43546c;background:#fff;font-size:9px;font-weight:700}.mjv-head-actions{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.mjv-work-btn{min-height:37px;padding:0 12px;display:inline-flex;align-items:center;justify-content:center;gap:7px;border:1px solid #74b824;border-radius:8px;color:#fff;background:linear-gradient(90deg,#7fc92d,#68aa1d);box-shadow:0 6px 14px rgba(104,170,29,.15);font-size:9px;font-weight:700}.mjv-work-btn:hover{color:#fff;background:linear-gradient(90deg,#74b824,#5d971b)}.mjv-grid{display:grid;grid-template-columns:minmax(0,1.5fr) minmax(280px,.7fr);gap:14px}.mjv-card{padding:0;overflow:hidden}.mjv-card-head{padding:12px 14px;border-bottom:1px solid #e7ecf1;background:#fbfcfd}.mjv-card-head h3{margin:0;color:#23344d;font-size:11px}.mjv-body{padding:14px}.mjv-job-top{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}.mjv-number{display:block;margin-bottom:5px;color:#123d70;font-size:9px;font-weight:700}.mjv-job-title{margin:0;color:#14233b;font-size:16px;font-weight:700}.mjv-badge{padding:4px 7px;border-radius:5px;color:#123d70;background:#edf2f7;font-size:8px;font-weight:700;text-transform:capitalize}.mjv-desc{margin:12px 0 0;color:#5f6f83;font-size:9.5px;line-height:1.65;white-space:pre-wrap}.mjv-info{margin-top:14px;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}.mjv-info-item{padding:10px;border:1px solid #edf1f4;border-radius:8px;background:#fcfdfd}.mjv-info-item small{display:block;margin-bottom:4px;color:#8a96a7;font-size:7.5px;text-transform:uppercase}.mjv-info-item strong{display:block;color:#3b4e66;font-size:9.5px;line-height:1.45}.mjv-section{margin-top:14px;padding-top:13px;border-top:1px solid #edf1f4}.mjv-section-title{margin:0 0 9px;color:#334760;font-size:9px;font-weight:700;text-transform:uppercase}.mjv-person{padding:10px;display:flex;align-items:center;gap:9px;border:1px solid #edf1f4;border-radius:8px;background:#fff}.mjv-person-icon{width:32px;height:32px;display:grid;place-items:center;border-radius:8px;color:#5d971b;background:#f0f8e5}.mjv-person strong,.mjv-person small{display:block}.mjv-person strong{color:#253750;font-size:9.5px}.mjv-person small{margin-top:2px;color:#8490a2;font-size:8px}.mjv-side-list{display:grid;gap:9px}.mjv-side-row{padding:10px 11px;border:1px solid #edf1f4;border-radius:8px;background:#fff}.mjv-side-row small{display:block;margin-bottom:4px;color:#8d98a7;font-size:7.5px;text-transform:uppercase}.mjv-side-row strong{display:block;color:#354961;font-size:9.5px;line-height:1.45}.mjv-empty{padding:38px 20px;text-align:center;color:#8290a3}.mjv-empty i{display:block;margin-bottom:10px;font-size:28px}.mjv-toast{width:min(300px,calc(100vw - 24px));position:fixed;top:82px;right:16px;z-index:25000;padding:8px 9px;border-radius:7px;color:#fff;background:#e45b66;box-shadow:0 10px 26px rgba(0,17,49,.18);opacity:0;transform:translateY(-8px);transition:.18s}.mjv-toast.show{opacity:1;transform:translateY(0)}
@media(max-width:900px){.mjv-grid{grid-template-columns:1fr}}@media(max-width:575px){.mjv-head{flex-direction:column}.mjv-head-actions{width:100%;display:grid;grid-template-columns:1fr 1fr}.mjv-back,.mjv-work-btn{width:100%;justify-content:center}.mjv-info{grid-template-columns:1fr}.mjv-toast{left:12px;right:12px;width:auto}}

/* ==========================================================
   Technician Update Work / Workflow Execution
   ========================================================== */
.fw-head{display:flex;align-items:flex-start;justify-content:space-between;gap:14px;margin-bottom:15px}
.fw-title{margin:0 0 5px;color:#0b1933;font-size:21px;font-weight:700}
.fw-sub{margin:0;color:#6f7b90;font-size:10px;line-height:1.5}
.fw-actions{display:flex;align-items:center;gap:8px;flex-wrap:wrap}
.fw-btn{min-height:38px;padding:0 12px;display:inline-flex;align-items:center;justify-content:center;gap:7px;border:1px solid #dfe5ec;border-radius:8px;color:#43546c;background:#fff;font-size:9px;font-weight:700;cursor:pointer;text-decoration:none!important}
.fw-btn:hover{border-color:#cfe3ae;color:#5d971b;background:#f9fcf4}
.fw-btn.primary{border-color:#74b824;color:#fff;background:linear-gradient(90deg,#7fc92d,#68aa1d);box-shadow:0 6px 15px rgba(104,170,29,.14)}
.fw-btn.primary:hover{color:#fff;background:linear-gradient(90deg,#74b824,#5d971b)}
.fw-btn:disabled{opacity:.55;cursor:not-allowed}
.fw-loader{width:13px;height:13px;display:none;border:2px dotted currentColor;border-radius:50%;animation:fwSpin .75s linear infinite}
.fw-btn.loading .fw-loader{display:inline-block}@keyframes fwSpin{to{transform:rotate(360deg)}}
.fw-summary{margin-bottom:14px;padding:13px 15px;display:grid;grid-template-columns:minmax(0,1fr) auto;align-items:center;gap:14px}
.fw-summary-main{min-width:0}.fw-job-no{display:block;margin-bottom:4px;color:#123d70;font-size:9px;font-weight:700}.fw-job-name{margin:0;color:#15243c;font-size:15px;font-weight:700}.fw-job-meta{margin-top:5px;color:#6f7b90;font-size:8.5px}
.fw-progress-box{min-width:210px}.fw-progress-top{margin-bottom:6px;display:flex;align-items:center;justify-content:space-between;gap:8px;color:#52627a;font-size:8px}.fw-progress-top strong{color:#263a55;font-size:9px}.fw-progress-track{height:7px;overflow:hidden;border-radius:999px;background:#edf1f4}.fw-progress-fill{height:100%;width:0;border-radius:999px;background:linear-gradient(90deg,#7fc92d,#68aa1d);transition:width .25s ease}
.fw-layout{display:grid;grid-template-columns:310px minmax(0,1fr);gap:14px}
.fw-card{overflow:hidden}.fw-card-head{padding:11px 13px;border-bottom:1px solid #e7ecf1;background:#fbfcfd}.fw-card-head h3{margin:0;color:#263950;font-size:10.5px}.fw-card-head small{display:block;margin-top:3px;color:#8a96a7;font-size:8px}
.fw-step-list{max-height:calc(100vh - 300px);overflow:auto;padding:7px;scrollbar-width:thin}
.fw-step{width:100%;padding:10px;display:flex;align-items:flex-start;gap:9px;border:1px solid transparent;border-radius:8px;color:#4a5c74;background:transparent;text-align:left;cursor:pointer}
.fw-step:hover{background:#f8faf8}.fw-step.active{border-color:#cfe3ae;background:#f5faee}
.fw-step.locked{opacity:.58;cursor:not-allowed}.fw-step-icon{width:29px;height:29px;flex:0 0 29px;display:grid;place-items:center;border-radius:8px;color:#66758a;background:#edf2f7;font-size:12px}.fw-step.completed .fw-step-icon{color:#fff;background:#74b824}.fw-step.in_progress .fw-step-icon,.fw-step.available .fw-step-icon{color:#5d971b;background:#f0f8e5}
.fw-step-copy{min-width:0;flex:1}.fw-step-copy strong{display:block;color:#263950;font-size:9.5px;line-height:1.35}.fw-step-copy small{display:block;margin-top:3px;color:#8793a5;font-size:7.8px;line-height:1.35}.fw-step-status{margin-top:5px;display:inline-flex;padding:3px 5px;border-radius:4px;color:#617087;background:#eef2f6;font-size:7px;font-weight:700;text-transform:capitalize}
.fw-step.completed .fw-step-status{color:#5d971b;background:#edf7e2}.fw-step.in_progress .fw-step-status,.fw-step.available .fw-step-status{color:#8a6312;background:#fff6df}
.fw-work-body{padding:15px}.fw-current-top{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:14px}.fw-current-top h2{margin:0 0 4px;color:#13243d;font-size:15px}.fw-current-top p{margin:0;color:#77859a;font-size:8.5px;line-height:1.5}.fw-status{padding:5px 7px;border-radius:5px;color:#52627a;background:#eef2f6;font-size:8px;font-weight:700;text-transform:capitalize}.fw-status.completed{color:#5d971b;background:#edf7e2}.fw-status.in_progress,.fw-status.available{color:#8a6312;background:#fff6df}
.fw-form{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.fw-field.full{grid-column:1/-1}.fw-field label{display:block;margin-bottom:6px;color:#3f526a;font-size:9px;font-weight:700}.fw-field label .required{color:#e45b66}.fw-help{display:block;margin-top:4px;color:#8a96a7;font-size:7.8px;line-height:1.4}
.fw-field input[type="text"],.fw-field input[type="number"],.fw-field input[type="date"],.fw-field input[type="time"],.fw-field input[type="datetime-local"],.fw-field select,.fw-field textarea,.fw-field input[type="file"]{width:100%;min-height:40px;padding:8px 10px;border:1px solid #dfe5ec;border-radius:8px;outline:0;color:#263750;background:#fff;font-size:9.5px}.fw-field textarea{min-height:86px;resize:vertical}.fw-field input:focus,.fw-field select:focus,.fw-field textarea:focus{border-color:#a9cf75;box-shadow:0 0 0 3px rgba(116,184,36,.11)}
.fw-options{display:grid;gap:7px}.fw-option{min-height:36px;padding:7px 9px;display:flex;align-items:center;gap:8px;border:1px solid #e3e8ed;border-radius:7px;color:#52627a;background:#fff;font-size:8.5px;cursor:pointer}.fw-option input{width:14px;height:14px;accent-color:#74b824}.fw-heading{grid-column:1/-1;padding:7px 0 5px;border-bottom:1px solid #edf1f4;color:#2c415b;font-size:10px;font-weight:700}
.fw-readonly{padding:9px 10px;border:1px solid #edf1f4;border-radius:8px;color:#4a5c74;background:#fbfcfd;font-size:9px;white-space:pre-wrap}.fw-file-existing{margin-top:5px}.fw-file-existing a{color:#5d971b;font-size:8px;font-weight:700}
.fw-notes{grid-column:1/-1;padding-top:5px;border-top:1px solid #eef2f5}
.fw-footer{margin-top:14px;padding-top:12px;display:flex;justify-content:flex-end;gap:8px;border-top:1px solid #edf1f4}
.fw-empty{min-height:340px;padding:30px;display:grid;place-items:center;color:#8793a5;text-align:center}.fw-empty i{display:block;margin-bottom:9px;font-size:28px}
.fw-toast{width:min(300px,calc(100vw - 24px));position:fixed;top:82px;right:16px;z-index:25000;padding:8px 9px;display:flex;align-items:center;gap:7px;border-radius:7px;color:#fff;background:#123d70;box-shadow:0 10px 26px rgba(0,17,49,.18);opacity:0;transform:translateY(-8px);pointer-events:none;transition:.18s}.fw-toast.show{opacity:1;transform:translateY(0)}.fw-toast.success{background:#5d971b}.fw-toast.error{background:#e45b66}.fw-toast.warning{background:#96a52f}.fw-toast span{flex:1;font-size:8.5px;font-weight:600}.fw-toast button{border:0;color:#fff;background:transparent}
@media(max-width:900px){.fw-layout{grid-template-columns:1fr}.fw-step-list{max-height:none}.fw-summary{grid-template-columns:1fr}.fw-progress-box{min-width:0}}
@media(max-width:575px){.fw-head{flex-direction:column}.fw-actions{width:100%}.fw-actions .fw-btn{flex:1}.fw-form{grid-template-columns:1fr}.fw-field.full,.fw-heading,.fw-notes{grid-column:auto}.fw-toast{top:72px;left:12px;right:12px;width:auto}}
</style>
</head>
...
<section class="fw-head">
  <div>
    <h1 class="fw-title">Update Work</h1>
    <p class="fw-sub">Complete the assigned service workflow step by step. Required fields must be completed before moving to the next step.</p>
  </div>
  <div class="fw-actions">
    <a class="fw-btn" href="my-job-view.php?id=<?= (int)$jobId ?>"><i class="bi bi-arrow-left"></i> Job Details</a>
    <a class="fw-btn" href="my-jobs.php"><i class="bi bi-briefcase"></i> My Jobs</a>
  </div>
</section>

<section class="fd-card fw-summary" id="jobSummary">
  <div class="fw-summary-main">
    <span class="fw-job-no">Loading...</span>
    <h2 class="fw-job-name">Loading workflow</h2>
    <div class="fw-job-meta">Please wait.</div>
  </div>
  <div class="fw-progress-box">
    <div class="fw-progress-top"><span>Workflow Progress</span><strong id="progressText">0%</strong></div>
    <div class="fw-progress-track"><div class="fw-progress-fill" id="progressFill"></div></div>
  </div>
</section>

<div class="fw-layout">
  <aside class="fd-card fw-card">
    <div class="fw-card-head"><h3>Workflow Steps</h3><small>Complete required steps in order.</small></div>
    <div class="fw-step-list" id="stepList"><div class="fw-empty"><div><i class="bi bi-hourglass-split"></i>Loading steps...</div></div></div>
  </aside>

  <section class="fd-card fw-card">
    <div class="fw-card-head"><h3>Work Update</h3><small>Enter the required evidence and work details for the selected step.</small></div>
    <div class="fw-work-body" id="workBody"><div class="fw-empty"><div><i class="bi bi-list-check"></i>Select an available workflow step.</div></div></div>
  </section>
</div>
</div>

<div class="fw-toast" id="fwToast"><span id="fwToastMsg">Notification</span><button type="button" id="fwToastClose"><i class="bi bi-x"></i></button></div> <script>
(function(){
'use strict';
...
var toastMsg=document.getElementById('fwToastMsg');
var progressFill=document.getElementById('progressFill');
var progressText=document.getElementById('progressText');
var state={job:null,workflow:null,steps:[],selectedId:0,dirty:false,submitting:false};
var toastTimer=null;

function esc(v){return String(v==null?'':v).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#039;')}
function title(v){return String(v||'-').replace(/_/g,' ').replace(/\b\w/g,function(x){return x.toUpperCase()})}
function notify(type,msg){if(toastTimer)clearTimeout(toastTimer);toast.className='fw-toast '+(type||'')+' show';toastMsg.textContent=msg||'Notification';toastTimer=setTimeout(function(){toast.classList.remove('show')},3000)}
function parse(r){return r.text().then(function(raw){var d,t=(raw||'').trim();try{d=t?JSON.parse(t):{}}catch(e){var clean=t.replace(/<br\s*\/?>/gi,' ').replace(/<[^>]*>/g,' ').replace(/\s+/g,' ').trim();throw new Error(clean||'Invalid server response.')}if(!r.ok||!d.success)throw new Error(d.message||'Unable to process workflow.');return d})}
function req(fd){fd.append('csrf_token',csrfToken);return fetch('api/update-workflow.php',{method:'POST',body:fd,credentials:'same-origin',headers:{'X-Requested-With':'XMLHttpRequest','Accept':'application/json'}}).then(parse)}
function statusIcon(s){if(s==='completed')return 'bi-check-lg';if(s==='in_progress')return 'bi-play-fill';if(s==='available')return 'bi-unlock';if(s==='blocked')return 'bi-slash-circle';return 'bi-lock'}
function calculateProgress(){var total=state.steps.length,done=state.steps.filter(function(s){return s.status==='completed'||s.status==='skipped'}).length;var pct=total?Math.round(done/total*100):0;progressFill.style.width=pct+'%';progressText.textContent=pct+'% ('+done+'/'+total+')'}
function renderSummary(){
  var j=state.job||{},w=state.workflow||{};
  summary.querySelector('.fw-job-no').textContent=j.job_no||'Job';
  summary.querySelector('.fw-job-name').textContent=j.title||'Assigned Work';
  summary.querySelector('.fw-job-meta').textContent=(j.service_name||'-')+' · Workflow: '+(w.name||'-')+' · '+title(j.status);
  calculateProgress();
}
function renderSteps(){
  if(!state.steps.length){stepList.innerHTML='<div class="fw-empty"><div><i class="bi bi-exclamation-circle"></i>No workflow steps configured.</div></div>';return}
  var html='';
  state.steps.forEach(function(s,index){
    var selectable=['available','in_progress','completed'].indexOf(s.status)!==-1;
    html+='<button type="button" class="fw-step '+esc(s.status)+(Number(s.id)===Number(state.selectedId)?' active':'')+(selectable?'':' locked')+'" data-step="'+Number(s.id)+'" '+(selectable?'':'disabled')+'>'+
      '<span class="fw-step-icon"><i class="bi '+statusIcon(s.status)+'"></i></span>'+
      '<span class="fw-step-copy"><strong>'+(index+1)+'. '+esc(s.step_name)+'</strong><small>'+esc(s.description||'No step description.')+'</small><span class="fw-step-status">'+esc(title(s.status))+'</span></span>'+
    '</button>';
  });
  stepList.innerHTML=html;
}
function optionRows(field,type){
  var opts=field.options||[],saved=field.saved_value;
  if(type==='select'){
    var h='<option value="">Select</option>';
    opts.forEach(function(o){h+='<option value="'+esc(o.option_value)+'" '+(String(saved||'')===String(o.option_value)?'selected':'')+'>'+esc(o.option_label)+'</option>'});
    return '<select name="field_'+Number(field.id)+'">'+h+'</select>';
  }
  var inputType=type==='radio'?'radio':'checkbox';
  var values=Array.isArray(saved)?saved:[saved];
  return '<div class="fw-options">'+opts.map(function(o){
    var checked=values.map(String).indexOf(String(o.option_value))!==-1?' checked':'';
    var name='field_'+Number(field.id)+(type==='checkbox'?'[]':'');
    return '<label class="fw-option"><input type="'+inputType+'" name="'+name+'" value="'+esc(o.option_value)+'"'+checked+'><span>'+esc(o.option_label)+'</span></label>';
  }).join('')+'</div>';
}
function fieldHtml(f,readonly){
  var type=f.field_type,req=Number(f.is_required)===1,saved=f.saved_value;
  var label='<label>'+esc(f.label||f.field_key)+(req?'<span class="required"> *</span>':'')+'</label>';
  var help=f.help_text?'<small class="fw-help">'+esc(f.help_text)+'</small>':'';
  var disabled=readonly?' disabled':'';
  if(type==='heading')return '<div class="fw-heading">'+esc(f.label||f.default_value||'Information')+'</div>';

  if(readonly){
    var display=Array.isArray(saved)?saved.join(', '):(saved||'-');
    if(f.saved_file_url)display='<a href="'+esc(f.saved_file_url)+'" target="_blank">View uploaded file</a>';
    return '<div class="fw-field '+(['textarea','photo_multiple','file','signature','location','customer_confirmation'].indexOf(type)!==-1?'full':'')+'">'+label+'<div class="fw-readonly">'+display+'</div>'+help+'</div>';
  }

  var input='',name='field_'+Number(f.id),placeholder=esc(f.placeholder||'');
  if(type==='textarea'||type==='customer_confirmation'||type==='location'){
    input='<textarea name="'+name+'" placeholder="'+placeholder+'" '+(req?'required':'')+'>'+esc(saved||'')+'</textarea>';
  }else if(type==='number'||type==='decimal'){
    input='<input type="number" name="'+name+'" value="'+esc(saved||'')+'" step="'+(type==='decimal'?'0.01':'1')+'" '+(f.min_value!==null?'min="'+esc(f.min_value)+'" ':'')+(f.max_value!==null?'max="'+esc(f.max_value)+'" ':'')+(req?'required':'')+'>';
  }else if(type==='date'||type==='time'||type==='datetime'){
    input='<input type="'+(type==='datetime'?'datetime-local':type)+'" name="'+name+'" value="'+esc(saved||'')+'" '+(req?'required':'')+'>';
  }else if(type==='yes_no'){
    input='<select name="'+name+'" '+(req?'required':'')+'><option value="">Select</option><option value="yes" '+(saved==='yes'?'selected':'')+'>Yes</option><option value="no" '+(saved==='no'?'selected':'')+'>No</option></select>';
  }else if(type==='select'||type==='radio'||type==='checkbox'||type==='checklist'){
    input=optionRows(f,type==='checklist'?'checkbox':type);
  }else if(type==='photo_single'||type==='photo_multiple'||type==='file'||type==='signature'){
    var accept=f.accept_types||((type==='signature'||type.indexOf('photo_')===0)?'image/*':'');
    input='<input type="file" name="'+name+(type==='photo_multiple'?'[]':'')+'" '+(type==='photo_multiple'?'multiple ':'')+(accept?'accept="'+esc(accept)+'" ':'')+(req&&!f.saved_file_url?'required':'')+'>';
    if(f.saved_file_url)input+='<div class="fw-file-existing"><a href="'+esc(f.saved_file_url)+'" target="_blank"><i class="bi bi-paperclip"></i> Existing upload</a></div>';
  }else{
    input='<input type="text" name="'+name+'" value="'+esc(saved||'')+'" placeholder="'+placeholder+'" '+(f.min_length!==null?'minlength="'+Number(f.min_length)+'" ':'')+(f.max_length!==null?'maxlength="'+Number(f.max_length)+'" ':'')+(req?'required':'')+'>';
  }
  return '<div class="fw-field '+(['textarea','photo_multiple','file','signature','location','customer_confirmation','checkbox','checklist','radio'].indexOf(type)!==-1?'full':'')+'">'+label+input+help+'</div>';
}
function selectedStep(){return state.steps.find(function(s){return Number(s.id)===Number(state.selectedId)})||null}
function renderWork(){
  var s=selectedStep();
  if(!s){workBody.innerHTML='<div class="fw-empty"><div><i class="bi bi-list-check"></i>Select an available workflow step.</div></div>';return}
  var readonly=s.status==='completed';
  var fields=(s.fields||[]).map(function(f){return fieldHtml(f,readonly)}).join('');
  var buttons='';
  if(!readonly){
    buttons='<div class="fw-footer"><button type="button" class="fw-btn" id="saveProgress"><span class="fw-loader"></span><i class="bi bi-save"></i> Save Progress</button><button type="submit" class="fw-btn primary" id="completeStep"><span class="fw-loader"></span><i class="bi bi-check2-circle"></i> Complete Step</button></div>';
  }else{
    buttons='<div class="fw-footer"><span class="fw-btn" style="cursor:default"><i class="bi bi-check-circle"></i> Completed</span></div>';
  }
  workBody.innerHTML='<div class="fw-current-top"><div><h2>'+esc(s.step_name)+'</h2><p>'+esc(s.description||'Complete this workflow step.')+'</p></div><span class="fw-status '+esc(s.status)+'">'+esc(title(s.status))+'</span></div>'+
  '<form id="stepForm" enctype="multipart/form-data"><div class="fw-form">'+fields+
  '<div class="fw-field fw-notes full"><label>Technician Notes</label><textarea name="step_notes" placeholder="Add work notes for this step...">'+esc(s.notes||'')+'</textarea></div></div>'+buttons+'</form>';
  if(!readonly){
    ...
    document.getElementById('saveProgress').onclick=function(){saveStep(false,this)};
    form.onsubmit=function(e){e.preventDefault();saveStep(true,document.getElementById('completeStep'))};
  }
}
function selectStep(id){
  var s=state.steps.find(function(x){return Number(x.id)===Number(id)});
  if(!s||['available','in_progress','completed'].indexOf(s.status)===-1)return;
  if(state.dirty&&!confirm('You have unsaved workflow changes. Continue without saving?'))return;
  state.dirty=false;state.selectedId=Number(id);renderSteps();renderWork();
}
function load(){
  var fd=new FormData();fd.append('action','load');fd.append('job_id',jobId);
  req(fd).then(function(d){
    state.job=d.job||{};state.workflow=d.workflow||{};state.steps=d.steps||[];
    var current=state.steps.find(function(s){return s.status==='in_progress'})||state.steps.find(function(s){return s.status==='available'})||state.steps.find(function(s){return s.status==='completed'});
    state.selectedId=current?Number(current.id):0;
    state.dirty=false;renderSummary();renderSteps();renderWork();
  }).catch(function(e){notify('error',e.message);workBody.innerHTML='<div class="fw-empty"><div><i class="bi bi-exclamation-circle"></i>'+esc(e.message)+'</div></div>'});
}
function saveStep(complete,button){
  var s=selectedStep(),form=document.getElementById('stepForm');if(!s||!form)return;
  if(complete&&!form.reportValidity()){notify('warning','Complete all required workflow fields.');return}
  var fd=new FormData(form);fd.append('action','save_step');fd.append('job_id',jobId);fd.append('workflow_step_id',s.id);fd.append('complete_step',complete?'1':'0');
  button.disabled=true;button.classList.add('loading');state.submitting=true;
  req(fd).then(function(d){state.dirty=false;notify('success',d.message||'Work updated successfully.');return load()}).catch(function(e){notify('error',e.message)}).finally(function(){state.submitting=false;button.disabled=false;button.classList.remove('loading')});
}
stepList.addEventListener('click',function(e){var b=e.target.closest('[data-step]');if(b&&!b.disabled)selectStep(Number(b.dataset.step))});
window.addEventListener('beforeunload',function(e){if(state.dirty&&!state.submitting){e.preventDefault();e.returnValue=''}});
document.getElementById('fwToastClose').onclick=function(){toast.classList.remove('show')};
if(jobId<=0){notify('error','Invalid job.')}else{load()}
})();
</script>
            </div>
        </main>
