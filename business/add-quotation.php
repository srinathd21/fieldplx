...
.qa-direct-box{margin-top:12px;padding:12px;border:1px solid #dfe7ef;border-radius:9px;background:#fbfcfd}.qa-direct-box.hide{display:none}.qa-product-tools{margin-top:10px;display:grid;grid-template-columns:minmax(240px,1fr) 110px auto auto;gap:8px;align-items:end}.qa-product-field label{display:block;margin-bottom:5px;color:#42536c;font-size:9px;font-weight:700}.qa-product-field input,.qa-product-field select,.qa-product-modal input,.qa-product-modal select,.qa-product-modal textarea{width:100%;min-height:39px;padding:8px 10px;border:1px solid #dfe5ec;border-radius:8px;background:#fff;color:#263750;font-size:10px;outline:0}.qa-product-create{display:none}.qa-product-create.show{display:inline-flex}.qa-modal .modal-content{border:0;border-radius:12px;overflow:hidden}.qa-modal .modal-header{background:#fbfcfd;border-bottom:1px solid var(--fd-border)}.qa-modal .modal-title{font-size:12px;color:var(--fd-text)}.qa-product-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.qa-product-grid .full{grid-column:1/-1}.qa-product-modal label{display:block;margin-bottom:5px;color:#42536c;font-size:9px;font-weight:700}.qa-product-modal textarea{min-height:85px;resize:vertical}.qa-unsaved-copy{font-size:10px;color:#56667c;line-height:1.6}@media(max-width:767.98px){.qa-product-tools{grid-template-columns:1fr 100px}.qa-product-tools .qa-btn{width:100%}.qa-product-grid{grid-template-columns:1fr}.qa-product-grid .full{grid-column:auto}}
</style>
</head>
<body>
    ...
    <main class="fieldplx-main-content">
        <div class="fieldplx-content-wrapper">
            <div class="fd-dashboard">
                <section class="qa-head"><div><h1 class="qa-title"><?= $viewOnly?'View Quotation':($quoteId>0?'Edit Quotation':'Add Quotation') ?></h1><p class="qa-sub">Create a quotation directly for a customer or optionally link it to an enquiry / revisit.</p></div><div class="qa-actions"><a class="qa-btn qa-nav-link" href="quotes.php"><i class="bi bi-arrow-left"></i> Back to Quotations</a></div></section>
                <form id="quoteForm" autocomplete="off">
                    ...
                    <section class="fd-card qa-card">
                        <div class="qa-section"><div class="qa-section-title"><div><strong>1. Select Enquiry / Revisit</strong><small>Optional. Leave this blank to create a direct quotation.</small></div><span class="qa-currency" id="currencyBadge">Currency</span></div><div class="qa-field"><label>Service Enquiry / Revisit</label><select name="request_source" id="requestId" <?= $viewOnly?'disabled':'' ?>><option value="">Direct Quotation - No Enquiry</option></select></div><div class="qa-enquiry-info" id="enquiryInfo"><div class="qa-info"><small>Customer</small><strong id="infoCustomer">-</strong></div><div class="qa-info"><small>Location</small><strong id="infoLocation">-</strong></div><div class="qa-info"><small>Requested Service</small><strong id="infoService">-</strong></div><div class="qa-info"><small>Enquiry Status</small><strong id="infoStatus">-</strong></div><div class="qa-info"><small>Quotation Source</small><strong id="infoSource">-</strong></div><div class="qa-info"><small>Visit Schedule</small><strong id="infoVisit">-</strong></div><div class="qa-info"><small>Revisit Remarks</small><strong id="infoRemarks">-</strong></div></div><div class="qa-direct-box" id="directBox"><div class="qa-grid"><div class="qa-field"><label>Customer *</label><select name="client_id" id="clientId" <?= $viewOnly?'disabled':'' ?>><option value="">Select Customer</option></select></div><div class="qa-field"><label>Customer Location</label><select name="location_id" id="locationId" <?= $viewOnly?'disabled':'' ?>><option value="">No Location</option></select></div></div></div></div>
                        <div class="qa-builder show" id="builder">
                            <div class="qa-section"><div class="qa-section-title"><div><strong>2. Quotation Details</strong><small>Add title, status, validity and customer-facing introduction.</small></div></div><div class="qa-grid"><div class="qa-field"><label>Quotation Title *</label><input type="text" name="title" id="quoteTitle" maxlength="190" required <?= $viewOnly?'disabled':'' ?>></div><div class="qa-field"><label>Status *</label><select name="status" id="quoteStatus" required <?= $viewOnly?'disabled':'' ?>><option value="sent" selected>Send for Approval</option><option value="draft">Draft</option><option value="internal_approval">Internal Approval</option><option value="approved">Approved</option><option value="viewed">Viewed</option><option value="changes_requested">Changes Requested</option><option value="rejected">Rejected</option><option value="expired">Expired</option></select></div><div class="qa-field"><label>Valid Until</label><input type="date" name="valid_until" id="validUntil" <?= $viewOnly?'disabled':'' ?>></div><div class="qa-field full"><label>Introduction / Notes</label><textarea name="introduction" id="introduction" <?= $viewOnly?'disabled':'' ?>></textarea></div></div></div>
                            <div class="qa-section"><div class="qa-section-title"><div><strong>3. Services, Products & Charges</strong><small>Add service/catalog charges, existing products, or create a new product without refreshing.</small></div></div><?php if(!$viewOnly): ?><div class="qa-tools"><select id="catalogItem"><option value="">Search service / material / fee / discount</option></select><button type="button" class="qa-btn" id="addCatalog"><i class="bi bi-cart-plus"></i> Add Selected</button><button type="button" class="qa-btn" id="addManual"><i class="bi bi-plus-lg"></i> Manual / Other Charge</button></div><div class="qa-product-tools"><div class="qa-product-field"><label>Product</label><input type="text" id="productSearch" list="productOptions" placeholder="Search existing product or type new product name"><datalist id="productOptions"></datalist></div><div class="qa-product-field"><label>Qty</label><input type="number" id="productQty" min="0.001" step="0.001" value="1"></div><button type="button" class="qa-btn primary" id="addProduct"><i class="bi bi-cart-plus"></i> Add to Cart</button><button type="button" class="qa-btn qa-product-create" id="openProductModal"><i class="bi bi-plus-circle"></i> Create New Product</button></div><?php endif; ?></div>
                            <div class="qa-table-wrap"><table class="qa-table"><thead><tr><th>Type</th><th>Item / Charge</th><th>Description</th><th>Qty</th><th>Unit Cost</th><th>Unit Price</th><th>Discount</th><th>Tax %</th><th>Tax</th><th>Total</th><th>Optional</th><th></th></tr></thead><tbody id="itemRows"><tr><td colspan="12" class="qa-empty">Add a service, product or charge.</td></tr></tbody></table></div>
                            ...
                            <div class="qa-footer"><a class="qa-btn qa-nav-link" href="quotes.php">Cancel</a><?php if(!$viewOnly): ?><button class="qa-btn primary" type="submit" id="saveButton"><span class="qa-loader"></span><i class="bi bi-check2-circle"></i> <span><?= $quoteId>0?'Update Quotation':'Create Quotation' ?></span></button><?php endif; ?></div>
                        </div>
                    </section>
                </form>
            </div>
...
<div class="modal fade qa-modal" id="productModal" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-dialog-centered"><div class="modal-content"><div class="modal-header"><h5 class="modal-title">Create New Product</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body qa-product-modal"><div class="qa-product-grid"><div><label>Product Name *</label><input type="text" id="newProductName" maxlength="190"></div><div><label>Unit / Base Price *</label><input type="number" id="newProductBasePrice" min="0" step="0.01" value="0.00"></div><div><label>Markup Type *</label><select id="newProductMarkupType"><option value="percentage">Percentage %</option><option value="fixed">Fixed Amount</option></select></div><div><label>Markup Value</label><input type="number" id="newProductMarkupValue" min="0" step="0.01" value="0.00"></div><div><label>Tax %</label><input type="number" id="newProductTax" min="0" step="0.01" value="0"></div><div class="full"><label>Description</label><textarea id="newProductDescription"></textarea></div></div></div><div class="modal-footer"><button type="button" class="qa-btn" data-bs-dismiss="modal">Cancel</button><button type="button" class="qa-btn primary" id="createProductButton"><span class="qa-loader"></span><i class="bi bi-check-lg"></i> Create Product</button></div></div></div></div>
<div class="modal fade qa-modal" id="unsavedModal" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-dialog-centered"><div class="modal-content"><div class="modal-header"><h5 class="modal-title">Unsaved Quotation Changes</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body qa-unsaved-copy">You have unsaved quotation changes. Do you want to leave this page and discard them?</div><div class="modal-footer"><button type="button" class="qa-btn" data-bs-dismiss="modal">Stay Here</button><button type="button" class="qa-btn primary" id="leavePageButton">Leave Page</button></div></div></div></div>
...
var meta={currency:{symbol:'',symbol_position:'before',decimal_places:2},requests:[],catalog:[],products:[],clients:[],locations:[]},cart=[],dirty=false,submitting=false,timer=null,pendingUrl='';
...
function addManual(){cart.push({product_service_id:null,product_id:null,item_type:'manual',item_name:'Other Charge',description:'',quantity:1,unit_cost:0,unit_price:0,discount_amount:0,tax_percent:0,is_optional:0});dirty=true;render()}
...
function updateProductCreateButton(){var v=document.getElementById('productSearch').value.trim(),exists=findProduct(v);document.getElementById('openProductModal').classList.toggle('show',v!==''&&!exists)}
function addProduct(){var name=document.getElementById('productSearch').value.trim(),qty=Math.max(.001,Number(document.getElementById('productQty').value||1));if(!name){notify('warning','Select or enter a product name.');return}var x=findProduct(name);if(!x){document.getElementById('newProductName').value=name;productModal.show();return}cart.push({product_service_id:null,product_id:Number(x.id),item_type:'product',item_name:x.name,description:x.description||'',quantity:qty,unit_cost:Number(x.base_unit_price||0),unit_price:Number(x.selling_price||0),discount_amount:0,tax_percent:Number(x.tax_percent||0),is_optional:0});dirty=true;render();document.getElementById('productSearch').value='';document.getElementById('productQty').value='1';updateProductCreateButton()}
function refreshProductDatalist(){var h='';(meta.products||[]).forEach(function(x){h+='<option value="'+esc(x.name)+'">'+esc(x.name)+' · '+money(x.selling_price)+'</option>'});document.getElementById('productOptions').innerHTML=h}
function createProduct(){var name=document.getElementById('newProductName').value.trim();if(!name){notify('warning','Product name is required.');return}var fd=new FormData();fd.append('action','create_product');fd.append('name',name);fd.append('base_unit_price',document.getElementById('newProductBasePrice').value||0);fd.append('markup_type',document.getElementById('newProductMarkupType').value);fd.append('markup_value',document.getElementById('newProductMarkupValue').value||0);fd.append('tax_percent',document.getElementById('newProductTax').value||0);fd.append('description',document.getElementById('newProductDescription').value||'');var btn=document.getElementById('createProductButton');btn.disabled=true;btn.classList.add('loading');req(fd).then(function(d){meta.products.push(d.product);refreshProductDatalist();document.getElementById('productSearch').value=d.product.name;productModal.hide();updateProductCreateButton();notify('success','Product created. Enter quantity and click Add to Cart.')}).catch(function(e){notify('error',e.message)}).finally(function(){btn.disabled=false;btn.classList.remove('loading')})}
function setMeta(m){meta=m||meta;var rh='<option value="">Direct Quotation - No Enquiry</option>';(meta.requests||[]).forEach(function(r){var key=String(r.source_key||('request:'+r.id)),label=r.request_no+' · '+r.client_name+' · '+(r.source_type==='revisit'?(r.source_label+' · '+(r.visit_date||'')):'Original Enquiry')+' · '+r.title;rh+='<option value="'+esc(key)+'">'+esc(label)+'</option>'});$('#requestId').html(rh);var ch='<option value="">Search service / material / fee / discount</option>';(meta.catalog||[]).filter(function(x){return x.item_type!=='product'}).forEach(function(x){ch+='<option value="'+Number(x.id)+'">'+esc(x.name)+' · '+esc(x.item_type)+' · '+money(x.unit_price)+'</option>'});$('#catalogItem').html(ch);var clients='<option value="">Select Customer</option>';(meta.clients||[]).forEach(function(x){clients+='<option value="'+Number(x.id)+'">'+esc(x.display_name)+(x.email?' · '+esc(x.email):'')+'</option>'});document.getElementById('clientId').innerHTML=clients;refreshProductDatalist();document.getElementById('currencyBadge').textContent=(meta.currency.currency_code||'')+' '+(meta.currency.symbol||'');$('#requestId,#catalogItem,#clientId,#locationId').select2({width:'100%'});if(quoteId>0)loadQuote();else{showRequest('');if(preRequestId>0){var desired=preRevisitId>0?('revisit:'+preRequestId+':'+preRevisitId):('request:'+preRequestId);if((meta.requests||[]).some(function(x){return String(x.source_key)===desired})){$('#requestId').val(desired).trigger('change')}}render()}}
...
function loadQuote(){var fd=new FormData();fd.append('action','get');fd.append('quote_id',quoteId);req(fd).then(function(d){var q=d.quotation||{};cart=d.items||[];var key=q.source_key||'';$('#requestId').val(String(key)).trigger('change.select2');document.getElementById('quoteTitle').value=q.title||'';document.getElementById('quoteStatus').value=q.status||'sent';document.getElementById('validUntil').value=q.valid_until||'';document.getElementById('introduction').value=q.introduction||'';document.getElementById('clientId').value=String(q.client_id||'');setLocations(q.client_id,q.location_id);$('#clientId,#locationId').trigger('change.select2');showRequest(key);render();dirty=false}).catch(function(e){notify('error',e.message)})}
$('#requestId').on('change',function(){showRequest(this.value);dirty=true});$('#clientId').on('change',function(){setLocations(this.value,'');$('#locationId').trigger('change.select2');dirty=true});
if(document.getElementById('addCatalog'))document.getElementById('addCatalog').onclick=addCatalog;if(document.getElementById('addManual'))document.getElementById('addManual').onclick=addManual;if(document.getElementById('addProduct'))document.getElementById('addProduct').onclick=addProduct;if(document.getElementById('createProductButton'))document.getElementById('createProductButton').onclick=createProduct;if(document.getElementById('openProductModal'))document.getElementById('openProductModal').onclick=function(){document.getElementById('newProductName').value=document.getElementById('productSearch').value.trim();productModal.show()};if(document.getElementById('productSearch'))document.getElementById('productSearch').addEventListener('input',updateProductCreateButton);
itemRows.addEventListener('input',function(e){var tr=e.target.closest('tr[data-i]');if(!tr)return;var i=Number(tr.dataset.i),f=e.target.dataset.f;if(!f||!cart[i]||f==='is_optional')return;if(['quantity','unit_cost','unit_price','discount_amount','tax_percent'].indexOf(f)>=0)cart[i][f]=Number(e.target.value||0);else cart[i][f]=e.target.value;dirty=true;refreshTotals(i)});itemRows.addEventListener('change',function(e){var tr=e.target.closest('tr[data-i]');if(!tr)return;var i=Number(tr.dataset.i),f=e.target.dataset.f;if(!f||!cart[i])return;if(f==='is_optional'){cart[i].is_optional=e.target.checked?1:0;dirty=true;refreshTotals(i)}});itemRows.addEventListener('click',function(e){var b=e.target.closest('[data-remove]');if(!b)return;cart.splice(Number(b.dataset.remove),1);dirty=true;render()});form.addEventListener('input',function(e){if(!e.target.closest('#itemRows'))dirty=true});form.addEventListener('change',function(e){if(!e.target.closest('#itemRows'))dirty=true});
form.addEventListener('submit',function(e){e.preventDefault();if(viewOnly)return;if(!form.reportValidity()){notify('warning','Complete the required quotation fields.');return}if(!document.getElementById('realRequestId').value&&!document.getElementById('clientId').value){notify('warning','Select a customer for the direct quotation.');return}if(!cart.length){notify('warning','Add at least one quotation item.');return}refreshTotals();var fd=new FormData(form);fd.append('action','save');var btn=document.getElementById('saveButton');submitting=true;btn.disabled=true;btn.classList.add('loading');req(fd).then(function(d){dirty=false;var msg=d.message||'Quotation saved successfully.';if(d.email_notice)msg+=' '+d.email_notice;notify(d.email_status==='failed'?'warning':'success',msg);setTimeout(function(){window.location.href='quotes.php'},1000)}).catch(function(er){notify('error',er.message)}).finally(function(){submitting=false;btn.disabled=false;btn.classList.remove('loading')})});
// Bootstrap unsaved-changes confirmation for links/buttons. No window.alert/confirm and no native beforeunload alert.
