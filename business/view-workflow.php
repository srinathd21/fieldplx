...
<head>
    ...
    <title>View Workflow - FieldPlx</title>
...
.wfv-summary{padding:15px;display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.wfv-info{padding:10px;border:1px solid #e4e9ef;border-radius:8px;background:#fbfcfd}.wfv-info small{display:block;color:#8a96a7;font-size:8px}.wfv-info strong{display:block;margin-top:4px;color:#34465f;font-size:10px}.wfv-steps{padding:14px;display:grid;gap:10px}.wfv-step{display:grid;grid-template-columns:38px minmax(0,1fr);gap:10px}.wfv-num{width:38px;height:38px;display:grid;place-items:center;border-radius:9px;background:#123f73;color:#fff;font-size:11px;font-weight:700}.wfv-step-card{border:1px solid #e0e6ed;border-radius:9px;overflow:hidden}.wfv-step-head{padding:10px 11px;background:#f8fafc;border-bottom:1px solid #e9edf2}.wfv-step-head strong{display:block;color:#263750;font-size:10.5px}.wfv-step-head small{display:block;margin-top:3px;color:#8a96a7;font-size:8px}.wfv-fields{padding:9px;display:grid;gap:6px}.wfv-field{padding:8px 9px;display:flex;align-items:center;justify-content:space-between;gap:10px;border:1px solid #edf0f4;border-radius:7px}.wfv-field strong{font-size:9px;color:#42536c}.wfv-field small{color:#8a96a7;font-size:8px}.wfv-type{padding:4px 6px;border-radius:5px;background:#edf4e4;color:#5d971b;font-size:7.8px;font-weight:700}@media(max-width:767px){.wfv-summary{grid-template-columns:repeat(2,1fr)}}</style></head><body>
<?php require_once __DIR__ . '/includes/nav.php'; ?>
<div class="fieldplx-main-layout">
<?php require_once __DIR__ . '/includes/sidebar.php'; ?>
<main class="fieldplx-main-content"><div class="fieldplx-content-wrapper"><div class="fd-dashboard"><section class="wf-head"><div><h1 id="pageTitle">Workflow</h1><p id="pageSubtitle">Preview the exact prebuilt technician work process.</p></div><div class="wf-actions"><a class="wf-btn" href="workflows.php"><i class="bi bi-arrow-left"></i> Back</a><a class="wf-btn primary" id="editLink" href="#"><i class="bi bi-pencil"></i> Edit Workflow</a></div></section><section class="wf-card"><div class="wfv-summary"><div class="wfv-info"><small>Service</small><strong id="serviceName">-</strong></div><div class="wfv-info"><small>Status</small><strong id="statusText">-</strong></div><div class="wfv-info"><small>Steps</small><strong id="stepCount">0</strong></div><div class="wfv-info"><small>Fields</small><strong id="fieldCount">0</strong></div></div><div class="wfv-steps" id="preview"><div class="wf-empty">Loading workflow...</div></div></section><div class="wf-toast info" id="toast"><span id="toastMessage">Notification</span><button id="toastClose"><i class="bi bi-x"></i></button></div><script>(function(){var csrf=<?= json_encode($workflowsCsrfToken) ?>,id=<?= (int)$workflowId ?>,toast=document.getElementById('toast'),msg=document.getElementById('toastMessage');function esc(v){return String(v==null?'':v).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#039;')}function note(m){toast.className='wf-toast error show';msg.textContent=m;setTimeout(function(){toast.classList.remove('show')},3000)}var fd=new FormData();fd.append('action','builder_get');fd.append('workflow_id',id);fd.append('csrf_token',csrf);fetch('api/workflows.php',{method:'POST',body:fd,credentials:'same-origin',headers:{'Accept':'application/json'}}).then(function(r){return r.json().then(function(d){if(!r.ok||!d.success)throw new Error(d.message||'Request failed');return d})}).then(function(d){var w=d.workflow||{},steps=d.steps||[];document.getElementById('pageTitle').textContent=w.name||'Workflow';document.getElementById('pageSubtitle').textContent=(w.description||'Prebuilt technician work process for selected service.');document.getElementById('serviceName').textContent=w.service_name||'-';document.getElementById('statusText').textContent=w.status||'-';document.getElementById('stepCount').textContent=steps.length;var fc=0,html='';steps.forEach(function(s,i){fc+=(s.fields||[]).length;html+='<div class="wfv-step"><div class="wfv-num">'+(i+1)+'</div><div class="wfv-step-card"><div class="wfv-step-head"><strong>'+esc(s.step_name)+(Number(s.required)===1?' *':'')+'</strong><small>'+esc(s.description||'')+'</small></div><div class="wfv-fields">';(s.fields||[]).forEach(function(f){html+='<div class="wfv-field"><div><strong>'+esc(f.label||f.field_key||'Field')+(Number(f.is_required)===1?' *':'')+'</strong><br><small>'+esc(f.help_text||'')+'</small></div><span class="wfv-type">'+esc(String(f.field_type).replace(/_/g,' '))+'</span></div>'});if(!(s.fields||[]).length)html+='<div class="wf-empty">No fields configured.</div>';html+='</div></div></div>'});document.getElementById('fieldCount').textContent=fc;document.getElementById('preview').innerHTML=html||'<div class="wf-empty">No process steps configured.</div>';document.getElementById('editLink').href='edit-workflow.php?id='+id}).catch(function(e){note(e.message)});document.getElementById('toastClose').onclick=function(){toast.classList.remove('show')}})();</script></div></div></main></div>
