      ...
      <div class="fieldplx-content-wrapper">
        <div class="fd-dashboard">
          <section class="fq-head">
            <div>
              <h1 class="fq-title">Quotes</h1>
              <p class="fq-sub">Create and manage customer quotations linked to service enquiries. Amounts and symbols
            ...
            <div class="fq-actions"><button type="button" class="fq-btn" id="refreshButton"><i
                ...
                href="add-quotation"><i class="bi bi-plus-lg"></i> Add Quotes</a><?php endif; ?></div>
          </section>
          ...
          <section class="fd-card fq-card">
            <div class="fq-toolbar">
              <div class="fq-search"><i class="bi bi-search"></i><input type="search" id="search"
                  placeholder="Search quote, enquiry or customer"></div>
              <select class="fq-filter" id="statusFilter">
              ...
              <input class="fq-filter" type="date" id="fromDate" title="From Date"><input class="fq-filter" type="date"
                id="toDate" title="To Date">
              <button type="button" class="fq-btn" id="clearFilters"><i class="bi bi-x-circle"></i> Clear</button><span
                class="fq-spacer"></span>
              <select class="fq-filter" id="perPage">
            ...
            <div class="fq-pagination"><span id="countText">Showing 0 quotations</span>
  ...
  <script>
    (function () {
      ...
      function notify(type, message) {
        ...
        toast.className = 'fq-toast ' + (type || 'info') + ' show';
        toastMsg.textContent = message || 'Notification';
      ...
      function money(value, currency) {
        ...
        var formatted = amount.toFixed(places);
      ...
      function renderRows(rows, pagination, currency) {
        ...
        rows.forEach(function (row, index) {
          var quoteId = Number(row.id || 0);
          var editable = ['draft', 'internal_approval', 'changes_requested'].indexOf(row.status) >= 0;
          var sourceTitle = row.quotation_source ||
            (row.request_no ? 'Original Enquiry' : 'Direct Quotes');
          ...
          html.push(
            '<tr class="fq-clickable-row" data-quote-id="' + quoteId + '" tabindex="0" role="link" aria-label="Open quotation ' + esc(row.quote_no || quoteId) + '">' +
              ...
              '<td>' +
                '<div class="fq-main">' +
                  '<strong>' + esc(row.quote_no || '-') + '</strong>' +
                  '<small>Revision ' + Number(row.revision_no || 0) + '</small>' +
                '</div>' +
              '</td>' +
              '<td>' +
                '<div class="fq-main">' +
                  '<strong>' + esc(sourceTitle) + '</strong>' +
                  '<small>' + esc(row.request_no || 'Direct') + '</small>' +
                '</div>' +
              '</td>' +
              '<td>' +
                '<div class="fq-client">' +
                  '<strong>' + esc(row.client_name || '-') + '</strong>' +
                  '<small>' + esc(row.client_phone || row.client_email || '') + '</small>' +
                '</div>' +
              '</td>' +
              '<td>' + esc(row.title || '-') + '</td>' +
              '<td>' + esc(formatDate(row.created_date || row.created_at)) + '</td>' +
              '<td>' + esc(formatDate(row.valid_until)) + '</td>' +
              '<td>' + esc(money(row.subtotal, currency)) + '</td>' +
              '<td>' + esc(money(row.tax_total, currency)) + '</td>' +
              '<td><strong>' + esc(money(row.total, currency)) + '</strong></td>' +
      ...
      function loadQuotations() {
        ...
        if (!body) {
          console.error('FieldPlx quotations: #quoteRows was not found.');
          return;
        }
        ...
        formData.append('from_date', state.from);
        formData.append('to_date', state.to);

        body.innerHTML =
          '<tr><td colspan="12" class="fq-empty">Loading quotations...</td></tr>';
        ...
        request(formData)
          .then(function (data) {
            ...
            if (next) {
              next.disabled = state.page >= Number(pagination.pages || 1);
            }
          })
          .catch(function (error) {
            ...
            body.innerHTML =
              ...
              esc(error.message || 'Unable to load quotations.') +
              '</td></tr>';

            notify('error', error.message || 'Unable to load quotations.');
          });
      }
      ...
      function init() {
        ...
        var perPage = el('perPage');
        var clearFilters = el('clearFilters');
        var refreshButton = el('refreshButton');
        var prevPage = el('prevPage');
        var nextPage = el('nextPage');
        ...
        if (perPage) {
          perPage.addEventListener('change', function () {
            state.perPage = Number(perPage.value || 10);
            state.page = 1;
            loadQuotations();
          });
        }
