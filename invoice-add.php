<?php
/**
 * FieldPlx - Add Invoice
 *
 * Upload as:
 * /public_html/invoice-add.php
 *
...
$pageTitle = 'Add Invoice - FieldPlx';
$activePage = 'invoice-add';
...
if (!function_exists('invoiceAddFetchAssoc')) {
    function invoiceAddFetchAssoc(mysqli_stmt $stmt)
    {
...
if (!function_exists('invoiceAddFetchAll')) {
    function invoiceAddFetchAll(mysqli_stmt $stmt)
    {
        $rows = array();
...
if (!function_exists('invoiceAddOld')) {
    function invoiceAddOld($key, $default = '')
    {
        if (
            isset($_POST[$key]) &&
            !is_array($_POST[$key])
...
if (!function_exists('invoiceAddNullable')) {
    function invoiceAddNullable($value)
    {
...
if (!function_exists('invoiceAddCsrfToken')) {
    function invoiceAddCsrfToken()
    {
...
if (!function_exists('invoiceAddVerifyCsrf')) {
    function invoiceAddVerifyCsrf($token)
    {
...
if (!function_exists('invoiceAddMoney')) {
    function invoiceAddMoney($amount, $currencyCode)
    {
...
if (!function_exists('invoiceAddSequencePeriod')) {
    function invoiceAddSequencePeriod($frequency)
    {
        if ($frequency === 'yearly') {
            return date('Y');
        }
...
if (!function_exists('invoiceAddGenerateNumber')) {
    function invoiceAddGenerateNumber(
        mysqli $conn,
        $tenantId
    ) {
        /*
...
if (!function_exists('invoiceAddLogActivity')) {
    function invoiceAddLogActivity(
        mysqli $conn,
        $tenantId,
        $userId,
        $invoiceId,
...
if (!function_exists('invoiceAddLineCalculation')) {
    function invoiceAddLineCalculation(
        $quantity,
        $unitPrice,
        $discountAmount,
        $taxRate,
...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    foreach (
        ...
        if (
            isset($_GET[$relationField]) &&
            (int) $_GET[$relationField] > 0
        ) {
            $_POST[$relationField] =
                (string) (int) $_GET[
                    $relationField
    ...
    /*
     * Resolve related client/property from the preselected source.
     */
    $preselectedQuoteId =
        (int) invoiceAddOld(
            'quote_id'
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    /*
     * Persist the invoice and all line items atomically.
     */
    if (empty($errors)) {
        try {
            ...
            if (!$stmt) {
                throw new Exception(
                    'Unable to prepare the invoice save operation: ' .
                    $conn->error
                );
            }

            $stmt->bind_param(
                'isiiiiisssddddddsssi',
                $tenantId,
                $invoiceNo,
                $clientId,
                $propertyId,
            ...
            if (!$stmt) {
                throw new Exception(
                    'Unable to prepare invoice line items: ' .
                    $conn->error
                );
            }
            ...
            foreach ($lineItems as $item) {
                ...
                if (!$stmt->execute()) {
                    throw new Exception(
                        'An invoice item could not be saved: ' .
                        $stmt->error
                    );
                }
            }
            ...
            /*
             * A sent invoice can close the "ready to invoice" stage.
             * Existing unrelated job states are not overwritten.
             */
            if (
                $jobId !== null &&
                in_array(
    ...
    <div class="ia-header">
        <div class="ia-header-main">
            ...
            <div>
                <h1>Add Invoice</h1>
                <p>
                    Create a tenant-safe invoice with line items, discounts, taxes, and related records.
                </p>
            </div>
        </div>
                ...
                <section class="ia-card">
                    <div class="ia-card-head">
                        <div>
                            ...
                            <p>
                                Select the customer, related records, dates, and invoice status.
                            </p>
                        </div>
                    </div>
                ...
                <section class="ia-card">
                    <div class="ia-card-head">
                        <div>
                            <h2>Invoice Items</h2>
                            <p>
                                Add custom items or load reusable items from the selected job or quote.
                            </p>
                        </div>
                ...
                <section class="ia-card">
                    <div class="ia-card-head">
                        <div>
                            ...
                            <p>
                                Totals are recalculated again by the server before saving.
                            </p>
                        </div>
                    </div>
