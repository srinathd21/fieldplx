<?php
/**
 * FieldPlx Mobile Jobs API
 * Upload: /business/api/mobile/jobs.php
 * Production: https://fieldplx.com/business/api/mobile/jobs.php
 * Authentication: Authorization: Bearer <access_token>
 * Supports GET list/details/meta, POST create/actions, PUT/PATCH update, and DELETE cancellation.
 */
/* FieldPlx Jobs API v2.9 - Create Job, visits, billing, line items, custom fields, discounts and notifications */
...
if (!defined('FIELDPLX_API_SECRET') || trim((string) FIELDPLX_API_SECRET) === '') {
    ...
    echo json_encode(array(
        ...
        'error' => array('code' => 'api_secret_not_configured')
...
function jbMobileBase64UrlDecode($data)
{
    $data = (string)$data;
...
function jbMobileAuthorizationHeader()
{
    ...
    if (function_exists('getallheaders')) {
        ...
        if (is_array($headers)) {
            foreach ($headers as $name => $value) {
                if (strtolower((string)$name) === 'authorization') return trim((string)$value);
            }
        }
    }
    return '';
...
function jbMobileVerifyToken()
{
    $header = jbMobileAuthorizationHeader();
    if ($header === '' || !preg_match('/^Bearer\s+(.+)$/i', $header, $m)) {
        jbRes(401, false, 'Authorization Bearer token is required.', array('error_code' => 'authorization_required'));
    }
    $token = trim((string)$m[1]);
    $parts = explode('.', $token);
    if (count($parts) !== 3) jbRes(401, false, 'Invalid access token.', array('error_code' => 'invalid_token'));

    $headerJson = jbMobileBase64UrlDecode($parts[0]);
    $payloadJson = jbMobileBase64UrlDecode($parts[1]);
    $signature = jbMobileBase64UrlDecode($parts[2]);
    if ($headerJson === false || $payloadJson === false || $signature === false) {
        jbRes(401, false, 'Invalid access token.', array('error_code' => 'invalid_token'));
    }
    $jwtHeader = json_decode($headerJson, true);
    $payload = json_decode($payloadJson, true);
    if (!is_array($jwtHeader) || !is_array($payload) || ($jwtHeader['alg'] ?? '') !== 'HS256') {
        jbRes(401, false, 'Invalid access token.', array('error_code' => 'invalid_token'));
    }
    $expected = hash_hmac('sha256', $parts[0] . '.' . $parts[1], FIELDPLX_API_SECRET, true);
    if (!hash_equals($expected, $signature)) {
        jbRes(401, false, 'Invalid access token.', array('error_code' => 'invalid_token_signature'));
    }
    $now = time();
    if (isset($payload['nbf']) && (int)$payload['nbf'] > $now) {
        jbRes(401, false, 'Access token is not active yet.', array('error_code' => 'token_not_active'));
    }
    if (!isset($payload['exp']) || (int)$payload['exp'] <= $now) {
        jbRes(401, false, 'Access token has expired. Please sign in again.', array('error_code' => 'token_expired'));
    }
    if (isset($payload['iss']) && (string)$payload['iss'] !== (string)FIELDPLX_TOKEN_ISSUER) {
        jbRes(401, false, 'Invalid access token.', array('error_code' => 'invalid_token_issuer'));
    }
    if (isset($payload['aud']) && (string)$payload['aud'] !== (string)FIELDPLX_TOKEN_AUDIENCE) {
        jbRes(401, false, 'Invalid access token.', array('error_code' => 'invalid_token_audience'));
    }
    if (empty($payload['user_id']) || empty($payload['tenant_id'])) {
        jbRes(401, false, 'Invalid access token.', array('error_code' => 'invalid_token_context'));
    }
    return $payload;
}

function jbMobileAuthContext(PDO $pdo)
{
    $claims = jbMobileVerifyToken();
    $stmt = $pdo->prepare("SELECT u.id AS user_id,u.tenant_id,u.branch_id,u.department_id,u.role_id,u.status AS user_status,t.status AS tenant_status,b.status AS branch_status,d.status AS department_status,r.status AS role_status FROM users u INNER JOIN tenants t ON t.id=u.tenant_id AND t.deleted_at IS NULL LEFT JOIN branches b ON b.id=u.branch_id AND b.tenant_id=u.tenant_id LEFT JOIN departments d ON d.id=u.department_id AND d.tenant_id=u.tenant_id LEFT JOIN roles r ON r.id=u.role_id AND r.tenant_id=u.tenant_id WHERE u.id=:u AND u.tenant_id=:t AND u.deleted_at IS NULL LIMIT 1");
    $stmt->execute(array(':u'=>(int)$claims['user_id'], ':t'=>(int)$claims['tenant_id']));
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$row) jbRes(401, false, 'Authenticated user account was not found.', array('error_code'=>'user_not_found'));
    if ((string)$row['user_status'] !== 'active') jbRes(403, false, 'Your user account is not active.', array('error_code'=>'user_not_active'));
    if (!in_array((string)$row['tenant_status'], array('trial','active'), true)) jbRes(403, false, 'This business account is not active.', array('error_code'=>'tenant_not_active'));
    if (!empty($row['branch_id']) && !empty($row['branch_status']) && (string)$row['branch_status'] !== 'active') jbRes(403, false, 'Your assigned branch is not active.', array('error_code'=>'branch_not_active'));
    if (!empty($row['department_id']) && !empty($row['department_status']) && (string)$row['department_status'] !== 'active') jbRes(403, false, 'Your assigned department is not active.', array('error_code'=>'department_not_active'));
    if (!empty($row['role_id']) && !empty($row['role_status']) && (string)$row['role_status'] !== 'active') jbRes(403, false, 'Your assigned role is not active.', array('error_code'=>'role_not_active'));
    return $row;
}

function jbMobilePrepareInput()
{
    $method = strtoupper((string)($_SERVER['REQUEST_METHOD'] ?? 'GET'));
    $contentType = strtolower((string)($_SERVER['CONTENT_TYPE'] ?? ''));

    if (in_array($method, array('POST','PUT','PATCH','DELETE'), true) && strpos($contentType, 'application/json') !== false) {
        $raw = file_get_contents('php://input');
        if ($raw !== false && trim($raw) !== '') {
            $decoded = json_decode($raw, true);
            if (!is_array($decoded)) jbRes(400, false, 'Invalid JSON request body.', array('error_code'=>'invalid_json'));
            foreach ($decoded as $k => $v) $_POST[$k] = $v;
        }
    }

    /* These backend fields are stored/parsed as JSON strings by the existing Jobs logic. */
    foreach (array('schedule_json','line_items_json','custom_fields_json','note_mentions_json','new_checklist_json','contacts_json','custom_values_json','options_json','payment_schedule_json') as $key) {
        if (isset($_POST[$key]) && is_array($_POST[$key])) {
            $_POST[$key] = json_encode($_POST[$key], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        }
    }

    /* Android may send array values directly. */
    foreach (array('user_ids','checklist_template_ids') as $key) {
        if (isset($_POST[$key]) && !is_array($_POST[$key])) $_POST[$key] = array($_POST[$key]);
    }

    if ($method === 'GET') {
        foreach ($_GET as $k => $v) $_POST[$k] = $v;
        if (isset($_GET['meta']) && (int)$_GET['meta'] === 1) $_POST['action'] = 'meta';
        elseif (isset($_GET['preview']) && (int)$_GET['preview'] === 1) $_POST['action'] = 'job_no_preview';
        elseif (isset($_GET['quote_id']) && isset($_GET['quote_details'])) $_POST['action'] = 'quote_details';
        elseif (isset($_GET['request_id']) && isset($_GET['request_details'])) $_POST['action'] = 'request_details';
        elseif (isset($_GET['id']) && (int)$_GET['id'] > 0) { $_POST['action'] = 'get'; $_POST['job_id'] = (int)$_GET['id']; }
        elseif (empty($_POST['action'])) $_POST['action'] = 'list';
    } elseif ($method === 'POST') {
        if (empty($_POST['action'])) $_POST['action'] = 'save';
    } elseif ($method === 'PUT' || $method === 'PATCH') {
        $_POST['action'] = 'save';
        if (isset($_GET['id']) && (int)$_GET['id'] > 0) $_POST['job_id'] = (int)$_GET['id'];
...
jbMobilePrepareInput();
$mobileAuth = jbMobileAuthContext($pdo);
$tenant = (int)$mobileAuth['tenant_id'];
$user = (int)$mobileAuth['user_id'];
$sessionBranch = !empty($mobileAuth['branch_id']) ? (int)$mobileAuth['branch_id'] : 0;
