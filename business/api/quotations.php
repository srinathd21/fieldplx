...
{
    /* Deliberately avoid branch/currency joins here. FieldPlx installations
       have used more than one currency schema; form loading must never fail
       just because a currency lookup differs. */
    try {
        if (qlColumnExists($pdo, 'tenants', 'currency_code')) {
            $st = $pdo->prepare("SELECT currency_code FROM tenants WHERE id = :tenant_id LIMIT 1");
            $st->execute(array(':tenant_id' => (int)$tenantId));
            $code = strtoupper(trim((string)$st->fetchColumn()));
            if ($code !== '') {
                $symbols = array('INR'=>'₹','USD'=>'$','EUR'=>'€','GBP'=>'£','AED'=>'د.إ','SAR'=>'ر.س');
                return array(
                    'id'=>null,
                    'currency_code'=>$code,
                    'currency_name'=>$code,
        ...
        error_log('FieldPlx quotation currency lookup: ' . $e->getMessage());
    }
...
function qxSafeFetchAll(PDO $pdo, $sql, array $params = array(), $context = '')
{
    try {
        $st = $pdo->prepare($sql);
        $st->execute($params);
        ...
        error_log('FieldPlx quotation optional query' . ($context !== '' ? ' ['.$context.']' : '') . ': ' . $e->getMessage());
        return array();
    }
}

function qxSafeFetchOne(PDO $pdo, $sql, array $params = array(), $context = '')
{
    $rows = qxSafeFetchAll($pdo, $sql, $params, $context);
...
function qxTaxIdByPercent(PDO $pdo, $tenantId, $percent)
{
    ...
    $sql = "SELECT id FROM tax_rates WHERE ABS(`$rateCol`-:r)<0.0001";
    $params = array(':r'=>$percent);
    if (isset($cols['tenant_id'])) { $sql .= ' AND tenant_id=:t'; $params[':t']=(int)$tenantId; }
    ...
    try {
        ...
        error_log('FieldPlx quotation tax lookup: '.$e->getMessage());
        return null;
    }
}
...
if ($action === 'form_meta') {
    ...
    if (qxTableExists($pdo,'clients')) {
        ...
        foreach(qxSafeFetchAll($pdo,$sql,array(':t'=>$tenantId),'clients') as $r){$r['name']=isset($r['display_name'])?$r['display_name']:(isset($r['name'])?$r['name']:'Client');$clients[]=$r;}
    }

    $locations=array(); $lt=qxLocationTable($pdo);
    if($lt!==''){
        $lcols=qxColumns($pdo,$lt); $sql="SELECT * FROM `$lt`"; $lparams=array(); if(isset($lcols['tenant_id'])){$sql.=' WHERE tenant_id=:t';$lparams[':t']=$tenantId;} else {$sql.=' WHERE 1=0';}
        if(qlColumnExists($pdo,$lt,'deleted_at'))$sql.=' AND deleted_at IS NULL';
        if(qlColumnExists($pdo,$lt,'status'))$sql.=" AND status <> 'archived'";
        $order=array(); if(qlColumnExists($pdo,$lt,'client_id'))$order[]='client_id'; if(qlColumnExists($pdo,$lt,'is_primary'))$order[]='is_primary DESC'; $order[]='id'; $sql.=' ORDER BY '.implode(',', $order);
        foreach(qxSafeFetchAll($pdo,$sql,$lparams,'locations') as $r){if(!isset($r['location_type']))$r['location_type']='site';$locations[]=$r;}
    }
    ...
    if(qxTableExists($pdo,'product_services')){
        ...
        $rows=qxSafeFetchAll($pdo,$sql,array(':t'=>$tenantId),'catalog');
        $taxRows=qxTaxRows($pdo,$tenantId);$taxMap=array();foreach($taxRows as $tr)$taxMap[(int)$tr['id']]=$tr;
        foreach($rows as $r){$tid=(int)(isset($r['tax_rate_id'])?$r['tax_rate_id']:0);$r['tax_percent']=$tid&&isset($taxMap[$tid])?(float)$taxMap[$tid]['rate_percent']:0;$catalog[]=$r;}
    }
    ...
    if(qxTableExists($pdo,'users')){
        ...
        $sales=qxSafeFetchAll($pdo,$sql,array(':t'=>$tenantId),'salespersons');
    }
    ...
    if($rt!==''){
        ...
        foreach(qxSafeFetchAll($pdo,$sql,array(':t'=>$tenantId),'requests') as $r){$r['source_key']='request:'.(int)$r['id'];$r['source_label']='Original Enquiry';if(!isset($r['location_id'])&&isset($r['property_id']))$r['location_id']=$r['property_id'];$requests[]=$r;}
    }
...
qlResponse(400, false, 'Unsupported quotation form action.');
} catch (PDOException $e) {
    error_log('FieldPlx quotation form action ' . $action . ' PDO: ' . $e->getMessage());
    qlResponse(500, false, 'Quotation form request failed. Please check the PHP error log.', array('action'=>$action));
} catch (Throwable $e) {
    error_log('FieldPlx quotation form action ' . $action . ': ' . $e->getMessage());
    qlResponse(500, false, 'Quotation form request failed. Please check the PHP error log.', array('action'=>$action));
}
}
