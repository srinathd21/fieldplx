...
<section class="fd-ar-head">
  <div><h1 class="fd-ar-title">Add Service Request</h1><p class="fd-ar-sub">Capture the customer requirement first. Employee assignment is shown only when the selected stage requires Assessment or a direct Job. Quote Required opens the quotation cart automatically.</p></div>
...
<div class="fd-ar-card-head"><div><h3>Request Information</h3><p>Request, Quote and Job numbers are generated automatically from tenant Document Number Formatting.</p></div></div>
...
<div class="fd-rflow-note"><strong>Automatic flow:</strong> Assessment Required → employee selection becomes available. Quote Required → quote cart opens and a draft Quote is created automatically. Job Required → employee selection becomes available and a Job is generated automatically and linked to the request. Other stages do not ask for employees.</div>

</div></div>
<div class="fd-ar-footer"><a href="requests.php" class="fd-ar-btn">Cancel</a><button type="submit" class="fd-ar-btn primary" id="saveButton"><span class="fd-ar-loader"></span><i class="bi bi-check-lg"></i><span id="saveText">Create Request</span></button></div>
</form>
</div>
...
$('#clientId').on('change',function(){loadLocations(this.value)});$('#teamId').on('change',function(){teamPreview(this.value)});$('#status').on('change',updateStage);document.querySelectorAll('input[name="assignment_mode"]').forEach(function(x){x.addEventListener('change',updateAssignmentMode)});
...
form.addEventListener('submit',function(e){e.preventDefault();if(!form.reportValidity()){notify('warning','Complete the required request fields.');return}var from=document.getElementById('preferredFrom').value,to=document.getElementById('preferredTo').value;if(from&&to&&from>=to){notify('warning','Preferred end time must be after start time.');return}if(!validateAssignment())return;var s=document.getElementById('status').value;if(s==='quote_required'&&!cart.length){notify('warning','Add at least one quotation item.');return}renderCart();var fd=new FormData(form);fd.append('action','create');submitting=true;loading(true);req(fd).then(function(d){dirty=false;var msg=d.message||'Service request created.';if(d.quote_no)msg+=' Quote: '+d.quote_no+'.';if(d.job_no)msg+=' Job: '+d.job_no+'.';notify('success',msg);setTimeout(function(){window.location.href='requests.php'},1100)}).catch(function(er){submitting=false;notify('error',er.message)}).finally(function(){loading(false)})});
