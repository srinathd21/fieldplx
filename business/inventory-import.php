<?php
/* FieldPlx Inventory Import - Version 1.0.0 - 2026-09-03 */
require_once __DIR__ . '/includes/auth.php';

$pageTitle = 'Inventory Import';
...
$csrfToken = (string)$_SESSION['inventory_csrf_token'];
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <title>Inventory Import - FieldPlx</title>
      ...
      <div class="fieldplx-content-wrapper">
        <div class="fd-dashboard">
          <section class="fd-import-header">
            <div><h1 class="fd-import-title">Inventory Import</h1><p class="fd-import-subtitle">Bulk create or update branch inventory levels using a spreadsheet-style grid, Excel paste, or CSV upload.</p></div>
            <div class="fd-import-actions"><a class="fd-team-button" href="inventory.php" data-import-leave-safe="1"><i class="bi bi-arrow-left"></i> Back to Inventory</a><button class="fd-team-button" type="button" id="sampleButton"><i class="bi bi-filetype-csv"></i> Sample CSV</button><button class="fd-team-button" type="button" id="exportGridButton"><i class="bi bi-download"></i> Export Grid CSV</button></div>
          </section>

          <div class="fd-import-notice"><i class="bi bi-info-circle"></i><div><strong>Inventory import:</strong> identify an existing tracked product using SKU or exact Product Name, then enter the Branch Code / Name and On Hand quantity. Existing inventory rows are updated; new product/branch rows are created. Reserved quantity is never overwritten, and On Hand cannot be reduced below Reserved.</div></div>
          ...
          <section class="fd-import-card">
            <div class="fd-import-toolbar">
              ...
              <span class="fd-import-help">Tip: copy rows from Excel and paste directly into the first inventory cell.</span>
            </div>
            <div class="fd-import-grid-wrap">
              <table class="fd-import-grid">
                <thead><tr><th>#</th><th>Import Result</th><th>Product SKU</th><th>Product Name</th><th>Branch Code / Name *</th><th>Quantity On Hand *</th><th>Reorder Level</th><th>Minimum Stock</th><th>Notes</th><th>Action</th></tr></thead>
            ...
            <div class="fd-import-footer">
              ...
              <button class="fd-team-button primary" type="button" id="saveImportButton"><i class="bi bi-database-add"></i> Import Inventory</button>
            </div>
          </section>
          ...
          <div class="modal fade" id="leavePageModal" tabindex="-1" aria-labelledby="leavePageModalTitle" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
              <div class="modal-content" style="border:1px solid #dfe5ec;border-radius:12px;overflow:hidden;box-shadow:0 24px 65px rgba(0,17,49,.24)">
                <div class="modal-header" style="background:#fbfcfd;border-bottom:1px solid var(--fd-border)"><div><h5 class="modal-title" id="leavePageModalTitle" style="font-size:12px;color:#0b1933">Unsaved inventory import data</h5><div style="margin-top:3px;color:#6f7b90;font-size:8.5px">Your spreadsheet contains changes that may be lost.</div></div><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button></div>
                <div class="modal-body"><div class="fd-import-leave-copy"><strong>Are you sure you want to leave or refresh this page?</strong><br>Inventory rows that have not been imported remain only in this browser page.</div><div class="fd-import-leave-warning"><i class="bi bi-exclamation-triangle me-1"></i> Export the grid as CSV first if you want to keep a backup.</div></div>
...
var apiUrl=basePath+'/api/inventory.php';
var columns=[
 ['product_sku','Product SKU','text'],
 ['product_name','Product Name','text'],
 ['branch','Branch Code / Name *','text'],
...
function emptyRow(){return {product_sku:'',product_name:'',branch:'',quantity_on_hand:'0.000',reorder_level:'0.000',minimum_stock:'0.000',notes:''}}
function ensureRows(count){while(rows.length<count)rows.push(emptyRow())}
function cellClass(key){if(key==='notes')return ' wide';if(key==='product_name'||key==='branch')return ' medium';if(key==='quantity_on_hand'||key==='reorder_level'||key==='minimum_stock')return ' narrow';return ''}
function render(){var body=el('bulkGridBody'),h='';rows.forEach(function(r,ri){var rr=rowResults[ri+1]||null,status=rr?String(rr.status||''):'';var rowCls=rr?(status==='Imported'||status==='Updated'?' import-ok':' import-error'):'';h+='<tr class="'+rowCls+'" data-row="'+ri+'"><td class="fd-import-row-no">'+(ri+1)+'</td>';var badgeCls=rr?(status==='Imported'||status==='Updated'?'ok':'error'):'pending';h+='<td class="fd-import-grid-result"><span class="fd-import-row-state '+badgeCls+'">'+esc(status||'Not Checked')+'</span>'+(rr&&rr.message?'<small>'+esc(rr.message)+'</small>':'')+'</td>';columns.forEach(function(c,ci){var key=c[0],type=c[2],cls=cellClass(key);h+='<td><input class="fd-import-cell'+cls+'" type="'+(type==='number'?'number':'text')+'" '+(type==='number'?'step="0.001" min="0" ':'')+'data-row="'+ri+'" data-col="'+ci+'" data-key="'+key+'" value="'+esc(r[key])+'"></td>'});h+='<td><button type="button" class="fd-import-row-action" data-remove="'+ri+'" title="Remove row"><i class="bi bi-trash"></i></button></td></tr>'});body.innerHTML=h;updateCount()}
function rowReady(r){return (String(r.product_sku||'').trim()!==''||String(r.product_name||'').trim()!=='')&&String(r.branch||'').trim()!==''&&String(r.quantity_on_hand||'').trim()!==''}
function updateCount(){var valid=rows.filter(rowReady).length;el('rowCount').textContent=rows.length;el('readyCount').textContent=valid}
...
function gridData(){return rows.filter(function(r){return Object.keys(r).some(function(k){return String(r[k]||'').trim()!==''&&String(r[k])!=='0.000'})}).map(function(r){var out={};columns.forEach(function(c){out[c[1].replace(' *','')]=r[c[0]]||''});return out})}
function normalizeImported(data){var map={};columns.forEach(function(c){map[c[1].replace(' *','').toLowerCase()]=c[0];map[c[0].toLowerCase()]=c[0]});map['sku']='product_sku';map['product']='product_name';map['branch code']='branch';map['branch name']='branch';map['on hand']='quantity_on_hand';var out=[];(data||[]).forEach(function(src){var r=emptyRow(),has=false;Object.keys(src||{}).forEach(function(k){var key=map[String(k).trim().toLowerCase()];if(key){r[key]=String(src[k]==null?'':src[k]).trim();if(r[key]!=='')has=true}});if(has)out.push(r)});return out}
function showResult(d){var box=el('importResult'),list=el('importResultList'),h='';rowResults={};(d.results||[]).forEach(function(x){rowResults[Number(x.row||0)]=x;var ok=x.status==='Imported'||x.status==='Updated',cls=ok?'ok':'error',entered=x.input||{};h+='<div class="fd-import-result-row '+cls+'"><span>'+esc(x.status||'Done')+'</span><div class="fd-import-result-main"><strong>Row '+Number(x.row||0)+' · '+esc(entered.product_sku||entered.product_name||'Inventory row')+'</strong><small>'+esc(x.message||'')+'</small></div></div>'});list.innerHTML=h||'<div class="fd-import-result-row"><span>Done</span><div class="fd-import-result-main"><small>No row details returned.</small></div></div>';el('resultSummary').textContent=Number(d.imported||0)+' created · '+Number(d.updated||0)+' updated · '+Number(d.failed||0)+' failed';box.classList.add('show');render();isDirty=Number(d.failed||0)>0;box.scrollIntoView({behavior:'smooth',block:'start'})}
...
el('csvFile').onchange=function(e){var file=e.target.files&&e.target.files[0];if(!file)return;Papa.parse(file,{header:true,skipEmptyLines:true,complete:function(res){var imported=normalizeImported(res.data||[]);if(!imported.length){notify('warning','No matching inventory columns were found in the CSV.');return}rows=imported;rowResults={};ensureRows(Math.max(10,rows.length));markDirty();render();notify('success',imported.length+' CSV row(s) loaded into the grid.')}})};
el('exportGridButton').onclick=function(){var data=gridData();if(!data.length){notify('warning','Enter inventory data before exporting.');return}downloadText('inventory-import-grid-'+new Date().toISOString().slice(0,10)+'.csv',Papa.unparse(data))};
el('sampleButton').onclick=function(){var sample=[{'Product SKU':'PRD-001','Product Name':'Sample Product','Branch Code / Name':'B001','Quantity On Hand':'10.000','Reorder Level':'3.000','Minimum Stock':'1.000','Notes':'Opening stock import'}];downloadText('inventory-import-sample.csv',Papa.unparse(sample));notify('success','Sample CSV generated successfully.')};
el('saveImportButton').onclick=function(){var payload=rows.filter(rowReady);if(!payload.length){notify('warning','Enter Product SKU/Name, Branch and Quantity On Hand for at least one row.');return}var btn=this,old=btn.innerHTML,fd=new FormData();fd.append('action','import');fd.append('rows_json',JSON.stringify(payload));btn.disabled=true;btn.innerHTML='<span class="spinner-border spinner-border-sm"></span> Importing...';request(fd).then(function(d){notify('success',d.message);showResult(d)}).catch(function(e){notify('error',e.message)}).finally(function(){btn.disabled=false;btn.innerHTML=old})};
