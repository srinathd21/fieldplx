...
try {
    if ($action === 'list') {
        ...
        // Absolute tenant client count. Used by the page to distinguish a true
        // first-use empty state from a filter/search that simply has no matches.
        $allCount = $pdo->prepare("SELECT COUNT(*) FROM clients WHERE tenant_id = :tenant_id AND deleted_at IS NULL");
        $allCount->execute(array(':tenant_id' => $tenantId));
        $allClientsTotal = (int) $allCount->fetchColumn();
        ...
        cmResponse(200, true, 'Clients loaded.', array(
            'clients' => $rows,
            'all_clients_total' => $allClientsTotal,
