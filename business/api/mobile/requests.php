<?php
/* FieldPlx Mobile Request API - Version 4.0.0 - 2026-09-08
 * Standalone Android/mobile endpoint using Bearer authentication.
 * Keeps Platform SMTP customer confirmation + explicit team assignment email.
 *
 * Production URL:
 *   https://fieldplx.com/business/api/mobile/requests.php
 *
 * Android routes:
 *   GET  ?meta=1
 *   GET  ?locations=1&client_id=10
 *   GET  ?id=25
 *   POST create request (JSON or multipart/form-data)
 *   PUT/PATCH ?id=25 update request
 *   POST ?action=create_customer
 *   POST ?action=create_catalog_item
 */
ob_start();
...
if (!defined('FIELDPLX_TOKEN_ISSUER')) define('FIELDPLX_TOKEN_ISSUER', 'FieldPlx');
if (!defined('FIELDPLX_TOKEN_AUDIENCE')) define('FIELDPLX_TOKEN_AUDIENCE', 'FieldPlx-Mobile');
...
function rfBootstrapInput(){
    static $done=false;
    if($done)return;
    $done=true;
    if(!empty($_POST))return;
    $method=strtoupper((string)($_SERVER['REQUEST_METHOD']??'GET'));
    if(!in_array($method,array('POST','PUT','PATCH'),true))return;
    $raw=file_get_contents('php://input');
    if($raw===false||trim($raw)==='')return;
    $ct=strtolower((string)($_SERVER['CONTENT_TYPE']??''));
    if(strpos($ct,'application/json')!==false){
        $decoded=json_decode($raw,true);
        if(!is_array($decoded))rfRes(400,false,'Invalid JSON request body.');
        $_POST=$decoded;
        return;
    }
    parse_str($raw,$parsed);
    if(is_array($parsed)&&$parsed)$_POST=$parsed;
}
function rfP($k,$d=''){
    rfBootstrapInput();
    $v=array_key_exists($k,$_POST)?$_POST[$k]:$d;
    if(is_array($v)&&substr((string)$k,-5)==='_json')return json_encode($v,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
    return $v;
}
function rfB64d($data){$data=(string)$data;$rem=strlen($data)%4;if($rem)$data.=str_repeat('=',4-$rem);return base64_decode(strtr($data,'-_','+/'),true);}
function rfAuthHeader(){
    ...
    if(function_exists('getallheaders')){foreach((array)getallheaders() as $n=>$v){if(strtolower((string)$n)==='authorization')return trim((string)$v);}}
    return '';
}
function rfBearer(){
    $h=rfAuthHeader();
    if($h===''||!preg_match('/^Bearer\\s+(.+)$/i',$h,$m))return '';
    return trim((string)$m[1]);
}
function rfVerifyToken($token){
    if(!defined('FIELDPLX_API_SECRET')||trim((string)FIELDPLX_API_SECRET)==='')rfRes(500,false,'Mobile API secret is not configured.');
    $p=explode('.',(string)$token);if(count($p)!==3)rfRes(401,false,'Invalid access token.');
    $hj=rfB64d($p[0]);$pj=rfB64d($p[1]);$sig=rfB64d($p[2]);
    $h=json_decode((string)$hj,true);$c=json_decode((string)$pj,true);
    if(!is_array($h)||!is_array($c)||$sig===false)rfRes(401,false,'Invalid access token.');
    if(($h['alg']??'')!=='HS256')rfRes(401,false,'Unsupported access token.');
    $expected=hash_hmac('sha256',$p[0].'.'.$p[1],FIELDPLX_API_SECRET,true);
    if(!hash_equals($expected,$sig))rfRes(401,false,'Invalid access token signature.');
    $now=time();
    if(isset($c['nbf'])&&(int)$c['nbf']>$now)rfRes(401,false,'Access token is not active yet.');
    if(!isset($c['exp'])||(int)$c['exp']<=$now)rfRes(401,false,'Access token has expired. Please sign in again.');
    if(isset($c['iss'])&&(string)$c['iss']!==(string)FIELDPLX_TOKEN_ISSUER)rfRes(401,false,'Invalid access token issuer.');
    if(isset($c['aud'])&&(string)$c['aud']!==(string)FIELDPLX_TOKEN_AUDIENCE)rfRes(401,false,'Invalid access token audience.');
    if(empty($c['user_id'])||empty($c['tenant_id']))rfRes(401,false,'Invalid access token context.');
    return $c;
}
function rfRequireAuth(PDO $pdo){
    $token=rfBearer();if($token==='')rfRes(401,false,'Authorization Bearer token is required.');
    $claims=rfVerifyToken($token);$uid=(int)$claims['user_id'];$tid=(int)$claims['tenant_id'];
    $s=$pdo->prepare("SELECT u.id,u.tenant_id,u.branch_id,u.department_id,u.role_id,u.status user_status,t.status tenant_status,b.status branch_status,d.status department_status,r.status role_status FROM users u INNER JOIN tenants t ON t.id=u.tenant_id AND t.deleted_at IS NULL LEFT JOIN branches b ON b.id=u.branch_id AND b.tenant_id=u.tenant_id LEFT JOIN departments d ON d.id=u.department_id AND d.tenant_id=u.tenant_id LEFT JOIN roles r ON r.id=u.role_id AND r.tenant_id=u.tenant_id WHERE u.id=:u AND u.tenant_id=:t AND u.deleted_at IS NULL LIMIT 1");
    $s->execute(array(':u'=>$uid,':t'=>$tid));$u=$s->fetch(PDO::FETCH_ASSOC);
    if(!$u)rfRes(401,false,'Authenticated user account was not found.');
    if((string)$u['user_status']!=='active')rfRes(403,false,'Your user account is not active.');
    if(!in_array((string)$u['tenant_status'],array('trial','active'),true))rfRes(403,false,'This business account is not active.');
    if(!empty($u['branch_id'])&&!empty($u['branch_status'])&&(string)$u['branch_status']!=='active')rfRes(403,false,'Your assigned branch is not active.');
    if(!empty($u['department_id'])&&!empty($u['department_status'])&&(string)$u['department_status']!=='active')rfRes(403,false,'Your assigned department is not active.');
    if(!empty($u['role_id'])&&!empty($u['role_status'])&&(string)$u['role_status']!=='active')rfRes(403,false,'Your assigned role is not active.');
    if(rfTable($pdo,'subscriptions')){
        $q=$pdo->prepare("SELECT status,expiry_date FROM subscriptions WHERE tenant_id=:t ORDER BY id DESC LIMIT 1");$q->execute(array(':t'=>$tid));$sub=$q->fetch(PDO::FETCH_ASSOC);
        if($sub){if(!in_array((string)$sub['status'],array('active','trial'),true))rfRes(403,false,'Your subscription is not active.');if(!empty($sub['expiry_date'])&&(string)$sub['expiry_date']<date('Y-m-d'))rfRes(403,false,'Your subscription has expired.');}
    }
    return array('tenant_id'=>$tid,'user_id'=>$uid,'branch_id'=>!empty($u['branch_id'])?(int)$u['branch_id']:0,'claims'=>$claims);
}
...
function rfSmtpConfig(PDO $pdo,$tenant,$branch){
    /* Business modules use the single active/default PLATFORM SMTP only. */
    if(function_exists('fieldplxPlatformSmtpConfig')) return fieldplxPlatformSmtpConfig($pdo);
    return null;
}
...
function rfMeta(PDO $pdo,$tenant,$branch){
    ...
    if(rfTable($pdo,'products')){
        ...
        $productSql="SELECT p.id AS source_id,$skuExpr AS sku,p.name,$descExpr AS description,$costExpr AS unit_cost,$priceExpr AS unit_price,$taxExpr AS tax_percent FROM products p".$taxJoin." WHERE p.tenant_id=:t AND p.status='active'";
    ...
    $m['smtp_configured']=rfSmtp($pdo,$tenant,$branch)!==null;
    return $m;
}
...
function rfSaveRequestChecklist(PDO $pdo,$tenant,$requestId,$createdBy,$raw,$jobId=null,$primaryUser=null){
    $raw=trim((string)$raw);if($raw==='')return array('template_ids'=>array(),'request_links'=>array(),'job_checklists'=>array());
    foreach(array('checklist_templates','checklist_template_items','service_request_checklists') as $t){if(!rfTable($pdo,$t))rfRes(500,false,'Run migration_request_jobber_ui.sql once before using Create a Checklist on requests.');}
    $c=json_decode($raw,true);if(!is_array($c))rfRes(422,false,'Checklist data is invalid.');$name=substr(trim((string)(isset($c['name'])?$c['name']:'')),0,190);$items=isset($c['items'])&&is_array($c['items'])?$c['items']:array();if($name===''||!$items)rfRes(422,false,'The checklist needs a name and at least one question.');
    $s=$pdo->prepare("INSERT INTO checklist_templates(tenant_id,name,description,status,created_by) VALUES(:t,:n,:d,'active',:u)");$s->execute(array(':t'=>$tenant,':n'=>$name,':d'=>!empty($c['description'])?$c['description']:null,':u'=>$createdBy));$templateId=(int)$pdo->lastInsertId();
    $ins=$pdo->prepare("INSERT INTO checklist_template_items(checklist_template_id,title,description,is_required,sort_order) VALUES(:ct,:title,:d,:r,:o)");$order=0;
    foreach($items as $item){$title=substr(trim((string)(isset($item['title'])?$item['title']:'')),0,255);if($title==='')continue;$order++;$meta=array('section_title'=>isset($item['section_title'])?(string)$item['section_title']:'','question_type'=>isset($item['question_type'])?(string)$item['question_type']:'short_answer','options'=>isset($item['options'])&&is_array($item['options'])?$item['options']:array());$ins->execute(array(':ct'=>$templateId,':title'=>$title,':d'=>json_encode($meta,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),':r'=>!empty($item['is_required'])||!empty($item['required'])?1:0,':o'=>$order));}
    if($order===0)rfRes(422,false,'The checklist needs at least one valid question.');
    $link=$pdo->prepare("INSERT INTO service_request_checklists(tenant_id,request_id,checklist_template_id,created_by) VALUES(:t,:r,:ct,:u)");$link->execute(array(':t'=>$tenant,':r'=>$requestId,':ct'=>$templateId,':u'=>$createdBy));$requestLinkId=(int)$pdo->lastInsertId();$jobChecklistIds=array();
    if($jobId&&rfTable($pdo,'job_checklists')&&rfTable($pdo,'job_checklist_items')){
        $cols="tenant_id,job_id,visit_id,checklist_template_id,name,status";$vals=":t,:j,NULL,:ct,:n,'open'";$pa=array(':t'=>$tenant,':j'=>$jobId,':ct'=>$templateId,':n'=>$name);if(rfCol($pdo,'job_checklists','assigned_user_id')){$cols.=',assigned_user_id';$vals.=',:au';$pa[':au']=$primaryUser?$primaryUser:null;}$jc=$pdo->prepare("INSERT INTO job_checklists($cols) VALUES($vals)");$jc->execute($pa);$jid=(int)$pdo->lastInsertId();$jobChecklistIds[]=$jid;$it=$pdo->prepare("SELECT title,description,is_required,sort_order FROM checklist_template_items WHERE checklist_template_id=:ct ORDER BY sort_order,id");$it->execute(array(':ct'=>$templateId));foreach($it->fetchAll() as $x){
    ...
    return array('template_ids'=>array($templateId),'request_links'=>array($requestLinkId),'job_checklists'=>$jobChecklistIds);
}
...
function rfSaveRequestUploads(PDO $pdo,$tenant,$requestId,$user){
    ...
    $base=dirname(__DIR__,2).'/uploads/requests/'.$tenant.'/'.$requestId;
...
$auth=rfRequireAuth($pdo);
$tenant=(int)$auth['tenant_id'];
$user=(int)$auth['user_id'];
$sessionBranch=(int)$auth['branch_id'];
rfBootstrapInput();
$method=strtoupper((string)($_SERVER['REQUEST_METHOD']??'GET'));
$a=trim((string)rfP('action',''));

/* REST-friendly Android aliases while keeping all original action names. */
if($method==='GET'){
    if(isset($_GET['meta'])&&(string)$_GET['meta']==='1')$a='meta';
    elseif(isset($_GET['locations'])&&(string)$_GET['locations']==='1'){$a='locations';if(!isset($_POST['client_id'])&&isset($_GET['client_id']))$_POST['client_id']=$_GET['client_id'];}
    elseif(isset($_GET['id'])){$a='get_edit';$_POST['request_id']=$_GET['id'];}
    elseif($a==='')$a='meta';
}else if(in_array($method,array('PUT','PATCH'),true)){
    if($a==='')$a='update';
    if(!isset($_POST['request_id'])&&isset($_GET['id']))$_POST['request_id']=$_GET['id'];
}else if($method==='POST'){
    if($a==='')$a='create';
}else{
    rfRes(405,false,'Method not allowed.');
}
try{
...
if($a==='create_catalog_item'){
  ...
  if($itemType==='service'){
    ...
    if($existing){$existing['id']=(int)$existing['id'];$existing['source_id']=$existing['id'];$existing['item_source']='service';rfRes(200,true,'Existing service selected.',array('created'=>0,'item'=>$existing));}
    ...
    $id=(int)$pdo->lastInsertId();
    $item=array('id'=>$id,'source_id'=>$id,'item_source'=>'service','name'=>$name,'item_type'=>'service','description'=>$description,'unit_cost'=>$unitCost,'unit_price'=>$unitPrice,'tax_percent'=>$taxPercent);
  ...
  if($existing){
      ...
      $item=array('id'=>-$realId,'source_id'=>$realId,'item_source'=>'product','name'=>$existing['name'],'item_type'=>'product','description'=>isset($existing['description'])?$existing['description']:'','unit_cost'=>isset($existing['base_unit_price'])?(float)$existing['base_unit_price']:0,'unit_price'=>isset($existing['selling_price'])?(float)$existing['selling_price']:0,'tax_percent'=>isset($existing['tax_percent'])?(float)$existing['tax_percent']:0);
  ...
  $item=array('id'=>-$realId,'source_id'=>$realId,'item_source'=>'product','name'=>$name,'item_type'=>'product','description'=>$description,'unit_cost'=>$unitCost,'unit_price'=>$unitPrice,'tax_percent'=>$taxPercent);
...
if($a==='create'){
  ...
  $checkRaw=trim((string)rfP('new_checklist_json',''));if($checkRaw!==''&&!rfTable($pdo,'service_request_checklists'))rfRes(500,false,'Run migration_request_jobber_ui.sql once before using Create a Checklist on requests.');
  ...
  try{
    ...
    $checkInfo=rfSaveRequestChecklist($pdo,$tenant,$requestId,$user,$checkRaw,$jobId,$primary);
    $pdo->commit();
  }catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
  $uploads=rfSaveRequestUploads($pdo,$tenant,$requestId,$user);
  $mailBranch=$branch!==null?$branch:$sessionBranch;
  $customerEmail=rfCustomerRequestEmail($pdo,$tenant,$mailBranch,$client,$requestId,$requestNo,$title,$description,$assessment);
  rfLog($pdo,$tenant,$mailBranch,$user,$customerEmail['status']==='sent'?'service_request_customer_email_sent':($customerEmail['status']==='failed'?'service_request_customer_email_failed':'service_request_customer_email_skipped'),'service_request',$requestId,$client,'Service request customer email: '.$requestNo,array('recipient'=>$customerEmail['recipient'],'status'=>$customerEmail['status'],'message'=>$customerEmail['message'],'smtp_config_id'=>$customerEmail['smtp_config_id'],'mail_log_id'=>$customerEmail['mail_log_id']));
  $notify=array('in_app'=>0,'email_sent'=>0,'email_failed'=>0,'email_skipped'=>0,'email_messages'=>array(),'smtp_config_id'=>null,'smtp_config_name'=>null);
